...
Die **`load8_lane`** [SIMD Lade-/Speicheranweisung](/de/docs/WebAssembly/Reference/SIMD/load_store) lädt einen einzelnen Wert von einer gegebenen Speicheradresse in die festgelegte Lane eines [`v128`](/de/docs/WebAssembly/Reference/Value_types/v128) Typs `i8x16` Wertinterpretation.
...
- `align=int` {{optional_inline}}
  - : Ein Integer, der der Wasm-Engine einen Hinweis darauf gibt, welche Ausrichtung für die endgültige Adresse erwartet wird. Der minimale, standardmäßige und maximale Wert ist `1`.
- `lane_value`
...
> [!NOTE]
> Während das Wasm-Textformat den buchstäblichen `align` Wert angibt, stellt das binäre Äquivalent den Exponenten der Formel `2^x` dar, die zur Berechnung der Ausrichtung verwendet wird. Also entspricht `align=1` `0x00` (`2^0`).

---
title: Character
slug: Glossary/Character
l10n:
  sourceCommit: 50e5e8a9b8a6b7d0dd9877610c9639d8b90f329f
---

{{GlossarySidebar}}

Ein **Zeichen** ist entweder ein Symbol (Buchstabe, Zahl, Satzzeichen) oder ein nicht druckbares "Steuerelement" (z.B. Wagenrücklauf oder bedingter Trennstrich). {{Glossary("UTF-8", "UTF-8")}} ist der am häufigsten verwendete Zeichensatz und enthält die Grapheme der meisten beliebten menschlichen Sprachen.

## Siehe auch

- [Character (computing)](<https://en.wikipedia.org/wiki/Character_(computing)>) auf Wikipedia
- [Zeichenkodierung](https://en.wikipedia.org/wiki/Character_encoding) auf Wikipedia
- Verwandte Glossarbegriffe:
  - {{Glossary("ASCII", "ASCII")}}
- [UTF-8](https://en.wikipedia.org/wiki/UTF-8) auf Wikipedia
- [Unicode](https://en.wikipedia.org/wiki/Unicode) auf Wikipedia

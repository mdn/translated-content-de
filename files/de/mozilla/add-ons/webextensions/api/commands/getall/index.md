...
Die Befehle werden als Array von {{WebExtAPIRef('commands.Command')}} Objekten zurückgegeben. Alternativ, wenn Sie die auf `Promise` basierende Version der API verwenden, `browser.commands.getAll()`, werden die Befehle an das `onFulfilled` Argument von [`Promise.then()`](/de/docs/Web/JavaScript/Reference/Global_Objects/Promise/then) übergeben.

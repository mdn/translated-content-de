...
- Das Attribut `pattern` bei {{HTMLElement("input")}} wird jetzt als regulärer Ausdruck mit dem `"u"` (unicode) Flag behandelt ([Firefox-Bug 1227906](https://bugzil.la/1227906)).
...
- Gecko akzeptiert jetzt die `-webkit-` präfixierte Version einiger Eigenschaften; hierzu muss `layout.css.prefixes.webkit` auf `true` gesetzt werden ([Firefox-Bug 1213126](https://bugzil.la/1213126)).
- Die experimentelle Unterstützung des Deskriptors {{cssxref("@font-face/font-display", "font-display")}} (von {{cssxref("@font-face")}}; muss `layout.css.font-display.enabled` auf `true` gesetzt werden ([Firefox-Bug 1157064](https://bugzil.la/1157064)).
- Unterstützung für [`@media (-webkit-transform-3d)`](/de/docs/Web/CSS/Reference/At-rules/@media/-webkit-transform-3d) als Medientyp für 3D-Transformationsunterstützung, wenn der about:config Präferenz `layout.css.prefixes.webkit` auf `true` gesetzt ist ([Firefox-Bug 1239799](https://bugzil.la/1239799)).
...
- Hinzugefügtes `-webkit-filter` für Web-Kompatibilität, hinter der Präferenz `layout.css.prefixes.webkit`, standardmäßig `false` ([Firefox-Bug 1236506](https://bugzil.la/1236506)).
- \[css-align] "unsafe start" (ehemals "true start") sollte zu "start" serialisiert werden, etc. ([Firefox-Bug 1230398](https://bugzil.la/1230398)).

### JavaScript

- Die ES2015 {{jsxref("RegExp.prototype.unicode", "RegExp unicode (u) flag", "", 1)}} wurde implementiert ([Firefox-Bug 1135377](https://bugzil.la/1135377)).
- Die ES2015 Blockbereichs-Funktionen wurden implementiert ([Firefox-Bug 1071646](https://bugzil.la/1071646)).
...
- Die Neudeklaration von [`let`](/de/docs/Web/JavaScript/Reference/Statements/let) und [`const`](/de/docs/Web/JavaScript/Reference/Statements/const) Variablen erzeugt nun einen {{jsxref("SyntaxError")}} anstelle eines {{jsxref("TypeError")}} gemäß der ECMAScript-Spezifikation ([Firefox-Bug 1198833](https://bugzil.la/1198833)).
- Im [Strict-Modus](/de/docs/Web/JavaScript/Reference/Strict_mode) wird das Setzen von Eigenschaften auf {{Glossary("primitive", "primitiven")}} Werten nun einen {{jsxref("TypeError")}} auslösen ([Firefox-Bug 603201](https://bugzil.la/603201)).
...
- Eingebaute Zugriffsfunktionnamen haben nun ein "get" oder "set" Präfix ([Firefox-Bug 1180290](https://bugzil.la/1180290), [Firefox-Bug 1235656](https://bugzil.la/1235656)).
- [JS1.7/JS1.8 (veraltete) Array- und Generator-Comprehensions](/de/docs/Web/JavaScript/Reference/Deprecated_and_obsolete_features#legacy_generator_and_iterator) wurden entfernt ([Firefox-Bug 1220564](https://bugzil.la/1220564)).

### Schnittstellen/APIs/DOM

#### DOM & HTML DOM

- Die veraltete Methode `Window.showModalDialog()` ist nicht mehr verfügbar, wenn Firefox im Mehrprozess-Modus (e10s) läuft ([Firefox-Bug 1234700](https://bugzil.la/1234700)).
...
- Wenn eine nicht vorhandene Option eines {{HTMLElement("select")}} Elements programmgesteuert ausgewählt wird, wird anstatt sie fälschlicherweise unverändert zu belassen, jetzt der [`selectedIndex`](/de/docs/Web/API/HTMLSelectElement/selectedIndex) Wert auf `-1` gesetzt, die [`selectedOptions`](/de/docs/Web/API/HTMLSelectElement/selectedOptions) auf eine leere [`HTMLCollection`](/de/docs/Web/API/HTMLCollection), und der [`value`](/de/docs/Web/API/HTMLSelectElement/value) auf einen leeren String ([Firefox-Bug 1203668](https://bugzil.la/1203668)).
...
- [`Navigator.serviceWorker`](/de/docs/Web/API/Navigator/serviceWorker) ist jetzt als SameObject markiert (siehe [Firefox-Bug 1238205](https://bugzil.la/1238205)).
- [`ExtendableMessageEvent.ports`](/de/docs/Web/API/ExtendableMessageEvent/ports) ist jetzt als SameObject markiert (siehe [Firefox-Bug 1238225](https://bugzil.la/1238225)).
...
- Die Methode [`RTCPeerConnection.createOffer()`](/de/docs/Web/API/RTCPeerConnection/createOffer) unterstützt jetzt den VP9-Videocodec, obwohl dies standardmäßig deaktiviert ist. Um ihn zu aktivieren, setzen Sie die Präferenz `media.peerconnection.video.vp9_enabled` auf `true` in `about:config`. Bei Aktivierung ist VP9 der bevorzugte Codec; zuvor war VP8 bevorzugt ([Firefox-Bug 1242324](https://bugzil.la/1242324)).
- Die Methode [`RTCRtpSender.setParameters()`](/de/docs/Web/API/RTCRtpSender/setParameters) wurde hinzugefügt, um die Werte der Parameter nach der Erstellung des [`RTCRtpSender`](/de/docs/Web/API/RTCRtpSender) ändern zu können.

#### Neue APIs

- In SVG implementiert die [`SVGStyleElement`](/de/docs/Web/API/SVGStyleElement) Schnittstelle nun das `LinkStyle` Mixin ([Firefox-Bug 1239128](https://bugzil.la/1239128)).
...
- Die [Permissions API](/de/docs/Web/API/Permissions_API) ist jetzt standardmäßig in allen Veröffentlichungen aktiviert, nicht nur in Nightly, wie es zuvor der Fall war ([Firefox-Bug 1221106](https://bugzil.la/1221106)).
...
- Unterstützung für {{rfc(7686)}} wurde hinzugefügt: standardmäßig wird kein Versuch unternommen, Domainnamen mit der TLD `.onion` aufzulösen. Dies wird durch die Präferenz `network.dns.blockDotOnion` gesteuert. Add-ons, die Tor unterstützen, können diese Präferenz ändern. ([Firefox-Bug 1228457](https://bugzil.la/1228457))

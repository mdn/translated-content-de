...
- Bei einem ungültigen `type`-Wert wird {{HTMLElement("ul")}} nicht mehr auf `decimal` abgebildet, sondern verhält sich jetzt so, als wäre kein `type`-Wert angegeben ([Firefox-Bug 241719](https://bugzil.la/241719)).
...
- Unsere Implementierung von CSS Grids wurde aktualisiert:
  ...
  - Der `true`-Wert wurde in `unsafe` umbenannt; dies wirkt sich auf die Eigenschaften {{cssxref("justify-content")}}, {{cssxref("align-content")}}, {{cssxref("justify-self")}}, {{cssxref("align-self")}}, {{cssxref("justify-items")}} und {{cssxref("align-items")}} aus ([Firefox-Bug 1230478](https://bugzil.la/1230478)).
...
- Gecko akzeptiert jetzt die `-webkit-`-präfixierte Version von [einigen Eigenschaften](https://wiki.mozilla.org/Compatibility/Mobile/Non_Standard_Compatibility); es erfordert das Umstellen von `layout.css.prefixes.webkit` auf `true` ([Firefox-Bug 1213126](https://bugzil.la/1213126)).
- Der experimentelle Support für den {{cssxref("@font-face/font-display", "font-display")}}-Deskriptor (von {{cssxref("@font-face")}}; erfordert das Umstellen von `layout.css.font-display.enabled` auf `true` ([Firefox-Bug 1157064](https://bugzil.la/1157064)).
- Unterstützung für [`@media (-webkit-transform-3d)`](/de/docs/Web/CSS/@media/-webkit-transform-3d) als Media Query für 3D-Transform-Unterstützung hinzugefügt, wenn in about:config der Pref `layout.css.prefixes.webkit` auf `true` gesetzt ist ([Firefox-Bug 1239799](https://bugzil.la/1239799)).
...
- `-webkit-filter` für Web-Kompatibilität hinzugefügt, hinter der Präferenz `layout.css.prefixes.webkit`, standardmäßig auf `false` ([Firefox-Bug 1236506](https://bugzil.la/1236506)).
- \[css-align] "unsafe start" (ehemals "true start") sollte als "start" etc. serialisiert werden ([Firefox-Bug 1230398](https://bugzil.la/1230398)).
...
- Die erneute Deklaration von [`let`](/de/docs/Web/JavaScript/Reference/Statements/let) und [`const`](/de/docs/Web/JavaScript/Reference/Statements/const) Variablen löst jetzt einen {{jsxref("SyntaxError")}} anstelle eines {{jsxref("TypeError")}} gemäß der ECMAScript-Spezifikation aus ([Firefox-Bug 1198833](https://bugzil.la/1198833)).
- Im [Strict-Modus](/de/docs/Web/JavaScript/Reference/Strict_mode) führt das Festlegen von Eigenschaften auf {{Glossary("primitive", "primitive")}} Werte jetzt zu einem {{jsxref("TypeError")}} ([Firefox-Bug 603201](https://bugzil.la/603201)).
...
- [JS1.7/JS1.8 (veraltete) Array-Konstrukte und Generator-Konstrukte](/de/docs/Web/JavaScript/Reference/Deprecated_and_obsolete_features#legacy_generator_and_iterator) wurden entfernt ([Firefox-Bug 1220564](https://bugzil.la/1220564)).
...
- Die [`ImageBitmap.close()`](/de/docs/Web/API/ImageBitmap/close) Methode wird nun unterstützt ([Firefox-Bug 1172796](https://bugzil.la/1172796)).
- Ein neuer [`ImageBitmapRenderingContext`](/de/docs/Web/API/ImageBitmapRenderingContext) Rendering-Kontext wurde implementiert. Verwenden Sie `"bitmaprenderer"` mit [`OffscreenCanvas.getContext()`](/de/docs/Web/API/OffscreenCanvas/getContext) oder [`HTMLCanvasElement.getContext()`](/de/docs/Web/API/HTMLCanvasElement/getContext), um diesen Kontext zu erhalten. ([Firefox-Bug 1172796](https://bugzil.la/1172796)).

#### WebGL

- Die [`WEBGL_compressed_texture_etc`](/de/docs/Web/API/WEBGL_compressed_texture_etc) Erweiterung wurde implementiert, um die Verwendung von [ETC2-komprimierten Texturformaten](https://en.wikipedia.org/wiki/Ericsson_Texture_Compression) zu ermöglichen ([Firefox-Bug 917505](https://bugzil.la/917505)). Um diese Erweiterung zu verwenden, setzen Sie die Präferenz `webgl.enable-draft-extensions` auf `true` in about:config.
...
- [`FetchEvent.request`](/de/docs/Web/API/FetchEvent/request) ist jetzt nicht-null-bar (siehe [Firefox-Bug 1238213](https://bugzil.la/1238213).)
- [`Navigator.serviceWorker`](/de/docs/Web/API/Navigator/serviceWorker) wurde nun als SameObject markiert (siehe [Firefox-Bug 1238205](https://bugzil.la/1238205).)
- [`ExtendableMessageEvent.ports`](/de/docs/Web/API/ExtendableMessageEvent/ports) wurde nun als SameObject markiert (siehe [Firefox-Bug 1238225](https://bugzil.la/1238225).)

#### Fetch

- [`Request.mode`](/de/docs/Web/API/Request/mode) hat jetzt einen neuen Wert `navigate`, um Anfragen zu unterstützen, die beim Navigieren zwischen Dokumenten generiert werden (siehe [Firefox-Bug 1209081](https://bugzil.la/1209081).)

#### WebRTC

- Die [`RTCPeerConnection.createOffer()`](/de/docs/Web/API/RTCPeerConnection/createOffer) Methode unterstützt jetzt den VP9-Video-Codec, obwohl dieser standardmäßig deaktiviert ist. Um ihn zu aktivieren, setzen Sie die Präferenz `media.peerconnection.video.vp9_enabled` auf `true` in `about:config`. Wenn aktiviert, ist VP9 der bevorzugte Codec; zuvor war VP8 bevorzugt ([Firefox-Bug 1242324](https://bugzil.la/1242324)).
...
- In SVG implementiert die [`SVGStyleElement`](/de/docs/Web/API/SVGStyleElement) Schnittstelle nun das `LinkStyle`-Mischformular ([Firefox-Bug 1239128](https://bugzil.la/1239128)).
...
- Die [Permissions API](/de/docs/Web/API/Permissions_API) wurde nun standardmäßig aktiviert, für alle Release-Versionen und nicht nur für Nightly, wie es zuvor war ([Firefox-Bug 1221106](https://bugzil.la/1221106).)
...
- Unterstützung von {{rfc(7686)}} wurde hinzugefügt: standardmäßig gibt es keinen Versuch, eine Domain mit der TLD `.onion` aufzulösen. Dies wird durch die Präferenz `network.dns.blockDotOnion` gesteuert. Add-ons, die Tor unterstützen, können diesen Pref ändern. ([Firefox-Bug 1228457](https://bugzil.la/1228457))

...
Firefox 38 wurde am 12. Mai 2015 veröffentlicht. Dieser Artikel listet wichtige Änderungen auf, die nicht nur für Webentwickler nützlich sind, sondern auch für Entwickler von Firefox und Gecko sowie Entwickler von Add-ons.
...
- Die {{cssxref(":unresolved")}} Pseudoklasse wurde für benutzerdefinierte Elemente implementiert ([Firefox-Bug 1111633](https://bugzil.la/1111633)).
- Der vordefinierte Stil {{cssxref("list-style-type", "ethiopic-numeric")}} verwendet nun ein Leerzeichen statt eines Punkts als Suffix, um einer kürzlichen Änderung der Spezifikation zu entsprechen ([Firefox-Bug 1120721](https://bugzil.la/1120721)).
- CSS-Transitionen auf generierten Inhalten (mit {{cssxref("::before")}} und {{cssxref("::after")}}) beginnen sowohl in einer Inline als auch im Block, der sie teilt, nun wie in der Spezifikation erwartet ([Firefox-Bug 1110277](https://bugzil.la/1110277)).
- Die Implementierung von CSS Logischen Eigenschaften hat große Fortschritte gemacht. Die folgenden Eigenschaften sind hinter dem `layout.css.vertical-text.enabled`-Flag (`false` standardmäßig) verfügbar:
...
- Wie [CSS-Transitionen](/de/docs/Web/CSS/CSS_transitions/Using_CSS_transitions) beginnen, wurde geändert, um einer kürzlichen Änderung der Spezifikationen zu entsprechen und ein interoperables Verhalten zwischen Browsern zu erreichen ([Firefox-Bug 960465](https://bugzil.la/960465)).
...
- Doppelte Parameternamen sind in knappen Methodendefinitionen nicht mehr erlaubt ([Firefox-Bug 1096378](https://bugzil.la/1096378)).
...
- Die {{jsxref("WeakMap.get", "get")}}, {{jsxref("WeakMap.has", "has")}} und {{jsxref("WeakMap.delete", "delete")}}-Methoden von {{jsxref("WeakMap")}}-Objekten lösen keinen Fehler mehr aus, wenn der `key`-Parameter kein Objekt ist ([Firefox-Bug 1127827](https://bugzil.la/1127827)).
...
- {{jsxref("RegExp.prototype.source")}} und {{jsxref("RegExp.prototype.toString()")}} maskieren nun reguläre Ausdrucksmuster ordnungsgemäß (z. B. Zeilenenden, "\n") ([Firefox-Bug 1130860](https://bugzil.la/1130860)).
- Die {{jsxref("Regexp")}}-Eigenschaften {{jsxref("Regexp.global", "global")}}, {{jsxref("Regexp.ignoreCase", "ignoreCase")}}, {{jsxref("Regexp.multiline", "multiline")}} und {{jsxref("Regexp.sticky", "sticky")}} sind jetzt Prototyp-Accessor-Eigenschaften statt eigene Dateneigenschaften von `RegExp`-Instanzen ([Firefox-Bug 1120169](https://bugzil.la/1120169)).
- Die {{jsxref("RegExp.prototype.source")}}-Eigenschaft ist jetzt eine Prototyp-Accessor-Eigenschaft statt einer eigenen Dateneigenschaft von `RegExp`-Instanzen ([Firefox-Bug 1120169](https://bugzil.la/1120169)). Verfügbar nur in Nicht-Release-Versionen, aufgrund von [Firefox-Bug 1150297](https://bugzil.la/1150297).
- {{jsxref("Function.prototype.toString()")}} löst nun Fehler für {{jsxref("Proxy")}}-Objekte aus ([Firefox-Bug 1100936](https://bugzil.la/1100936)).
...
- [`KeyboardEvent.code`](/de/docs/Web/API/KeyboardEvent/code) unterstützt jetzt Sondertasten auf Sun-Tastaturen auf Linux, Android und Firefox OS. ([Firefox-Bug 1020139](https://bugzil.la/1020139))
...
- Sie können nun [WebSockets](/de/docs/Web/API/WebSockets_API) in [Workers](/de/docs/Web/API/Web_Workers_API) verwenden ([Firefox-Bug 504553](https://bugzil.la/504553)).
- Die Eigenschaften [`XMLHttpRequest.responseType`](/de/docs/Web/API/XMLHttpRequest/responseType) und [`XMLHttpRequest.withCredentials`](/de/docs/Web/API/XMLHttpRequest/withCredentials) können jetzt festgelegt werden, bevor [`XMLHttpRequest.open()`](/de/docs/Web/API/XMLHttpRequest/open) aufgerufen wird ([Firefox-Bug 707484](https://bugzil.la/707484)).
...
- In Firefox wird das `autocomplete`=false-Attribut beim Umgang mit einem Anmeldeformular jetzt ignoriert ([Firefox-Bug 1025703](https://bugzil.la/1025703)). Dies soll die Verwendung sichererer Passwörter fördern, indem Passwort-Manager-Tools zuverlässiger arbeiten können.
- RC4 ist jetzt beim Einsatz von TLS deaktiviert, außer für einige explizit erlaubte Websites. Diese Ausnahmeliste ist eine Zwischenlösung, bis diese Seiten verbessert werden ([Firefox-Bug 1124039](https://bugzil.la/1124039)). Dieses Fallback wird von der `security.tls.unrestricted_rc4_fallback`-Einstellung kontrolliert, die momentan standardmäßig auf `true` gesetzt ist ([Firefox-Bug 1138882](https://bugzil.la/1138882)).
- Websites, die auf eine unsichere Version von TLS zurückfallen müssen, um zu funktionieren, sind jetzt in einer fest kodierten Ausnahmeliste, die mit der Zeit verkleinert wird ([Firefox-Bug 1114816](https://bugzil.la/1114816)). Die Liste kann deaktiviert werden, indem `security.tls.insecure_fallback_hosts.use_static_list` auf `false` gesetzt wird.
...
- `toolkit/loader` verfügt nun über eine optionale Modulkompatibilitätsprüfung, die für alle jpm-Add-ons aktiviert ist ([Firefox-Bug 1037235](https://bugzil.la/1037235)).
...
- [GitHub-Komits, die zwischen Firefox 37 und Firefox 38 gemacht wurden](https://github.com/mozilla/addon-sdk/compare/firefox36...firefox38).
...
- [`DownloadTarget`](/de/docs/Mozilla/JavaScript_code_modules/Downloads.jsm/DownloadTarget)-Objekte haben jetzt Eigenschaften `exists` und `size`, die es Ihnen ermöglichen, das Vorhandensein und die Größe der Ziel-Datei des Downloads auf der Festplatte zu bestimmen, sowie eine neue `refresh()`-Methode, die verlangt, dass diese Werte aktualisiert werden.

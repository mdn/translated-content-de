...
- Das {{cssxref("::placeholder")}} Pseudo-Element ist jetzt unpräfixiert ([Firefox-Bug 1069012](https://bugzil.la/1069012)).
- Der {{cssxref(":valid")}} CSS-Pseudoklasse-Fehler, der keine gültigen {{HTMLElement("form")}}-Elemente erkannte, wurde behoben ([Firefox-Bug 1285425](https://bugzil.la/1285425)).
...
- Die Höhe der Flexlinie wird in einem einzeiligen Auto-Höhe-Flex-Container mit max-height nun korrekt beschränkt (Spezifikationsänderung) ([Firefox-Bug 1000957](https://bugzil.la/1000957)).
...
- Die Verwendung von {{jsxref("Statements/const", "const")}} in [`for...of`](/de/docs/Web/JavaScript/Reference/Statements/for...of)-Schleifen hat jetzt bei jeder Iteration eine frische Bindung und wirft keinen {{jsxref("SyntaxError")}} mehr ([Firefox-Bug 1101653](https://bugzil.la/1101653)).
...
- [Generator-Funktionen](/de/docs/Web/JavaScript/Reference/Statements/function*) können keine [Labels](/de/docs/Web/JavaScript/Reference/Statements/label) mehr haben, und `let` als Label-Name ist jetzt unzulässig ([Firefox-Bug 1288459](https://bugzil.la/1288459)).
...
- Mit einem Parent für Kind-indizierte Pseudoklasse-Selektoren sollte eine Übereinstimmung erzielt werden ([Firefox-Bug 1300374](https://bugzil.la/1300374)).

### Entwicklerwerkzeuge

- [Der Netzwerkmonitor zeigt jetzt einen "Blockiert"-Zustand für Netzwerkabfragen an.](https://firefox-source-docs.mozilla.org/devtools-user/network_monitor/index.html#timings)
...
- [WebGL 2](/de/docs/Web/API/WebGL_API) ist jetzt standardmäßig aktiviert. Besuchen Sie [webglsamples.org/WebGL2Samples](https://webglsamples.org/WebGL2Samples/) für einige Demos.
  ...
  - WebGL 2 bietet das Interface [`WebGL2RenderingContext`](/de/docs/Web/API/WebGL2RenderingContext), welches OpenGL ES 3.0 auf das {{HTMLElement("canvas")}}-Element bringt.
...
- Die `WEBGL_compressed_texture_es3`-Erweiterung (in Firefox 46 implementiert) wurde umbenannt in [`WEBGL_compressed_texture_etc`](/de/docs/Web/API/WEBGL_compressed_texture_etc) ([Firefox-Bug 1316778](https://bugzil.la/1316778)) und ist nicht mehr standardmäßig in WebGL 2-Kontexten enthalten ([Firefox-Bug 1306174](https://bugzil.la/1306174)).
...
- [IndexedDB](/de/docs/Web/API/IndexedDB_API) Version 2 Implementierung ist jetzt vollständig:
...
- Die Methode [`CanvasRenderingContext2D.imageSmoothingEnabled`](/de/docs/Web/API/CanvasRenderingContext2D/imageSmoothingEnabled) wurde unpräfixiert ([Firefox-Bug 768072](https://bugzil.la/768072))

### SVG

- Das Attribut {{SVGAttr("tabindex")}} wurde hinzugefügt ([Firefox-Bug 778654](https://bugzil.la/778654)).
- Das Attribut {{SVGAttr("href")}} wurde hinzugefügt, welches {{SVGAttr("xlink:href")}} obsolet macht ([Firefox-Bug 1245751](https://bugzil.la/1245751)).
- Sie können jetzt benutzerdefinierte Datenattribute auf SVG-Elementen über die Eigenschaft [`HTMLElement.dataset`](/de/docs/Web/API/HTMLElement/dataset) und die Attributreihe {{SVGAttr("data-*")}} verwenden ([Firefox-Bug 921834](https://bugzil.la/921834)).
- CSS-Animationen, die in einem SVG-Bild verwendet werden, das in einem {{HTMLElement("img")}}-Element angezeigt wird, funktionieren jetzt wieder; dies war ein alter Regression ([Firefox-Bug 1190881](https://bugzil.la/1190881)).

### Web Worker

- Der nicht standardisierte und veraltete `onclose`-Ereignishandler und die Nutzung des `close`-Ereignisses durch [`Worker`](/de/docs/Web/API/Worker) wurden aus Firefox entfernt.

### Netzwerke

- Skripte, die mit einem `image/*`, `video/*`, `audio/*` oder `text/csv` MIME-Typ ausgeliefert werden, werden jetzt blockiert und nicht geladen oder ausgeführt. Dies geschieht, wenn sie mit {{HTMLElement("script")}} deklariert oder über [`WorkerGlobalScope.importScripts()`](/de/docs/Web/API/WorkerGlobalScope/importScripts), [`Worker()`](/de/docs/Web/API/Worker/Worker), [`SharedWorker()`](/de/docs/Web/API/SharedWorker/SharedWorker) geladen werden ([Firefox-Bug 1229267](https://bugzil.la/1229267) und [Firefox-Bug 1288361](https://bugzil.la/1288361)).
...
- Neue Zertifikate von WoSign und StartCom werden nicht mehr akzeptiert ([Firefox-Bug 1309707](https://bugzil.la/1309707)), siehe [Distrusting New WoSign and StartCom Certificates](https://blog.mozilla.org/security/2016/10/24/distrusting-new-wosign-and-startcom-certificates/) für weitere Informationen.
...
- Die Eigenschaft [`XMLHttpRequest.responseXML`](/de/docs/Web/API/XMLHttpRequest/responseXML) gibt nicht mehr ein partielles [`Document`](/de/docs/Web/API/Document) mit einem \<parsererror>-Knoten an der Spitze zurück, wenn ein Parsing-Fehler beim Versuch, die empfangenen Daten zu interpretieren, auftritt. Stattdessen gibt sie korrekt `null` zurück ([Firefox-Bug 289714](https://bugzil.la/289714)).
- Um der neuesten Spezifikation zu entsprechen, wird ein [`XMLHttpRequest`](/de/docs/Web/API/XMLHttpRequest) jetzt mit einem {{HTTPHeader("Accept")}}-Header gesendet, der mit [`setRequestHeader()`](/de/docs/Web/API/XMLHttpRequest/setRequestHeader) gesetzt wurde, wenn keiner gesetzt ist, mit dem Wert `*/*` ([Firefox-Bug 918752](https://bugzil.la/918752)).
- Die Funktion [`XMLHttpRequest.open()`](/de/docs/Web/API/XMLHttpRequest/open) wurde korrigiert, sodass die Parameter `username` und `password` jetzt, wenn sie ausgelassen werden, standardmäßig auf `null` gesetzt werden, entsprechend der Spezifikation ([Firefox-Bug 933759](https://bugzil.la/933759)).
...
- Die Methode [`HTMLMediaElement.captureStream()`](/de/docs/Web/API/HTMLMediaElement/captureStream), die einen [`MediaStream`](/de/docs/Web/API/MediaStream) zurückgibt, der den Inhalt des angegebenen {{HTMLElement("video")}} oder {{HTMLElement("audio")}} enthält, ist noch als `mozCaptureStream()` präfixiert und entspricht nicht genau der Spezifikation.
...
- Sie können jetzt den Effekt einer Web-Animation mittels Setzen des Wertes ihrer [`effect`](/de/docs/Web/API/Animation/effect)-Eigenschaft ändern. Bisher war diese Eigenschaft nur lesbar ([Firefox-Bug 1049975](https://bugzil.la/1049975)).
- Die Permissions API-Methode [`Permissions.revoke()`](/de/docs/Web/API/Permissions/revoke) wurde hinter einer Einstellung (`dom.permissions.revoke.enable`) platziert und standardmäßig deaktiviert, da ihr Design und sogar ihre Existenz in der [Web Application Security Working Group](https://www.w3.org/2011/webappsec) diskutiert werden.
- Die [`Navigator.storage`](/de/docs/Web/API/Navigator/storage)-Eigenschaft der [Storage API](/de/docs/Web/API/Storage_API) und die Methode [`StorageManager.estimate()`](/de/docs/Web/API/StorageManager/estimate) wurden zusammen mit dem benötigten Support-Code implementiert. Funktionen zur Persistenz von Speicher sind noch nicht implementiert. Siehe [Firefox-Bug 1267941](https://bugzil.la/1267941).
...
- Die nicht standardisierte Simple Push API, hauptsächlich zur Verwendung mit Firefox OS vorgesehen und jetzt durch die [W3C Push API](/de/docs/Web/API/Push_API) ersetzt, wurde vollständig aus Gecko entfernt ([Firefox-Bug 1296579](https://bugzil.la/1296579)).
- Die nicht standardisierte Alarms API, hauptsächlich zur Verwendung mit Firefox OS vorgesehen, wurde vollständig aus Gecko entfernt ([Firefox-Bug 1300884](https://bugzil.la/1300884)).
...
- Sie können jetzt [ein WebExtension in einen Legacy-Add-on-Typ einbetten](/de/docs/Mozilla/Add-ons/WebExtensions/Embedded_WebExtensions) ([Firefox-Bug 1252215](https://bugzil.la/1252215)).
...
- [localStorage](/de/docs/Web/API/Window/localStorage) wird jetzt gelöscht, wenn ein WebExtension deinstalliert wird ([Firefox-Bug 1213990](https://bugzil.la/1213990))
...
- Die [Browser API](/de/docs/Mozilla/Gecko/Chrome/API/Browser_API) enthält jetzt ein `detail`-Objekt auf dem Ereignisobjekt des [`mozbrowserlocationchange`](/de/docs/Web/Events/mozbrowserlocationchange)-Ereignisses, das `canGoForward`/`canGoBack`-Eigenschaften enthält, um den Vor-/Zurück-Status des mozBrowser synchron abzurufen ([Firefox-Bug 1279635](https://bugzil.la/1279635)).

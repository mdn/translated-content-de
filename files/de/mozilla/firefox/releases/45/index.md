...
- Änderungen im Viewport oder eine Größenänderung lösen nun die Neuauswahl responsiver Bilder für `<img srcset>` aus ([Firefox-Bug 1166138](https://bugzil.la/1166138)).

### CSS

- {{cssxref("word-spacing")}} erlaubt jetzt Prozentwerte ([Firefox-Bug 1038663](https://bugzil.la/1038663)).
- Unsere Implementierung von CSS-Grids wurde verbessert und wird nicht mehr als experimentell betrachtet; sie ist jetzt standardmäßig in Nightly und Developer Edition aktiviert, jedoch nicht für Beta und Release ([Firefox-Bug 1000592](https://bugzil.la/1000592)):

  - Rinnen, das sind die Eigenschaften `grid-column-gap`, `grid-row-gap` und `grid-gap`, werden jetzt unterstützt ([Firefox-Bug 1176792](https://bugzil.la/1176792)).
  - Die implizierte Mindestgröße von Gitter-Elementen, das spezielle {{cssxref("min-width")}} und {{cssxref("min-height")}} `auto`-Verhalten, wurde implementiert ([Firefox-Bug 1176775](https://bugzil.la/1176775)).
  - {{cssxref("align-self")}} und {{cssxref("justify-self")}} werden nun bei Gitterlayouts unterstützt ([Firefox-Bug 1151213](https://bugzil.la/1151213)).
  - {{cssxref("align-content")}} und {{cssxref("justify-content")}} werden nun bei Gitterlayouts unterstützt ([Firefox-Bug 1151214](https://bugzil.la/1151214)).
...
- Volle Unterstützung für CSS-Box-Ausrichtung für CSS-Gitter implementieren, fehlende Werte unterstützen: `start`, `end`, `self-start`, `self-end`, `left`, `right`, `last-baseline`, `space-evenly` ([Firefox-Bug 1176782](https://bugzil.la/1176782)). CSS-Box-Ausrichtung gilt derzeit nur für CSS-Flexbox und CSS-Gitter.
- \[css-grid]\[css-flexbox] Raster/Flex-Layout für `<fieldset>` implementieren ([Firefox-Bug 1230207](https://bugzil.la/1230207)).
- Die Werte `inline-start` und `inline-end` wurden zu {{cssxref("float")}} und {{cssxref("clear")}} hinzugefügt ([Firefox-Bug 1122918](https://bugzil.la/1122918)). Sie sind standardmäßig auf Nightly und Aurora (Dev Edition) sowie auf Firefox OS aktiviert; um sie auf einer Release- oder Beta-Version zu aktivieren, müssen Sie `layout.css.float-logical-values.enabled` auf `true` setzen.
...
- Mehrere `-webkit`-Präfixe-Eigenschaften und -Werte wurden für die Web-Kompatibilität hinzugefügt, hinter der Präferenz `layout.css.prefixes.webkit`, die standardmäßig auf `false` steht:
...
- {{jsxref("Operators/Expression_closures", "Expression closures", "", 1)}} sind veraltet und zeigen jetzt eine Warnung in der Konsole an ([Firefox-Bug 995610](https://bugzil.la/995610)).
- {{jsxref("String.prototype.replace")}} stellt nach der Ausführung des Funktionsparameters die {{jsxref("Global_Objects/RegExp/n", "RegExp-Static-Eigenschaften", "", 1)}} nicht mehr wieder her ([Firefox-Bug 1226936](https://bugzil.la/1226936)).
- {{jsxref("Math.random()")}} wurde auf den verbesserten XorShift128+-Algorithmus aktualisiert ([Firefox-Bug 322529](https://bugzil.la/322529)).
...
- Das [`HTMLImageElement.srcset`](/de/docs/Web/API/HTMLImageElement/srcset)-Attribut reagiert nun auf Größenänderungen/Viewports ([Firefox-Bug 1166138](https://bugzil.la/1166138)).
...
- Das Optionsobjekt, das als Parameter beim Aufruf von [`Clients.matchAll()`](/de/docs/Web/API/Clients/matchAll) übergeben werden kann, kann jetzt eine `includeUncontrolled`-Eigenschaft enthalten. Dies ist ein boolescher Wert — wenn auf `true` gesetzt, wird die Übereinstimmungsoperation alle Service Worker-Clients zurückgeben, die denselben Ursprung wie der aktuelle Service Worker haben. Andernfalls gibt es nur die vom aktuellen Service Worker kontrollierten Service Worker-Clients zurück. Der Standardwert ist `false`.
...
- Nachdem Sie einen Videospur aus einem Medienstream mit [`MediaStream.removeTrack()`](/de/docs/Web/API/MediaStream/removeTrack) entfernt haben, können Sie jetzt später eine andere Videospur mit [`MediaStream.addTrack()`](/de/docs/Web/API/MediaStream/addTrack) hinzufügen und abspielen ([Firefox-Bug 1223696](https://bugzil.la/1223696)).
...
- Die SVG-Stroke-Treffererkennung ist fehlerhaft, wenn Cairo das Moz2D-Backend ist ([Firefox-Bug 676001](https://bugzil.la/676001)).
- Es ist nicht möglich, mit Elementen zu interagieren, die große Transformations-/Verschiebungswerte haben ([Firefox-Bug 1217012](https://bugzil.la/1217012)).
...
Ab Firefox 45 werden Such-Plugins, die sich im Verzeichnis `searchplugins` des Benutzerprofils befinden, nicht mehr automatisch beim Start geladen. Stattdessen wird eine Liste der vom Benutzer installierten Plugins gepflegt und nur diese Plugins werden geladen. Dies bedeutet, dass die einzigen Möglichkeiten, neue Such-Plugins zu installieren, darin bestehen, dass der Benutzer dies in der Firefox-UX tut (z.B. über [OpenSearch-Entdeckung](/de/docs/Mozilla/Add-ons/Creating_OpenSearch_plugins_for_Firefox)) oder ein Add-on, das es installiert. Außerdem wird beim Installieren eines neuen Such-Plugins mehr Informationen darüber aufgezeichnet, woher es stammt, für die zukünftige Verwendung durch Profil-Debugging- und Bereinigungstools.

### Sonstiges

- WebIDL-Konstruktoren konnten im Chrome-Kontext ohne den [`new`](/de/docs/Web/JavaScript/Reference/Operators/new)-Operator aufgerufen werden. Jetzt wird solch ein Code, ebenso wie im Web-Inhalt seit Firefox 30, einen [`TypeError`](/de/docs/Web/JavaScript/Reference/Global_Objects/TypeError) hervorrufen. Zum Beispiel muss `var req = XMLHttpRequest();` zu `var req = new XMLHttpRequest();` geändert werden.

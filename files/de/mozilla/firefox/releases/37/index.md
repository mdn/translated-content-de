...
- Das Quirks-Mode-Verhalten von {{cssxref("empty-cells")}} wurde entfernt: es ist jetzt standardmäßig `show` wie im Standardmodus ([Firefox Bug 1020400](https://bugzil.la/1020400)).
...
- Der `'.'` gefolgt von der Erweiterung ist nun im `<input accept>` erlaubt: Wenn verwendet, filtert ein Dateiauswahl-Dialog mit dieser gegebenen Erweiterung, um dem Benutzer vorgeschlagen zu werden ([Firefox Bug 826176](https://bugzil.la/826176)).
...
- Die ES2015 {{jsxref("Proxy")}} enumerate trap für [`for...in`](/de/docs/Web/JavaScript/Reference/Statements/for...in) Schleifen wurde implementiert ([Firefox Bug 783829](https://bugzil.la/783829)).
- Das Attribut `configurable` der Eigenschaft {{jsxref("Function.length")}} ist jetzt `true` gemäß der ES2015 Spezifikation ([Firefox Bug 911142](https://bugzil.la/911142)).
- Die Entwicklung von [ParallelJS (PJS)](https://web.archive.org/web/20161113115816/http://wiki.ecmascript.org/doku.php?id=strawman:data_parallelism) wurde eingestellt, da die begrenzten Zukunftsperspektiven, wenig Aufmerksamkeit und die Komplexität des Codes überwiegen. Die experimentelle Implementierung, die nur im Nightly-Kanal aktiviert war, einschließlich der Methoden `Array.prototype.mapPar`, `filterPar` und `reducePar`, wurde vollständig entfernt.
...
- In Übereinstimmung mit der sich entwickelnden WebRTC-Spezifikation haben wir `RTCIceServer.url` zugunsten von `RTCIceServer.urls` abgelehnt, womit Sie mehr als eine URL für einen bestimmten ICE-Server angeben können.
- Einige Tastenbezeichnungen von `KeyboardEvent.key` wurden geändert, um sich an [die neueste DOM Level 3 Events Spezifikation](https://w3c.github.io/DOM-Level-3-Events-key/) anzupassen. Siehe [die Tabellen der `KeyboardEvent.key` Werte in MDN](/de/docs/Web/API/KeyboardEvent/key#key_values). Die grünen Zellen sind neue Werte. Lila Werte sind noch instabil. Seien Sie vorsichtig, wenn Sie sie verwenden (Metabug für diese Änderungen ist [Firefox Bug 900372](https://bugzil.la/900372)).
...
- WebSockets unterstützt jetzt die `permessage` Komprimierungsmethode, wenn der Server diese unterstützt ([Firefox Bug 792831](https://bugzil.la/792831)).

## Sicherheit

- Die Verwendung von schwachen Protokollen oder Chiffren, wie SSL 3.0 und RC4, wird jetzt in der Konsole protokolliert, um Sites zu warnen, die diese verwenden ([Firefox Bug 1092835](https://bugzil.la/1092835)).

...
[Um die neuesten Entwicklerfunktionen von Firefox zu testen, installieren Sie die Firefox Developer Edition](https://www.firefox.com/en-US/channel/desktop/developer/). Firefox 48 wurde am 2. August 2016 veröffentlicht. Dieser Artikel listet wichtige Änderungen auf, die nicht nur für Webentwickler nützlich sind, sondern auch für Firefox und Gecko-Entwickler sowie Add-on-Entwickler.
...
- Verbesserte Vorschläge für CSS-Eigenschaften ([Firefox-Bug 1168246](https://bugzil.la/1168246)).
...
- Mehrere `-webkit`-präprafixierte Eigenschaften und Werte wurden zur Webkompatibilität hinzugefügt, hinter der Präferenz `layout.css.prefixes.webkit`, die standardmäßig auf `false` gesetzt ist:
  ...
  - Der Wert `-webkit-inline-box` ist jetzt ein Alias von `inline-flex` für die {{cssxref("display")}} Eigenschaft ([Firefox-Bug 1257661](https://bugzil.la/1257661)).
  ...
  - Hinzugefügt: `-webkit-box-flex`, `-webkit-box-ordinal-group`, `-webkit-box-align` und `-webkit-box-pack` Eigenschaften und `-webkit-box` Wert zu {{cssxref("display")}} als Aliase für moderne [CSS Flexbox](/de/docs/Web/CSS/Guides/Flexible_box_layout/Use_cases) ([Firefox-Bug 1208635](https://bugzil.la/1208635)).
...
- Der `absolute` Wert der {{cssxref("position")}} Eigenschaften auf dem obersten Layer-Element ([Firefox-Bug 1236828](https://bugzil.la/1236828)).
- Ein internes, nur für das systeminterne Syntax für {{cssxref("@supports")}} zur Erkennung von Prefs wurde hinzugefügt ([Firefox-Bug 1259889](https://bugzil.la/1259889)).
...
- Die `String.prototype.contains()` Methode wurde entfernt (sie war seit Version 40 veraltet). Verwenden Sie die {{jsxref("String.prototype.includes()")}} Methode stattdessen ([Firefox-Bug 1103588](https://bugzil.la/1103588)).
...
- Firefox gibt jetzt ordnungsgemäß Ausnahmen statt Zahlen zurück, wenn etwas während eines Aufrufs von [`Navigator.registerProtocolHandler()`](/de/docs/Web/API/Navigator/registerProtocolHandler) schiefgeht.
...
- `Node.localName`, `Node.namespaceURI` und `Node.prefix` wurden in die APIs [`Element`](/de/docs/Web/API/Element) und [`Attr`](/de/docs/Web/API/Attr) verschoben ([Firefox-Bug 1055776](https://bugzil.la/1055776)).
- Entsprechend der neuesten Spezifikation wurden die von [`KeyboardEvent.code`](/de/docs/Web/API/KeyboardEvent/code) zurückgegebenen Werte für die folgenden Tasten geändert; siehe ([Firefox-Bug 1264150](https://bugzil.la/1264150)):
...
- Der Eintrag `iceRestart` wird jetzt im Code-Wörterbuch `RTCOfferOptions` unterstützt, sodass [`createOffer()`](/de/docs/Web/API/RTCPeerConnection/createOffer) verwendet werden kann, um ICE-Neustarts anzufordern ([Firefox-Bug 906986](https://bugzil.la/906986)).
...
- WebM/VP8-Video, das Videoauflösungsänderungen enthält und mit [`MediaRecorder`](/de/docs/Web/API/MediaRecorder) aufgezeichnet wurde, kann jetzt erfolgreich abgespielt werden.
...
- Mehrere Firefox OS-APIs, die fälschlicherweise im Web verfügbar waren, wurden jetzt versteckt, wie es hätte sein sollen — `mozContact`, `MozContactChangeEvent`, `navigator.mozContacts`, `MozPowerManager`, `MozSettingsEvent` (siehe [Firefox-Bug 1043562](https://bugzil.la/1043562), [Firefox-Bug 1256414](https://bugzil.la/1256414) und [Firefox-Bug 1256046](https://bugzil.la/1256046)).
- Die Unterstützung für UTF-16 wurde vom [`TextEncoder`](/de/docs/Web/API/TextEncoder) entfernt ([Firefox-Bug 1257877](https://bugzil.la/1257877)).

...
- Der [Shape Path Editor](https://firefox-source-docs.mozilla.org/devtools-user/page_inspector/how_to/edit_css_shapes/index.html) ist nun standardmäßig für über {{cssxref("clip-path")}} erzeugte Formen aktiviert ([Firefox-Bug 1405339](https://bugzil.la/1405339)).
- Der [Netzwerkmonitor](https://firefox-source-docs.mozilla.org/devtools-user/network_monitor/index.html) verfügt nun über eine Schaltfläche, um die [Aufzeichnung des Netzwerkverkehrs zu pausieren/fortzusetzen](https://firefox-source-docs.mozilla.org/devtools-user/network_monitor/index.html#pausing-and-resume-network-traffic-recording) ([Firefox-Bug 1005755](https://bugzil.la/1005755)).
- Im [Netzwerkmonitor](https://firefox-source-docs.mozilla.org/devtools-user/network_monitor/index.html) ist der "Flash"-Filter-Button nicht mehr verfügbar, und Flash-Anforderungen sind im "Andere"-Filter enthalten ([Firefox-Bug 1413540](https://bugzil.la/1413540)).
- Der Code für den alten "Responsive Design Mode" (standardmäßig aktiviert vor Firefox 52) wurde nun aus den Devtools entfernt ([Firefox-Bug 1305777](https://bugzil.la/1305777)). Für Informationen über die neuen Werkzeuge siehe [Responsive Design Mode](https://firefox-source-docs.mozilla.org/devtools-user/responsive_design_mode/index.html).
...
- Die [`PerformanceNavigationTiming`](/de/docs/Web/API/PerformanceNavigationTiming) API wurde implementiert ([Firefox-Bug 1263722](https://bugzil.la/1263722)).
  ...
  - Gecko wurde auch eine Voreinstellung gegeben, die bei Bedarf zur Deaktivierung der Schnittstelle verwendet werden kann — `dom.enable_performance_navigation_timing`, standardmäßig auf `true` gesetzt ([Firefox-Bug 1403926](https://bugzil.la/1403926)).

#### DOM

- Fehler, die über Fehlerobjekte in bestimmten APIs gemeldet werden — zum Beispiel in der `error`-Eigenschaft von [`FileReader`](/de/docs/Web/API/FileReader), [`IDBRequest`](/de/docs/Web/API/IDBRequest) und [`IDBTransaction`](/de/docs/Web/API/IDBTransaction) und wenn bei bestimmten Methoden von [`RTCPeerConnection`](/de/docs/Web/API/RTCPeerConnection) gestellte Anfragen erfolglos sind — werden jetzt durch [`DOMException`](/de/docs/Web/API/DOMException) Instanzen dargestellt. [`DOMError`](/de/docs/Web/API/DOMError) ist jetzt veraltet, da es aus der DOM4-Spezifikation entfernt wurde ([Firefox-Bug 1120178](https://bugzil.la/1120178)).
...
- Das auf Budget basierende Hintergrundzeitlimit-Drosselsystem wurde implementiert — siehe [Richtlinien zur Unterstützung der Leistungsfähigkeit von Hintergrundseiten](/de/docs/Web/API/Page_Visibility_API#policies_in_place_to_aid_background_page_performance) für weitere Details ([Firefox-Bug 1377766](https://bugzil.la/1377766)).
...
- Die Verwendung von [`MediaStream.addTrack()`](/de/docs/Web/API/MediaStream/addTrack), um Tracks zu einem Strom hinzuzufügen, der mit [`getUserMedia()`](/de/docs/Web/API/MediaDevices/getUserMedia) erhalten wurde, und anschließend den resultierenden Stream aufzuzeichnen, funktioniert jetzt wie erwartet. Zuvor wurden nur die Tracks, die ursprünglich im Stream enthalten waren, der von `getUserMedia()` zurückgegeben wurde, im aufgezeichneten Medium berücksichtigt ([Firefox-Bug 1296531](https://bugzil.la/1296531)).
- Die [WebVTT](/de/docs/Web/API/WebVTT_API) [`VTTRegion`](/de/docs/Web/API/VTTRegion) Schnittstelle wurde immer beim Interpretieren von WebVTT-Dateien erstellt, aber die resultierenden Regionen wurden bisher nicht genutzt. Ab Firefox 58 werden sie genutzt, wenn Sie die Voreinstellung `media.webvtt.regions.enabled` aktivieren, indem Sie den Wert auf `true` setzen.
...
- Firefox implementiert jetzt einen TLS-Handshaketimer mit einem Standardwert von 30 Sekunden. Der Timeout-Wert kann durch Bearbeiten der Voreinstellung `network.http.tls-handshake-timeout` in about:config geändert werden ([Firefox-Bug 1393691](https://bugzil.la/1393691)).
...
- Der [425: Too Early](/de/docs/Web/HTTP/Status/425) Statuscode und das verwandte {{httpheader("Early-Data")}} Request-Header werden jetzt unterstützt ([Firefox-Bug 1406908](https://bugzil.la/1406908)).
...
- Sie können ein {{htmlelement("a")}}-Element nicht mehr innerhalb eines {{htmlelement("map")}}-Elements schachteln, um eine Hotspot-Region zu erstellen — ein {{htmlelement("area")}}-Element muss stattdessen verwendet werden ([Firefox-Bug 1317937](https://bugzil.la/1317937)).
...
- Die proprietäre Mozilla-Pseudoklasse `:-moz-styleeditor-transitioning` ist nicht mehr für Webinhalte verfügbar ([Firefox-Bug 1396099](https://bugzil.la/1396099)).
...
- Die `dom.abortController.enabled` und `dom.abortController.fetch.enabled` Voreinstellungen, die die Offenlegung der [Abort API-Funktionalität](/de/docs/Web/API/Fetch_API#aborting_a_fetch) steuerten, wurden jetzt entfernt, da diese Funktionen jetzt standardmäßig aktiviert sind ([Firefox-Bug 1402317](https://bugzil.la/1402317)).
- Die proprietäre `mozSrcObject` Eigenschaft wurde in Firefox 58 entfernt ([Firefox-Bug 1183495](https://bugzil.la/1183495)). Verwenden Sie stattdessen die standardmäßige [`HTMLMediaElement.srcObject`](/de/docs/Web/API/HTMLMediaElement/srcObject) Eigenschaft.
...
- privatsphäre
  ...
  - Die Erste-Partei-Isolierung kann jetzt über firstPartyIsolate umgeschaltet werden (Bug 1409045)
...
- tabs
  ...
  - Die openInReaderMode-Option von tabs.created wurde implementiert (Bug 1408993)
  - tabs.onUpdated benachrichtigt nun beim Ein-/Austreten aus dem Lesemodus (Bug 1402921)

- theme

  - Die Methode [getCurrent](/de/docs/Mozilla/Add-ons/WebExtensions/API/theme/getCurrent)() zur Abfrage der aktuellen Themenattribute (Bug [1349944](https://bugzil.la/1349944))
  - Die onUpdated Methode zur Aufnahme von WebExtension-Themenaktualisierungen (Bug [1349944](https://bugzil.la/1349944))

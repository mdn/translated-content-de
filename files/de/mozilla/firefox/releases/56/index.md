...
- Das neue CSS Grid Layout Panel ist jetzt verfügbar und ermöglicht eine wesentlich bessere Fehlerbehebung von CSS-Grid-Layouts ([Firefox-Bug 1181227](https://bugzil.la/1181227)). Weitere Details finden Sie unter [Leistungsstarke neue Ergänzungen zum CSS-Grid-Inspector in Firefox Nightly](https://hacks.mozilla.org/2017/06/new-css-grid-layout-panel-in-firefox-nightly/).

### HTML

- Die `labels`-Eigenschaft für beschriftbare Formularkontrollen wurde implementiert, zum Beispiel [`HTMLInputElement.labels`](/de/docs/Web/API/HTMLInputElement/labels) ([Firefox-Bug 556743](https://bugzil.la/556743)).
- Implementiert `<link rel="preload">`; siehe [Vorladen von Inhalten mit rel="preload"](/de/docs/Web/HTML/Attributes/rel/preload) für weitere Details ([Firefox-Bug 1222633](https://bugzil.la/1222633)). Beachten Sie, dass Firefox derzeit nur das Vorladen von zwischenspeicherbaren Ressourcen unterstützt.

### CSS

- Eigens Mozilla-spezifische {{cssxref("&lt;color&gt;")}}-Werte `-moz-win-accentcolor` und `-moz-win-accentcolortext` wurden implementiert (siehe [Firefox-Bug 1344910](https://bugzil.la/1344910)), ebenso die proprietäre Medienabfrage [`-moz-windows-accent-color-in-titlebar`](/de/docs/Web/CSS/CSS_media_queries/Using_media_queries#-moz-windows-accent-color-in-titlebar) (siehe [Firefox-Bug 1379938](https://bugzil.la/1379938)).
...
- Firefox akzeptierte stillschweigend `iso-2022-jp-2`-Sequenzen, wenn ein `iso-2022-jp`-[`TextDecoder()`](/de/docs/Web/API/TextDecoder/TextDecoder) instanziiert wurde, dies wurde jedoch entfernt, um die API zu vereinfachen, da keine anderen Browser dies unterstützen und keine Seiten es zu nutzen scheinen. ([Firefox-Bug 715833](https://bugzil.la/715833)).
- Das 4ms-Clamping-Verhalten von [`setTimeout()`](/de/docs/Web/API/SetTimeout) und [`setInterval()`](/de/docs/Web/API/SetInterval) wurde aktualisiert, um mehr im Einklang mit anderen Browsern zu sein, wie in [Timeouts auf >=4ms gedrosselt](/de/docs/Web/API/setTimeout#timeouts_throttled_to_%3e4ms) beschrieben ([Firefox-Bug 1378586](https://bugzil.la/1378586)).
...
- Die Implementierung der [`HTMLFormElement.action`](/de/docs/Web/API/HTMLFormElement/action), [`HTMLInputElement.formAction`](/de/docs/Web/API/HTMLInputElement/formAction) und [`HTMLButtonElement.formAction`](/de/docs/Web/API/HTMLButtonElement/formAction)-Eigenschaften wurde aktualisiert, damit sie die korrekte URL für die Formularübermittlung gemäß Spezifikation zurückgeben ([Firefox-Bug 1366361](https://bugzil.la/1366361)).

#### DOM-Ereignisse

- [`onwheel`](/de/docs/Web/API/HTMLElement/wheel_event) ist jetzt auf [`HTMLElement`](/de/docs/Web/API/HTMLElement) verfügbar — dies war vorher nicht der Fall ([Firefox-Bug 1370550](https://bugzil.la/1370550)).
...
- Gecko kodiert URLs jetzt intern als [Punycode](https://en.wikipedia.org/wiki/Punycode), um URL-Codierungsprobleme zu vermeiden (siehe [Firefox-Bug 945240](https://bugzil.la/945240), siehe auch die Diskussion in [Firefox-Bug 942074](https://bugzil.la/942074)).
- Firefox unter Windows und macOS kann nun im [headlosen Modus](/de/docs/Mozilla/Firefox/Headless_mode) mit dem `-headless`-Flag ausgeführt werden (siehe [Firefox-Bug 1355150](https://bugzil.la/1355150) und [Firefox-Bug 1355147](https://bugzil.la/1355147)).

## Entfernungen aus der Webplattform

### HTML

- Das `<isindex>`-Element wurde aus dem HTML-Parser und aus der Formularübertragung entfernt ([Firefox-Bug 1266495](https://bugzil.la/1266495)).
...
- [downloads.open()](/de/docs/Mozilla/Add-ons/WebExtensions/API/downloads/open) kann jetzt nur noch durch eine Benutzeraktion aufgerufen werden
...
- [history.onVisited](/de/docs/Mozilla/Add-ons/WebExtensions/API/history/onVisited) enthält nun den Seitentitel, falls dieser bekannt ist.
...
- [permissions](/de/docs/Mozilla/Add-ons/WebExtensions/manifest.json/permissions) unterstützt nun "unlimitedStorage"
- [privacy.services](/de/docs/Mozilla/Add-ons/WebExtensions/API/privacy/services) enthält nun passwordSavingEnabled

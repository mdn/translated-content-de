...
- [User-Agent-Stile in der Rules-Ansicht des Inspectors](https://firefox-source-docs.mozilla.org/devtools-user/page_inspector/index.html#rules-view)
...
- [Node-Dimensionen zur Infobar des Inspectors hinzugefügt](https://firefox-source-docs.mozilla.org/devtools-user/page_inspector/index.html#firefox-32-onwards)
- [Schaltfläche für Vollbildschirm-Screenshots hinzugefügt](https://firefox-source-docs.mozilla.org/devtools-user/tools_toolbox/index.html#extra-tools)
- HiDPI-Bilder zu den Tools hinzugefügt
- Nodes mit `display:none` werden im Inspector unterschiedlich angezeigt
...
- `position:sticky` standardmäßig in Release-Builds aktiviert (zuvor nur in Nightly und Aurora) ([Firefox-Bug 916315](https://bugzil.la/916315)).
- {{cssxref("box-decoration-break")}} implementiert und die nicht-standardmäßige `-moz-background-inline-policy` entfernt ([Firefox-Bug 613659](https://bugzil.la/613659)).
- {{cssxref("flex-grow")}} und {{cssxref("flex-shrink")}} können zwischen null und nicht-null Werten wechseln, z. B. 'flex-grow: 0.6' ([Firefox-Bug 996945](https://bugzil.la/996945)).

### HTML

- Experimentelle Implementierung der {{HTMLElement("img")}} [`srcset`](/de/docs/Web/HTML/Element/img#srcset) Eigenschaft, aktiviert über die Einstellung `dom.image.srcset.enable` auf `true` ([Firefox-Bug 870021](https://bugzil.la/870021)).
- [**id**](/de/docs/Web/HTML/Global_attributes/id) und [**class**](/de/docs/Web/HTML/Global_attributes/class) sind nun echte [globale Attribute](/de/docs/Web/HTML/Global_attributes) und gelten auch für XML-Elemente, unabhängig vom Namensraum ([Firefox-Bug 741295](https://bugzil.la/741295)).
...
- Die experimentelle Implementierung der [`Document.timeline`](/de/docs/Web/API/Document/timeline) Schnittstelle, in Verbindung mit der [Web Animation API](https://drafts.fxtf.org/web-animations/), wurde hinzugefügt ([Firefox-Bug 998246](https://bugzil.la/998246)). Sie wird über die Präferenz `layout.web-animations.api.enabled` gesteuert, die derzeit nur in Nightly und Aurora aktiviert ist.
- Die [Data Store API](/de/docs/Web/API/Data_Store_API) ist nun für [Web Workers](/de/docs/Web/API/Web_Workers_API/Using_web_workers) verfügbar gemacht worden ([Firefox-Bug 949325](https://bugzil.la/949325)). Sie ist weiterhin nur für zertifizierte Anwendungen aktiviert.
...
- Die [`Navigator.doNotTrack`](/de/docs/Web/API/Navigator/doNotTrack) Eigenschaften geben nun `'1'` oder `'0'` zurück, entsprechend dem HTTP-Wert, anstelle von `'yes'` oder `'no'` ([Firefox-Bug 887703](https://bugzil.la/887703)).
...
- Neue Einschränkungen für [WebRTC](/de/docs/Glossary/WebRTC)'s [`getUserMedia()`](/de/docs/Web/API/NavigatorUserMedia/getUserMedia), `width`, `height`, und `framerate` wurden hinzugefügt, um die Dimensionen des Streams und die Bildrate zu begrenzen ([Firefox-Bug 907352](https://bugzil.la/907352)):
...
- [Privilegierter Code erhält jetzt Einblick in JavaScript `Object` und `Array` Instanzen](https://firefox-source-docs.mozilla.org/dom/scriptSecurity/xray_vision.html#xray-semantics-for-object-and-array).

## Änderungen für Add-on- und Mozilla-Entwickler

Einblick erfolgt nun auch in JavaScript-Objekte, die keine DOM-Objekte sind: [Einblick in JavaScript-Objekte](https://firefox-source-docs.mozilla.org/dom/scriptSecurity/xray_vision.html#xrays-for-javascript-objects).

Eine `getDataDirectory()` Methode wurde zu [`Addon`](/de/docs/Mozilla/Add-ons/Add-on_Manager/Addon) Instanzen hinzugefügt. Diese Methode gibt den bevorzugten Speicherort im aktuellen Profil zurück, wo Add-ons Daten speichern können.
...
[GitHub Commits, die zwischen Firefox 31 und Firefox 32 gemacht wurden](https://github.com/mozilla/addon-sdk/compare/firefox31...firefox32). Diese enthalten keine Erhöhungen, die nach Eintritt dieses Releases in Aurora vorgenommen wurden.

[Behobene Bugs zwischen Firefox 31 und Firefox 32](https://bugzilla.mozilla.org/buglist.cgi?resolution=FIXED&chfieldto=2014-06-09&chfield=resolution&query_format=advanced&chfieldfrom=2014-04-28&chfieldvalue=FIXED&bug_status=RESOLVED&bug_status=VERIFIED&bug_status=CLOSED&product=Add-on%20SDK&list_id=10493962). Diese enthalten keine Erhöhungen, die nach Eintritt dieses Releases in Aurora vorgenommen wurden.

### XPCOM

- Die `nsIUDPSocket` Schnittstelle bietet jetzt Multicast-Unterstützung durch die Hinzufügung der neuen Eigenschaften `nsIUDPSocket.multicastLoopback`, `nsIUDPSocket.multicastInterface` und `nsIUDPSocket.multicastInterfaceAddr`, sowie die Methoden `nsIUDPSocket.joinMulticast()` und `nsIUDPSocket.leaveMulticast()`.

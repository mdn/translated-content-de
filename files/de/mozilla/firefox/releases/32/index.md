...
- [Knotenabmessungen zur Infoleiste des Inspektors hinzugefügt](https://firefox-source-docs.mozilla.org/devtools-user/page_inspector/index.html#firefox-32-onwards)
- [Schaltfläche für Vollbildschirm-Screenshot hinzugefügt](https://firefox-source-docs.mozilla.org/devtools-user/tools_toolbox/index.html#extra-tools)
...
- {{cssxref("flex-grow")}} und {{cssxref("flex-shrink")}} ermöglichen jetzt Übergänge zwischen null und nicht-null Werten, wie 'flex-grow: 0.6' ([Firefox-Bug 996945](https://bugzil.la/996945)).

### HTML

- Experimentell implementiert, hinter einem Pref, die {{HTMLElement("img")}} [`srcset`](/de/docs/Web/HTML/Element/img#srcset) Eigenschaft. Um sie zu aktivieren, setzen Sie `dom.image.srcset.enable` auf `true` ([Firefox-Bug 870021](https://bugzil.la/870021)).
...
- Die Methoden [`KeyboardEvent.getModifierState()`](/de/docs/Web/API/KeyboardEvent/getModifierState) und [`MouseEvent.getModifierState()`](/de/docs/Web/API/MouseEvent/getModifierState) wurden erweitert, um den `Accel` virtuellen Modifizierer zu unterstützen ([Firefox-Bug 1009388](https://bugzil.la/1009388)).
- Die [`KeyboardEvent.code`](/de/docs/Web/API/KeyboardEvent/code) Eigenschaft wurde experimentell implementiert: Sie ist in der Release-Build deaktiviert ([Firefox-Bug 865649](https://bugzil.la/865649)).
- Geltungsbereich-Selektoren für [`Document.querySelector()`](/de/docs/Web/API/Document/querySelector) und [`Document.querySelectorAll()`](/de/docs/Web/API/Document/querySelectorAll), zum Beispiel `querySelector(":scope > li")` wurden implementiert ([Firefox-Bug 528456](https://bugzil.la/528456)).
- Die experimentelle Implementierung der [`Document.timeline`](/de/docs/Web/API/Document/timeline) Schnittstelle, die zur [Web Animation API](https://drafts.fxtf.org/web-animations/) gehört, wurde hinzugefügt ([Firefox-Bug 998246](https://bugzil.la/998246)). Sie wird über das `layout.web-animations.api.enabled` Präferenz kontrolliert und ist momentan nur in Nightly und Aurora aktiviert.
- Die [Data Store API](/de/docs/Web/API/Data_Store_API) wurde für [Web Worker](/de/docs/Web/API/Web_Workers_API/Using_web_workers) verfügbar gemacht ([Firefox-Bug 949325](https://bugzil.la/949325)). Sie ist immer noch nur für zertifizierte Anwendungen aktiviert.
...
- Um die Spezifikation und die Entwicklung der CSS-Syntax zu unterstützen, wurden kleine Änderungen an [`CSS.escape()`](/de/docs/Web/API/CSS/escape_static) vorgenommen. Der Bezeichner kann jetzt mit `'--'` beginnen, und der zweite Bindestrich muss nicht mehr entkommen werden. Auch Herstellerbezeichner werden nicht mehr entkommen. ([Firefox-Bug 1008719](https://bugzil.la/1008719))
- Um unsere Hit-Regionen-Implementierung abzuschließen, wurde `MouseEvent.region` implementiert ([Firefox-Bug 979692](https://bugzil.la/979692)).
...
- [Privilegierter Code erhält jetzt Röntgenblick für JavaScript `Object` und `Array` Instanzen](https://firefox-source-docs.mozilla.org/dom/scriptSecurity/xray_vision.html#xray-semantics-for-object-and-array).

## Änderungen für Add-on- und Mozilla-Entwickler

Röntgenblick wird jetzt auf JavaScript-Objekte angewendet, die nicht selbst DOM-Objekte sind: [Röntgenblick für JavaScript-Objekte](https://firefox-source-docs.mozilla.org/dom/scriptSecurity/xray_vision.html#xrays-for-javascript-objects).
...
- Der [Add-on Debugger](/de/docs/Mozilla/Add-ons/Add-on_Debugger) umfasst nun eine Konsole und ein Scratchpad.

#### Details

[GitHub-Commits, die zwischen Firefox 31 und Firefox 32 gemacht wurden](https://github.com/mozilla/addon-sdk/compare/firefox31...firefox32). Dies wird keine Aufwertungen nach dem Eintritt dieser Version in Aurora beinhalten.

[Bugs, die zwischen Firefox 31 und Firefox 32 behoben wurden](https://bugzilla.mozilla.org/buglist.cgi?resolution=FIXED&chfieldto=2014-06-09&chfield=resolution&query_format=advanced&chfieldfrom=2014-04-28&chfieldvalue=FIXED&bug_status=RESOLVED&bug_status=VERIFIED&bug_status=CLOSED&product=Add-on%20SDK&list_id=10493962). Diese beinhalten keine Aufwertungen nach dem Eintritt dieser Version in Aurora.

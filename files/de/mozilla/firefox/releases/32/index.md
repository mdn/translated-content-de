...
Firefox 32 wurde am 2. September 2014 veröffentlicht. Dieser Artikel listet wichtige Änderungen auf, die nicht nur für Webentwickler nützlich sind, sondern auch für Firefox- und Gecko-Entwickler sowie für Add-on-Entwickler.
...
- [User-Agent-Stile in der Ansichtsregel des Inspektors](https://firefox-source-docs.mozilla.org/devtools-user/page_inspector/index.html#rules-view)
...
- [Vollbild-Screenshot-Button hinzugefügt](https://firefox-source-docs.mozilla.org/devtools-user/tools_toolbox/index.html#extra-tools)
...
- `position:sticky` ist standardmäßig in den Release-Builds aktiviert (zuvor nur in Nightly und Aurora) ([Firefox-Bug 916315](https://bugzil.la/916315)).
...
- Experimentell implementiert, hinter einer Voreinstellung, die {{HTMLElement("img")}} [`srcset`](/de/docs/Web/HTML/Reference/Elements/img#srcset) Eigenschaft, um sie zu aktivieren, setzen Sie `dom.image.srcset.enable` auf `true` ([Firefox-Bug 870021](https://bugzil.la/870021)).
...
- Scopierte Selektoren für [`Document.querySelector()`](/de/docs/Web/API/Document/querySelector) und [`Document.querySelectorAll()`](/de/docs/Web/API/Document/querySelectorAll), zum Beispiel `querySelector(":scope > li")`, wurden implementiert ([Firefox-Bug 528456](https://bugzil.la/528456)).
- Die experimentelle Implementierung der [`Document.timeline`](/de/docs/Web/API/Document/timeline)-Schnittstelle, die mit der [Web Animations API](/de/docs/Web/API/Web_Animations_API) verbunden ist, wurde hinzugefügt ([Firefox-Bug 998246](https://bugzil.la/998246)). Sie wird durch die Präferenz `layout.web-animations.api.enabled` gesteuert und ist momentan nur in Nightly und Aurora aktiviert.
- Die [Data Store API](https://web.archive.org/web/20210613234447/https://developer.mozilla.org/de/docs/Archive/B2G_OS/API/Data_Store_API) ist jetzt für [Web Workers](/de/docs/Web/API/Web_Workers_API/Using_web_workers) verfügbar ([Firefox-Bug 949325](https://bugzil.la/949325)). Sie ist immer noch nur für zertifizierte Anwendungen aktiviert.
...
- Die MSISDN-Überprüfungs-API, nur für privilegierte Apps aktiviert, wurde hinzugefügt ([Firefox-Bug 988469](https://bugzil.la/988469)).
...
- Um der Spezifikation und der Entwicklung der CSS-Syntax zu entsprechen, wurden kleinere Änderungen an [`CSS.escape()`](/de/docs/Web/API/CSS/escape_static) vorgenommen. Der Bezeichner kann jetzt mit `'--'` beginnen und der zweite Bindestrich muss nicht mehr maskiert werden. Auch Anbieterkennungen werden nicht mehr maskiert ([Firefox-Bug 1008719](https://bugzil.la/1008719)).
- Um unsere Implementierung von Treffregionen zu vervollständigen, wurde `MouseEvent.region` implementiert ([Firefox-Bug 979692](https://bugzil.la/979692)).
...
- Die [`Navigator.doNotTrack`](/de/docs/Web/API/Navigator/doNotTrack)-Eigenschaft gibt jetzt `'1'` oder `'0'` zurück, entsprechend dem HTTP-Wert, anstelle von `'yes'` oder `'no'` ([Firefox-Bug 887703](https://bugzil.la/887703)).
...
- Neue Einschränkungen für [`getUserMedia()`](/de/docs/Web/API/Navigator/getUserMedia) von {{Glossary("WebRTC", "WebRTC")}}, `width`, `height` und `framerate`, wurden hinzugefügt, um die Stream-Dimensionen und die Bildrate zu begrenzen ([Firefox-Bug 907352](https://bugzil.la/907352)):
...
- WebRTC-Methoden, die zuvor Rückruffunktionen als Eingabeparameter verwendeten, sind jetzt auch mit JavaScript-[Promisies](/de/docs/Web/JavaScript/Reference/Global_Objects/Promise) verfügbar.
...
- Der [Add-on Debugger](https://extensionworkshop.com/documentation/develop/debugging/) umfasst jetzt eine Konsole und ein Scratchpad.

#### Einzelheiten

[Zwischen Firefox 31 und Firefox 32 vorgenommene GitHub-Commits](https://github.com/mozilla/addon-sdk/compare/firefox31...firefox32). Dies schließt keine Erhöhungen ein, die nach dem Eintritt dieser Version in Aurora vorgenommen wurden.

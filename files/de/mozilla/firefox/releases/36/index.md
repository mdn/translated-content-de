...
Firefox 36 wurde am 24. Februar 2015 veröffentlicht. Dieser Artikel listet wichtige Änderungen auf, die nicht nur für Web-Entwickler nützlich sind, sondern auch für Firefox- und Gecko-Entwickler sowie Add-on-Entwickler.
...
- [Überprüfen Sie DOM-Versprechen in der Konsole](https://firefox-source-docs.mozilla.org/devtools-user/web_console/index.html#type-specific-rich-output)
...
- Der {{cssxref("@font-face/unicode-range", "unicode-range")}}-Deskriptor wird jetzt von {{cssxref("@font-face")}} unterstützt ([Firefox-Bug 475891](https://bugzil.la/475891)), aber nicht standardmäßig aktiviert.
- Die Eigenschaften {{cssxref("text-decoration-color")}}, {{cssxref("text-decoration-line")}} und {{cssxref("text-decoration-style")}} sind jetzt ohne Präfix verwendbar ([Firefox-Bug 825004](https://bugzil.la/825004)). Die mit Präfix versehene Version ist weiterhin verfügbar, um den Übergang zu erleichtern ([Firefox-Bug 1097922](https://bugzil.la/1097922)).
...
- Im [Quirks-Modus](/de/docs/Web/HTML/Quirks_Mode_and_Standards_Mode) wurde die [`:active` und `:hover` Quiver-Ausnahme](/de/docs/Mozilla_Quirks_Mode_Behavior#Miscellaneous_.26_Style) so geändert, dass sie seltener angewendet wird: Sie wird jetzt nur bei Links verwendet, nur wenn sich keine Pseudo-Elemente oder andere Pseudo-Klassen im Element befinden und wenn es nicht Teil eines Pseudo-Klassen-Elements ist ([Firefox-Bug 783213](https://bugzil.la/783213)).
- Die {{cssxref("isolation")}}-Eigenschaft wurde implementiert ([Firefox-Bug 1077872](https://bugzil.la/1077872)).
- CSS {{cssxref("&lt;gradient&gt;")}} gilt nun für die vorab multiplizierten Farben, was dem Standard und anderen Browsern entspricht und unerwartete Grautöne beseitigt ([Firefox-Bug 591600](https://bugzil.la/591600)).
...
- In Firefox werden {{HTMLElement("input")}}-Filter, die im [`accept`](/de/docs/Web/HTML/Element/input#accept)-Attribut angegeben sind, standardmäßig ausgewählt, es sei denn, es gibt einen unbekannten Wert, z. B. einen unbekannten MIME-Typ oder einen fehlerhaft formatierten Wert im [`accept`](/de/docs/Web/HTML/Element/input#accept)-Attribut. Früher wurden nur für `image/*`, `video/*` und `audio/*` Werte spezifizierte Filter standardmäßig ausgewählt ([Firefox-Bug 826185](https://bugzil.la/826185)).

### JavaScript

- Der [ECMAScript 2015](/de/docs/Web/JavaScript/New_in_JavaScript/ECMAScript_6_support_in_Mozilla)-Symbol-Datentyp ist jetzt standardmäßig aktiviert (war seit Version 33 im Nightly-Kanal verfügbar) ([Firefox-Bug 1066322](https://bugzil.la/1066322)):
...
- Der alte Platzhalter-String `"@@iterator"` wurde durch das echte ES2015 wohlbekannte Symbol {{jsxref("Symbol.iterator")}} für den [iterable](/de/docs/Web/JavaScript/Reference/Iteration_protocols)-Schnittstellenschlüssel ersetzt ([Firefox-Bug 918828](https://bugzil.la/918828)).
...
- Die [`const`](/de/docs/Web/JavaScript/Reference/Statements/const)-Deklaration ist jetzt blockbezogen und erfordert einen Initialisierer ([Firefox-Bug 611388](https://bugzil.la/611388)). Sie kann auch nicht mehr neu deklariert werden ([Firefox-Bug 1095439](https://bugzil.la/1095439)).
  ...
  - `{const a=1}; a;` wirft jetzt einen {{jsxref("ReferenceError")}} und gibt nicht mehr `1` zurück aufgrund der Blockbindung.
  - `const a;` wirft jetzt einen {{jsxref("SyntaxError")}} ("missing = in const declaration`"`): Ein Initialisierer ist erforderlich.
  - `const a = 1; a = 2;` wirft nun auch einen {{jsxref("SyntaxError")}} ("invalid assignment to const a").
...
- Der [`delete`](/de/docs/Web/JavaScript/Reference/Operators/delete)-Operator löst jetzt die "[temporale tödliche Zone](/de/docs/Web/JavaScript/Reference/Statements/let#temporal_dead_zone_tdz)" aus, wenn er mit [`let`](/de/docs/Web/JavaScript/Reference/Statements/let) und [`const`](/de/docs/Web/JavaScript/Reference/Statements/const) verwendet wird ([Firefox-Bug 1074571](https://bugzil.la/1074571)).
- Die nicht standardmäßigen [`let`-Blöcke und `let`-Ausdrücke](/de/docs/Web/JavaScript/Reference/Deprecated_and_obsolete_features#statements_2) sind veraltet und werden jetzt mit einer Warnung in der Konsole protokolliert. Verwenden Sie sie nicht mehr, sie werden in Zukunft entfernt.
- Der [WeakMap](/de/docs/Web/JavaScript/Reference/Global_Objects/WeakMap)-Konstruktor behandelt jetzt ein optionales iterable Argument ([Firefox-Bug 1092537](https://bugzil.la/1092537)).
...
- Die EME-bezogene Methode [`Navigator.requestMediaKeySystemAccess()`](/de/docs/Web/API/Navigator/requestMediaKeySystemAccess) und die zugehörige [`MediaKeySystemAccess`](/de/docs/Web/API/MediaKeySystemAccess) werden jetzt unterstützt ([Firefox-Bug 1095257](https://bugzil.la/1095257)).
...
- Die Standardwerte der Optionen für [`MutationObserver.observe()`](/de/docs/Web/API/MutationObserver/observe) wurden aktualisiert, um dem neuesten Standard zu entsprechen ([Firefox-Bug 973638](https://bugzil.la/973638)).
- Experimentelle Unterstützung für virtuelle Realität hat hinter der `dom.vr.enabled`-Einstellung Einzug gehalten, die standardmäßig deaktiviert ist ([Firefox-Bug 1036604](https://bugzil.la/1036604)).
- Die Funktion, die mit [`RTCPeerConnection.onsignalingstatechange`](/de/docs/Web/API/RTCPeerConnection/signalingstatechange_event) verknüpft ist, erhält jetzt ein Ereignis als Parameter, wie im Standard beschrieben ([Firefox-Bug 1075133](https://bugzil.la/1075133)).
...
- Die Methode `nsIWebBrowserPersist.saveURI()` erfordert jetzt 8 Argumente, in einer Reihenfolge, die mit früheren Veröffentlichungen nicht kompatibel ist.
- Unterstützung für Media Source Extensions (MSE) ist in Nicht-Build-Veröffentlichungen (Nightly und Developer Edition) standardmäßig aktiviert ([Firefox-Bug 1000686](https://bugzil.la/1000686)). Auf Beta- und Release-Version bleibt sie standardmäßig deaktiviert.
...
- RC4 wird jetzt als unsicher betrachtet und alle UI-Indikatoren reagieren entsprechend; SSLv3 wurde standardmäßig in Firefox 34 deaktiviert, aber die UI wurde geändert, um dem Benutzer ein besseres Verständnis darüber zu geben, was passiert ([Firefox-Bug 1093595](https://bugzil.la/1093595)).
...
- In den Firefox-Einstellungen ist das [Do not track](/de/docs/Web/HTTP/Headers/DNT)-Auswahl-Widget wieder ein Ein-/Ausschalter ([Firefox-Bug 1071747](https://bugzil.la/1071747)).
...
- Globale `require`-Funktion implementiert, um SDK-Module überall zugänglich zu machen ([Firefox-Bug 1070927](https://bugzil.la/1070927)), durch Verwendung von:
...
Neue Konstanten wurden zu `nsIContentPolicy` hinzugefügt, damit Gecko-Interna und Add-on-Code besser zwischen verschiedenen Arten von Anfragen unterscheiden können. Diese sind:

- `TYPE_FETCH`
  - : Zeigt eine Inhaltsanfrage an, die durch die [`fetch()`](/de/docs/Web/API/Window/fetch)-Methode initiiert wurde.
- `TYPE_IMAGESET`
  - : Zeigt eine Anfrage an, ein {{HTMLElement("img")}} (mit dem [`srcset`](/de/docs/Web/HTML/Element/img#srcset)-Attribut oder einem {{HTMLElement("picture")}}-Element zu laden.

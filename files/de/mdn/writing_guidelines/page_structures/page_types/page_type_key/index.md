...
Dies ermöglicht es den MDN-Inhaltstools, die Inhaltsüberprüfung und die Organisation der Seitenleiste besser zu automatisieren.
...
Jeder Hauptbereich der Seite – JavaScript, CSS, und so weiter – hat eine Reihe von domänenspezifischen `page-type` Werten und es gibt auch eine Reihe generischer Werte, die in jedem Bereich der Seite erscheinen können.
...
- `landing-page`: eine Übersicht über das Thema, die Einleitung des Abschnitts und die Navigation zu den wichtigsten Bereichen.
...
- `learn-faq`: die Antwort auf eine spezifische Frage zur Webentwicklung, wie zum Beispiel [_Was ist ein Domainname?_](/de/docs/Learn_web_development/Howto/Web_mechanics/What_is_a_domain_name).

### Zugänglichkeit (Accessibility) Seitentypen
...
- `css-media-feature`: eine [Medieneigenschaft](/de/docs/Web/CSS/Reference/At-rules/@media#media_features), wie {{cssxref("@media/hover")}}.
...
- `css-selector`: ein [Grundselektor](/de/docs/Web/CSS/Guides/Selectors/Selectors_and_combinators#basic_selectors), wie der [Klassenselektor](/de/docs/Web/CSS/Reference/Selectors/Class_selectors).
- `css-shorthand-property`: eine [Kurzeigenschaft](/de/docs/Web/CSS/Guides/Cascade/Shorthand_properties), wie {{cssxref("background")}}.
- `css-type`: ein [Datentyp](/de/docs/Web/CSS/Reference/Values/Data_types), wie {{cssxref("&lt;color&gt;")}}.

### Glossar Seitentypen

Dieser Abschnitt listet `page-type` Werte für Seiten unter [Glossary](/de/docs/Glossary) auf. Jede Seite in diesem Teil des Baums muss einen `page-type` haben, und sein Wert muss einer der unten aufgeführten Werte sein.
...
- `http-method`: ein [HTTP-Anforderungsmethode](/de/docs/Web/HTTP/Reference/Methods) wie [`GET`](/de/docs/Web/HTTP/Reference/Methods/GET).
...
- `webassembly-function`: eine globale Funktion, d.h. eine Methode direkt unter dem `WebAssembly` Objekt, das als Namensraum fungiert, wie [`WebAssembly.instantiate()`](/de/docs/WebAssembly/Reference/JavaScript_interface/instantiate_static).

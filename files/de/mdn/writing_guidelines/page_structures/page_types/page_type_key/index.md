...
Der Frontmatter-Schlüssel `page-type` beschreibt den Typ einer MDN-Seite. Dies ermöglicht es MDN-Content-Tools, die automatische Inhaltsprüfung und die Organisation der Sidebar besser zu automatisieren.

Wie bei jedem anderen Frontmatter-Schlüssel wird der `page-type`-Schlüssel im YAML am Anfang von "index.md" angegeben:
...
Jeder Hauptbereich der Seite — JavaScript, CSS usw. — hat eine Reihe von bereichsspezifischen `page-type`-Werten, und es gibt auch eine Reihe von generischen Werten, die in jedem Bereich der Seite vorkommen können.
...
- `landing-page`: eine Übersicht des Themas, Einführung in den Abschnitt und Navigation zu Schlüsselbereichen.
...
Dieser Abschnitt listet Seitentypen auf, die für einen einzelnen Bereich von MDN spezifisch sind.

### Seitentypen für den Lernbereich

Dieser Abschnitt listet `page-type`-Werte für Seiten unter [Learn](/de/docs/Learn_web_development) auf. Jede Seite in diesem Teil des Baumes muss einen `page-type` haben, und sein Wert muss einer der unten aufgeführten oder einer der generischen Seitentyp-Werte sein.
...
- `learn-module`: eine Übersicht über ein Modul, d. h. eine geordnete Sammlung von Leitfäden, wie [_Strukturieren von Inhalten mit HTML_](/de/docs/Learn_web_development/Core/Structuring_content).
...
- `learn-module-assessment`: ein spezieller Leitfaden mit einer Aktivität, um das Verständnis eines Moduls oder eines Teils davon zu bewerten, wie [_Testen Sie Ihre Fähigkeiten: grundlegende Steuerelemente_](/de/docs/Learn_web_development/Extensions/Forms/Test_your_skills:_Basic_controls).
...
Dieser Abschnitt listet `page-type`-Werte für Seiten unter [Web/Accessibility](/de/docs/Web/Accessibility) auf. Jede Seite in diesem Teil des Baumes muss einen `page-type` haben, und sein Wert muss einer der unten aufgeführten oder einer der [generischen Seitentypen](#generische_seitentypen) sein.
...
- `css-at-rule-descriptor`: ein at-rule-Beschreibungsmerkmal, wie [`@counter-style/prefix`](/de/docs/Web/CSS/@counter-style/prefix).
...
- `css-shorthand-property`: eine [kurze Eigenschaft](/de/docs/Web/CSS/CSS_cascade/Shorthand_properties), wie {{cssxref("background")}}.
...
- `glossary-disambiguation`: eine Seite mit Links zu zwei oder mehr Definitionsseiten für einen mehrdeutigen Begriff, wie {{Glossary("Node", "Knoten")}}.
...
- `javascript-instance-accessor-property`: eine Zugriffsrichtlinie auf einer Objektinstanz, wie [`Map.prototype.size`](/de/docs/Web/JavaScript/Reference/Global_Objects/Map/size).
- `javascript-instance-data-property`: eine Datenrichtlinie auf einer Objektinstanz, wie die [`length`](/de/docs/Web/JavaScript/Reference/Global_Objects/Array/length) Eigenschaft von `Array`.
...
- `javascript-static-accessor-property`: eine statische Zugriffsrichtlinie, wie [`RegExp.lastMatch`](/de/docs/Web/JavaScript/Reference/Global_Objects/RegExp/lastMatch).
- `javascript-static-data-property`: eine statische Datenrichtlinie, wie [`Math.E`](/de/docs/Web/JavaScript/Reference/Global_Objects/Math/E).
...
- `webassembly-function`: eine globale Funktion, also eine Methode direkt unter dem `WebAssembly` Objekt, das als Namespace fungiert, wie [`WebAssembly.instantiate()`](/de/docs/WebAssembly/Reference/JavaScript_interface/instantiate_static).
...
- `webassembly-instruction`: eine Anweisung oder eine Gruppe von Anweisungen, wie [`Wrap`](/de/docs/WebAssembly/Reference/Numeric/Wrap).
...
Dieser Abschnitt listet `page-type`-Werte für Seiten unter [Web/Manifest](/de/docs/Web/Progressive_web_apps/Manifest) auf. Jede Seite in diesem Teil des Baumes muss einen `page-type` haben, und sein Wert muss einer der unten aufgeführten oder einer der [generischen Seitentypen](#generische_seitentypen) sein.
...
Dieser Abschnitt listet `page-type`-Werte für Seiten unter [Web/XPath](/de/docs/Web/XML/XPath) auf. Jede Seite in diesem Teil des Baumes muss einen `page-type` haben, und sein Wert muss einer der unten aufgeführten oder einer der [generischen Seitentypen](#generische_seitentypen) sein.

- `xpath-function`: eine Funktion, wie [`ceiling()`](/de/docs/Web/XML/XPath/Reference/Functions/ceiling)

### XSLT-Seitentypen

Dieser Abschnitt listet `page-type`-Werte für Seiten unter [Web/XSLT](/de/docs/Web/XML/XSLT) auf. Jede Seite in diesem Teil des Baumes muss einen `page-type` haben, und sein Wert muss einer der unten aufgeführten oder einer der [generischen Seitentypen](#generische_seitentypen) sein.

- `xslt-element`: ein Element von XSLT, wie [`<xsl:message>`](/de/docs/Web/XML/XSLT/Reference/Element/message).

### EXSLT-Seitentypen

Dieser Abschnitt listet `page-type`-Werte für Seiten unter [Web/EXSLT](/de/docs/Web/XML/EXSLT) auf. Jede Seite in diesem Teil des Baumes muss einen `page-type` haben, und sein Wert muss einer der unten aufgeführten oder einer der [generischen Seitentypen](#generische_seitentypen) sein.

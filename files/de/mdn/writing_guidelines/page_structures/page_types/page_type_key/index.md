...
Der Front-Matter-Schlüssel `page-type` beschreibt den Typ einer MDN-Seite. Dies ermöglicht MDN-Inhaltstools eine bessere Automatisierung der Inhaltsprüfung und der Organisation der Seitenleiste.
...
Jeder Hauptbereich der Seite — JavaScript, CSS und so weiter — hat eine Reihe von domänenspezifischen `page-type`-Werten, und es gibt auch eine Reihe von generischen Werten, die in jedem Bereich der Seite erscheinen können.
...
- `landing-page`: eine Seite, die hauptsächlich als Navigation dient und Links zu anderen Seiten auflistet.
- `how-to`: eine Seite, die hauptsächlich als zielorientierter Anleitungsartikel dient.
...
Dieser Abschnitt listet `page-type`-Werte für Seiten unter [Learn](/de/docs/Learn) auf. Jede Seite in diesem Teil des Baums muss einen `page-type` haben, und dessen Wert muss einer der unten aufgeführten oder einer der generischen Seitentypwerte sein.

- `learn-topic`: eine Übersicht über ein Thema, das heißt eine Sammlung von Modulen wie [_CSS_](/de/docs/Learn/CSS).
- `learn-module`: eine Übersicht über ein Modul, das heißt eine geordnete Sammlung von Leitfäden, wie [_Introduction to HTML_](/de/docs/Learn/HTML/Introduction_to_HTML).
...
- `learn-module-assessment`: ein spezieller Leitfaden mit einer Aktivität, die es ermöglicht, das Verständnis für ein Modul oder einen Teil davon einzuschätzen, wie [_Test your skills: basic controls_](/de/docs/Learn/Forms/Test_your_skills:_Basic_controls).
- `learn-faq`: die Antwort auf eine spezifische Frage zur Webentwicklung, wie [_What is a domain name?_](/de/docs/Learn/Common_questions/Web_mechanics/What_is_a_domain_name).
...
- `css-media-feature`: eine [Medienfeature](/de/docs/Web/CSS/@media#media_features), wie [`hover`](/de/docs/Web/CSS/@media/hover).
...
- `http-method`: eine [HTTP-Request-Methode](/de/docs/Web/HTTP/Methods), wie [`GET`](/de/docs/Web/HTTP/Methods/GET).
...
- `javascript-instance-data-property`: eine Dateieigenschaft auf einer Objektinstanz, wie die [`length`](/de/docs/Web/JavaScript/Reference/Global_Objects/Array/length)-Eigenschaft von `Array`.
...
- `javascript-static-data-property`: eine statische Dateieigenschaft, wie [`Math.E`](/de/docs/Web/JavaScript/Reference/Global_Objects/Math/E).
...
Dieser Abschnitt listet `page-type`-Werte für Seiten unter [Web/Manifest](/de/docs/Web/Manifest) auf. Jede Seite in diesem Teil des Baums muss einen `page-type` haben, und dessen Wert muss einer der unten aufgeführten oder einer der [generischen Seitentypen](#generische_seitentypen) sein.

- `web-manifest-member`: ein Mitglied eines Manifests, wie [`description`](/de/docs/Web/Manifest/description).

### XPath Seitentypen

Dieser Abschnitt listet `page-type`-Werte für Seiten unter [Web/XPath](/de/docs/Web/XPath) auf. Jede Seite in diesem Teil des Baums muss einen `page-type` haben, und dessen Wert muss einer der unten aufgeführten oder einer der [generischen Seitentypen](#generische_seitentypen) sein.

- `xpath-function`: eine Funktion, wie [`ceiling()`](/de/docs/Web/XPath/Functions/ceiling).

### XSLT Seitentypen

Dieser Abschnitt listet `page-type`-Werte für Seiten unter [Web/XSLT](/de/docs/Web/XSLT) auf. Jede Seite in diesem Teil des Baums muss einen `page-type` haben, und dessen Wert muss einer der unten aufgeführten oder einer der [generischen Seitentypen](#generische_seitentypen) sein.
...
Dieser Abschnitt listet `page-type`-Werte für Seiten unter [Web/EXSLT](/de/docs/Web/EXSLT) auf. Jede Seite in diesem Teil des Baums muss einen `page-type` haben, und dessen Wert muss einer der unten aufgeführten oder einer der [generischen Seitentypen](#generische_seitentypen) sein.

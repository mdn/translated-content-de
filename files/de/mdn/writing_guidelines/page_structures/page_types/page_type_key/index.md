---
title: Der `page-type` Frontmatter-Key
slug: MDN/Writing_guidelines/Page_structures/Page_types/Page_type_key
l10n:
  sourceCommit: 719645a32546d9e514ac530a5eb66aa4c26d4f51
---

Der `page-type` Frontmatter-Key beschreibt den Typ einer MDN-Seite. Dies ermöglicht es MDN-Content-Tools, die Inhaltsüberprüfung und die Organisation der Seitenleiste besser zu automatisieren.

Wie jeder andere Frontmatter-Key wird der `page-type` Key im YAML am Anfang von "index.md" angegeben:
...
Jeder Hauptbereich der Website — JavaScript, CSS und so weiter — hat eine Reihe von bereichsspezifischen `page-type` Werten. Es gibt auch eine Reihe von generischen Werten, die in jedem Bereich der Website erscheinen können.
...
- `listing-page`: eine kurze Beschreibung des Abschnitts und eine Liste von Unterseiten in diesem Abschnitt.
...
Dieser Abschnitt listet die Seitentypen auf, die spezifisch für einen bestimmten Bereich von MDN sind.
...
- `learn-topic`: eine Übersicht über ein Thema, das ist, eine Sammlung von Modulen wie [_CSS_](/de/docs/Learn_web_development/Core/Styling_basics).
- `learn-module`: eine Übersicht über ein Modul, das ist, eine geordnete Sammlung von Leitfäden, wie [_Inhalt mit HTML strukturieren_](/de/docs/Learn_web_development/Core/Structuring_content).
...
- `learn-module-assessment`: ein spezieller Leitfaden mit einer Aktivität, die es erlaubt, das Verständnis eines Moduls oder eines Teils davon zu bewerten, wie [_Testen Sie Ihre Fähigkeiten: grundlegende Steuerelemente_](/de/docs/Learn_web_development/Extensions/Forms/Test_your_skills:_Basic_controls).
...
- `css-combinator`: ein Kombinator, wie der [Nachfolger-Kombinator](/de/docs/Web/CSS/Descendant_combinator).
...
Dieser Abschnitt listet `page-type` Werte für Seiten unter [Web/Manifest](/de/docs/Web/Manifest) auf. Jede Seite in diesem Teil des Baums muss einen `page-type` haben, und sein Wert muss einer der unten aufgeführten oder einer der [generischen Seitentypen](#generische_seitentypen) sein.

- `web-manifest-member`: ein Mitglied eines Manifests, wie [`description`](/de/docs/Web/Manifest/Reference/description).

### XPath Seitentypen

Dieser Abschnitt listet `page-type` Werte für Seiten unter [Web/XPath](/de/docs/Web/XPath) auf. Jede Seite in diesem Teil des Baums muss einen `page-type` haben, und sein Wert muss einer der unten aufgeführten oder einer der [generischen Seitentypen](#generische_seitentypen) sein.

- `xpath-function`: eine Funktion, wie [`ceiling()`](/de/docs/Web/XPath/Functions/ceiling)

### XSLT Seitentypen

Dieser Abschnitt listet `page-type` Werte für Seiten unter [Web/XSLT](/de/docs/Web/XSLT) auf. Jede Seite in diesem Teil des Baums muss einen `page-type` haben, und sein Wert muss einer der unten aufgeführten oder einer der [generischen Seitentypen](#generische_seitentypen) sein.

- `xslt-element`: ein Element von XSLT, wie [`<xsl:message>`](/de/docs/Web/XSLT/Reference/Element/message).

### EXSLT Seitentypen

Dieser Abschnitt listet `page-type` Werte für Seiten unter [Web/EXSLT](/de/docs/Web/EXSLT) auf. Jede Seite in diesem Teil des Baums muss einen `page-type` haben, und sein Wert muss einer der unten aufgeführten oder einer der [generischen Seitentypen](#generische_seitentypen) sein.

...
Jeder Hauptbereich der Seite — JavaScript, CSS usw. — hat eine Reihe von bereichsspezifischen `page-type` Werten, und es gibt auch eine Reihe von generischen Werten, die in jedem Bereich der Seite erscheinen können.
...
- `landing-page`: ein Überblick über das Thema, eine Abschnittseinführung und die Navigation zu Schlüsselbereichen.
...
Dieser Abschnitt listet `page-type` Werte für Seiten unter [Learn](/de/docs/Learn_web_development) auf. Jede Seite in diesem Baumabschnitt muss einen `page-type` haben, und dessen Wert muss einer der unten aufgeführten oder einer der generischen Seitentypen sein.
...
- `learn-module` ein Überblick über ein Modul, also eine geordnete Sammlung von Leitfäden, wie [_Strukturieren von Inhalten mit HTML_](/de/docs/Learn_web_development/Core/Structuring_content).
...
Dieser Abschnitt listet `page-type` Werte für Seiten unter [Web/Accessibility](/de/docs/Web/Accessibility) auf. Jede Seite in diesem Teil des Baums muss einen `page-type` haben, und sein Wert muss einer der unten aufgeführten sein oder einer der [generischen Seitentypen](#generische_seitentypen).
...
Dieser Abschnitt listet `page-type` Werte für Seiten unter [Web/CSS](/de/docs/Web/CSS) auf. Jede Seite in diesem Abschnitt des Baums muss einen `page-type` haben, und sein Wert muss einer der unten aufgeführten sein oder einer der [generischen Seitentypen](#generische_seitentypen).

- `css-at-rule`: eine [at-rule](/de/docs/Web/CSS/Guides/Syntax/At-rules), wie {{cssxref("@media")}}.
- `css-at-rule-descriptor`: ein Attributsbeschreiber, wie [`@counter-style/prefix`](/de/docs/Web/CSS/Reference/At-rules/@counter-style/prefix).
- `css-combinator`: ein Kombinator, wie der [Nachkommen Kombinator](/de/docs/Web/CSS/Reference/Selectors/Descendant_combinator).
...
- `css-media-feature`: ein [Media-Feature](/de/docs/Web/CSS/Reference/At-rules/@media#media_features), wie [`hover`](/de/docs/Web/CSS/Reference/At-rules/@media/hover).
...
### Glossary-Seitentypen

Dieser Abschnitt listet `page-type` Werte für Seiten unter [Glossary](/de/docs/Glossary) auf. Jede Seite in diesem Teil des Baums muss einen `page-type` haben, und deren Wert muss einer der unten aufgeführten sein.
...
Dieser Abschnitt listet `page-type` Werte für Seiten unter [Web/HTML](/de/docs/Web/HTML) auf. Jede Seite in diesem Teil des Baums muss einen `page-type` haben, und dessen Wert muss einer der unten aufgeführten sein oder einer der [generischen Seitentypen](#generische_seitentypen).
...
Dieser Abschnitt listet `page-type` Werte für Seiten unter [Web/HTTP](/de/docs/Web/HTTP) auf. Jede Seite in diesem Teil des Baums muss einen `page-type` haben, und deren Wert muss einer der unten aufgeführten sein oder einer der [generischen Seitentypen](#generische_seitentypen).
...
- `javascript-namespace`: ein Objekt, das nicht instantiierbar ist und nur statische Mitglieder hat, wie [`Math`](/de/docs/Web/JavaScript/Reference/Global_Objects/Math).
...
Dieser Abschnitt listet `page-type` Werte für Seiten unter [Web/SVG](/de/docs/Web/SVG) auf. Jede Seite in diesem Abschnitt des Baums muss einen `page-type` haben, und dessen Wert muss einer der unten aufgeführten sein oder einer der [generischen Seitentypen](#generische_seitentypen).
...
Dieser Abschnitt listet `page-type` Werte für Seiten unter [Web/API](/de/docs/Web/API) auf. Jede Seite in diesem Teil des Baums muss einen `page-type` haben, und dessen Wert muss einer der unten aufgeführten sein oder einer der [generischen Seitentypen](#generische_seitentypen).
...
- `webgl-extension-method`: eine Methode einer WebGL-Erweiterung, wie [`OES_vertex_array_object.bindVertexArrayOES()`](/de/docs/Web/API/OES_vertex_array_object/bindVertexArrayOES).
...
Dieser Abschnitt listet `page-type` Werte für Seiten unter [Web/WebDriver](/de/docs/Web/WebDriver) auf. Jede Seite in diesem Teil des Baums muss einen `page-type` haben, und dessen Wert muss einer der unten aufgeführten sein oder einer der [generischen Seitentypen](#generische_seitentypen).
...
Dieser Abschnitt listet `page-type` Werte für Seiten unter [Mozilla/Add-ons/WebExtensions](/de/docs/Mozilla/Add-ons/WebExtensions) auf. Jede Seite in diesem Teil des Baums muss einen `page-type` haben und dessen Wert muss einer der unten aufgeführten sein oder einer der [generischen Seitentypen](#generische_seitentypen).
...
Dieser Abschnitt listet `page-type` Werte für Seiten unter [Web/Manifest](/de/docs/Web/Progressive_web_apps/Manifest) auf. Jede Seite in diesem Abschnitt des Baums muss einen `page-type` haben, und dessen Wert muss einer der unten aufgeführten sein oder einer der [generischen Seitentypen](#generische_seitentypen).
...
Dieser Abschnitt listet `page-type` Werte für Seiten unter [Mozilla/Firefox](/de/docs/Mozilla/Firefox) auf. Jede Seite in diesem Abschnitt des Baums muss einen `page-type` haben, und dessen Wert muss einer der unten aufgeführten oder einer der [generischen Seitentypen](#generische_seitentypen) sein.

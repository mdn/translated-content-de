...
Der `page-type` Front-Matter-Schlüssel beschreibt den Typ einer MDN-Seite. Dies ermöglicht es den MDN-Inhaltswerkzeugen, die Inhaltsprüfung und die Organisation der Seitenleiste besser zu automatisieren.
...
Jeder Hauptbereich der Seite — JavaScript, CSS usw. — hat einen Satz domänenspezifischer `page-type`-Werte, und es gibt auch einen Satz generischer Werte, die in jedem Bereich der Seite erscheinen können.
...
- `landing-page`: eine Übersicht über das Thema, eine Einführung in den Abschnitt und Navigation zu wichtigen Bereichen.
...
Dieser Abschnitt listet `page-type` Werte für Seiten unter [Learn](/de/docs/Learn_web_development) auf. Jede Seite in diesem Teil der Struktur muss einen `page-type` haben, und der Wert muss einer der unten aufgeführten oder einer der generischen Seitentypwerte sein.
...
- `learn-module-assessment` ein spezieller Leitfaden mit einer Aktivität zur Bewertung des Verständnisses eines Moduls oder eines Teils davon, wie [_Testen Sie Ihre Fähigkeiten: Formulare und Schaltflächen_](/de/docs/Learn_web_development/Core/Structuring_content/Test_your_skills/Forms_and_buttons).
- `learn-faq`: die Antwort auf eine spezifische Frage zur Web-Entwicklung, wie [_Was ist ein Domain-Name?_](/de/docs/Learn_web_development/Howto/Web_mechanics/What_is_a_domain_name).

### Seitentypen für Zugänglichkeit

Dieser Abschnitt listet `page-type` Werte für Seiten unter [Web/Accessibility](/de/docs/Web/Accessibility) auf. Jede Seite in diesem Teil der Struktur muss einen `page-type` haben, und der Wert muss einer der unten aufgeführten oder einer der [generischen Seitentypen](#generische_seitentypen) sein.
...
Dieser Abschnitt listet `page-type` Werte für Seiten unter [Web/CSS](/de/docs/Web/CSS) auf. Jede Seite in diesem Teil der Struktur muss einen `page-type` haben, und der Wert muss einer der unten aufgeführten oder einer der [generischen Seitentypen](#generische_seitentypen) sein.

- `css-at-rule`: eine [Regel](/de/docs/Web/CSS/Guides/Syntax/At-rules), wie {{cssxref("@media")}}.
- `css-at-rule-descriptor`: ein Regel-Deskriptor, wie [`@counter-style/prefix`](/de/docs/Web/CSS/Reference/At-rules/@counter-style/prefix).
- `css-combinator`: ein Kombinator, wie der [Nachkomme-Kombinator](/de/docs/Web/CSS/Reference/Selectors/Descendant_combinator).
...
- `css-media-feature`: ein [Medienmerkmal](/de/docs/Web/CSS/Reference/At-rules/@media#media_features), wie {{cssxref("@media/hover")}}.
...
- `css-selector`: ein [einfacher Selektor](/de/docs/Web/CSS/Guides/Selectors/Selectors_and_combinators#basic_selectors), wie der [Klassenselektor](/de/docs/Web/CSS/Reference/Selectors/Class_selectors).
- `css-shorthand-property`: eine [Kurzform-Eigenschaft](/de/docs/Web/CSS/Guides/Cascade/Shorthand_properties), wie {{cssxref("background")}}.
- `css-type`: ein [Datentyp](/de/docs/Web/CSS/Reference/Values/Data_types), wie {{cssxref("&lt;color&gt;")}}.

### Glossar Seitentypen

Dieser Abschnitt listet `page-type` Werte für Seiten unter [Glossary](/de/docs/Glossary) auf. Jede Seite in diesem Teil der Struktur muss einen `page-type` haben, und der Wert muss einer der unten aufgeführten sein.
...
Dieser Abschnitt listet `page-type` Werte für Seiten unter [Web/HTML](/de/docs/Web/HTML) auf. Jede Seite in diesem Teil der Struktur muss einen `page-type` haben, und der Wert muss einer der unten aufgeführten oder einer der [generischen Seitentypen](#generische_seitentypen) sein.
...
Dieser Abschnitt listet `page-type` Werte für Seiten unter [Web/HTTP](/de/docs/Web/HTTP) auf. Jede Seite in diesem Teil der Struktur muss einen `page-type` haben, und der Wert muss einer der unten aufgeführten oder einer der [generischen Seitentypen](#generische_seitentypen) sein.
...
Dieser Abschnitt listet `page-type` Werte für Seiten unter [Web/JavaScript](/de/docs/Web/JavaScript) auf. Jede Seite in diesem Teil der Struktur muss einen `page-type` haben, und der Wert muss einer der unten aufgeführten oder einer der [generischen Seitentypen](#generische_seitentypen) sein.
...
Dieser Abschnitt listet `page-type` Werte für Seiten unter [Web/MathML](/de/docs/Web/MathML) auf. Jede Seite in diesem Teil der Struktur muss einen `page-type` haben, und der Wert muss einer der unten aufgeführten oder einer der [generischen Seitentypen](#generische_seitentypen) sein.
...
Dieser Abschnitt listet `page-type` Werte für Seiten unter [Web/SVG](/de/docs/Web/SVG) auf. Jede Seite in diesem Teil der Struktur muss einen `page-type` haben, und der Wert muss einer der unten aufgeführten oder einer der [generischen Seitentypen](#generische_seitentypen) sein.
...
Dieser Abschnitt listet `page-type` Werte für Seiten unter [Web/API](/de/docs/Web/API) auf. Jede Seite in diesem Teil der Struktur muss einen `page-type` haben, und der Wert muss einer der unten aufgeführten oder einer der [generischen Seitentypen](#generische_seitentypen) sein.
...
Dieser Abschnitt listet `page-type` Werte für Seiten unter [WebAssembly/](/de/docs/WebAssembly) auf. Jede Seite in diesem Teil der Struktur muss einen `page-type` haben, und der Wert muss einer der unten aufgeführten oder einer der generischen Seitentypen sein.

- `webassembly-function`: eine globale Funktion, das heißt, eine Methode direkt unter dem `WebAssembly`-Objekt, die als Namensraum fungiert, wie [`WebAssembly.instantiate()`](/de/docs/WebAssembly/Reference/JavaScript_interface/instantiate_static).
...
- `webassembly-instruction`: eine Anweisung oder ein Satz von Anweisungen, wie [`wrap`](/de/docs/WebAssembly/Reference/Numeric/wrap_i64).

### WebDriver Seitentypen

Dieser Abschnitt listet `page-type` Werte für Seiten unter [Web/WebDriver](/de/docs/Web/WebDriver) auf. Jede Seite in diesem Teil der Struktur muss einen `page-type` haben, und der Wert muss einer der unten aufgeführten oder einer der [generischen Seitentypen](#generische_seitentypen) sein.
...
Dieser Abschnitt listet `page-type` Werte für Seiten unter [Mozilla/Add-ons/WebExtensions](/de/docs/Mozilla/Add-ons/WebExtensions) auf. Jede Seite in diesem Teil der Struktur muss einen `page-type` haben, und der Wert muss einer der unten aufgeführten oder einer der [generischen Seitentypen](#generische_seitentypen) sein.
...
Dieser Abschnitt listet `page-type` Werte für Seiten unter [Web/Manifest](/de/docs/Web/Progressive_web_apps/Manifest) auf. Jede Seite in diesem Teil der Struktur muss einen `page-type` haben, und der Wert muss einer der unten aufgeführten oder einer der [generischen Seitentypen](#generische_seitentypen) sein.
...
Dieser Abschnitt listet `page-type` Werte für Seiten unter [Web/XPath](/de/docs/Web/XML/XPath) auf. Jede Seite in diesem Teil der Struktur muss einen `page-type` haben, und der Wert muss einer der unten aufgeführten oder einer der [generischen Seitentypen](#generische_seitentypen) sein.

- `xpath-function`: eine Funktion, wie [`ceiling()`](/de/docs/Web/XML/XPath/Reference/Functions/ceiling)

### XSLT Seitentypen

Dieser Abschnitt listet `page-type` Werte für Seiten unter [Web/XSLT](/de/docs/Web/XML/XSLT) auf. Jede Seite in diesem Teil der Struktur muss einen `page-type` haben, und der Wert muss einer der unten aufgeführten oder einer der [generischen Seitentypen](#generische_seitentypen) sein.

- `xslt-element`: ein XSLT-Element, wie [`<xsl:message>`](/de/docs/Web/XML/XSLT/Reference/Element/message).

### EXSLT Seitentypen

Dieser Abschnitt listet `page-type` Werte für Seiten unter [Web/EXSLT](/de/docs/Web/XML/EXSLT) auf. Jede Seite in diesem Teil der Struktur muss einen `page-type` haben, und der Wert muss einer der unten aufgeführten oder einer der [generischen Seitentypen](#generische_seitentypen) sein.

- `exslt-function`: eine EXSLT-Funktion, wie [`exsl:node-set()`](/de/docs/Web/XML/EXSLT/Reference/exsl/node-set).

### Firefox Seitentypen

Dieser Abschnitt listet `page-type` Werte für Seiten unter [Mozilla/Firefox](/de/docs/Mozilla/Firefox) auf. Jede Seite in diesem Teil der Struktur muss einen `page-type` haben, und der Wert muss einer der unten aufgeführten oder einer der [generischen Seitentypen](#generische_seitentypen) sein.

- `firefox-release-notes`: die Release Notes für eine bestimmte Firefox-Version, wie [_Firefox 115 für Entwickler_](/de/docs/Mozilla/Firefox/Releases/115).

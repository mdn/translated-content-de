...
Dies ermöglicht es MDN-Inhaltstools, die Inhaltsprüfung und die Organisation der Seitenleiste besser zu automatisieren.
...
Jeder Hauptbereich der Website — JavaScript, CSS und so weiter — hat eine Reihe von domainspezifischen `page-type`-Werten. Es gibt auch eine Reihe von generischen Werten, die in jedem Bereich der Website auftauchen können.
...
- `landing-page`: ein Überblick über das Thema, eine Einführung in den Abschnitt und Navigation zu Schlüsselbereichen.
...
Dieser Abschnitt listet `page-type`-Werte für Seiten unter [Learn](/de/docs/Learn_web_development) auf. Jede Seite in diesem Teil des Baums muss einen `page-type` haben, und sein Wert muss einer der unten aufgeführten oder einer der generischen Seitentypwerte sein.
...
- `learn-module`: ein Überblick über ein Modul, das heißt, eine geordnete Sammlung von Leitfäden wie [_Structuring content with HTML_](/de/docs/Learn_web_development/Core/Structuring_content).
...
- `learn-module-assessment`: ein spezieller Leitfaden mit einer Aktivität, die es ermöglicht, das Verständnis eines Moduls oder eines Teils davon zu bewerten, wie [_Test your skills: basic controls_](/de/docs/Learn_web_development/Extensions/Forms/Test_your_skills:_Basic_controls).
...
Dieser Abschnitt listet `page-type`-Werte für Seiten unter [Web/Accessibility](/de/docs/Web/Accessibility) auf. Jede Seite in diesem Teil des Baums muss einen `page-type` haben, und sein Wert muss einer der unten aufgeführten oder einer der [generischen Seitentyp](#generische_seitentypen)-Werte sein.
...
- `css-at-rule-descriptor`: ein at-rule Descriptor, wie [`@counter-style/prefix`](/de/docs/Web/CSS/@counter-style/prefix).
- `css-combinator`: ein Kombinator, wie der [descendant combinator](/de/docs/Web/CSS/Descendant_combinator).
...
- `javascript-language-feature`: ein Teil der JavaScript-Syntax, der in keine andere Kategorie passt, wie [rest parameters](/de/docs/Web/JavaScript/Reference/Functions/rest_parameters).
...
Dieser Abschnitt listet `page-type`-Werte für Seiten unter [WebAssembly/](/de/docs/WebAssembly) auf. Jede Seite in diesem Teil des Baums muss einen `page-type` haben, und sein Wert muss einer der unten aufgeführten oder einer der generischen Seitentypwerte sein.

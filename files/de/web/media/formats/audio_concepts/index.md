...
Der Prozess der Komprimierung und Dekomprimierung von Audio wird durch das Kodieren und Dekodieren mit einem Audio **{{Glossary("codec", "Codec")}}** (**CO**der/**DE**coder) durchgeführt. Im Laufe der Jahre wurde eine große Vielzahl von Codecs entwickelt, von denen mehrere häufig im Web verwendet werden. Details zu den wichtigsten und nützlichsten, die Webentwickler kennen sollten, finden Sie im Artikel [Leitfaden zu Audiocodecs, die im Web verwendet werden](/de/docs/Web/Media/Formats/Audio_codecs).

---
title: "ARIA: command-Rolle"
slug: Web/Accessibility/ARIA/Roles/command_role
l10n:
  sourceCommit: 194bd13942ad0c532c92d364e0d5d0c36732d98c
---

{{AccessibilitySidebar}}

Die `command`-Rolle definiert ein Widget, das eine Aktion ausführt, aber keine Eingabedaten empfängt.

> [!NOTE]
> Die `command`-Rolle ist eine abstrakte Rolle. Sie wird hier zur Vollständigkeit der Dokumentation aufgeführt. Webautoren sollten sie nicht verwenden.

## Beschreibung

Ein Befehl ist eine Art von Widget, das eine Aktion ausführt, aber keine Eingabedaten empfängt. Es ist eine Superklasse für die Widget-Rollen [`button`](/de/docs/Web/Accessibility/ARIA/Roles/button_role), [`link`](/de/docs/Web/Accessibility/ARIA/Roles/link_role), und [`menuitem`](/de/docs/Web/Accessibility/ARIA/Roles/menuitem_role).

## Beste Praktiken

Nicht verwenden.

## Spezifikationen

{{Specifications}}

## Siehe auch

- [ARIA: `widget`-Rolle](/de/docs/Web/Accessibility/ARIA/Roles/widget_role)
- [ARIA: `button`-Rolle](/de/docs/Web/Accessibility/ARIA/Roles/button_role)
- [ARIA: `link`-Rolle](/de/docs/Web/Accessibility/ARIA/Roles/link_role)
- [ARIA: `menuitem`-Rolle](/de/docs/Web/Accessibility/ARIA/Roles/menuitem_role)

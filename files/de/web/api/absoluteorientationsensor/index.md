...
Die **`AbsoluteOrientationSensor`**-Schnittstelle der [Sensor APIs](/de/docs/Web/API/Sensor_APIs) beschreibt die physische Ausrichtung des Geräts in Bezug auf das Erd-Koordinatensystem.

Um diesen Sensor zu verwenden, muss der Nutzer die Erlaubnis für die Gerätesensoren `'accelerometer'`, `'gyroscope'` und `'magnetometer'` über die [Permissions API](/de/docs/Web/API/Permissions_API) erteilen.
...
Die Verwendung von Orientierungssensoren erfordert die Anforderung von Berechtigungen für mehrere Gerätesensoren. Weil die [`Permissions`](/de/docs/Web/API/Permissions)-Schnittstelle Promises verwendet, ist eine gute Möglichkeit, Berechtigungen anzufordern, die Nutzung von {{jsxref('Promise.all')}}.

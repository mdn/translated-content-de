...
> [!NOTE]
> Dieses Beispiel funktioniert höchstwahrscheinlich in allen modernen Desktop-Browsern. Es kann jedoch sein, dass es in einigen mobilen oder älteren Browsern nicht funktioniert. Wenn die Leinwand leer bleibt, können Sie die Ausgabe des nächsten Beispiels überprüfen, das genau dasselbe zeichnet. Vergessen Sie jedoch nicht, die Erklärungen und den Code auf dieser Seite zu lesen, bevor Sie zum nächsten Beispiel übergehen.
...
function setupWebGL(evt) {
  ...
  if (!gl.getProgramParameter(program, gl.LINK_STATUS)) {
    ...
    document.querySelector("p").textContent =
      `Shader-Programm wurde nicht erfolgreich verknüpft. Fehlerprotokoll: ${linkErrLog}`;
    return;
  }
...
function getRenderingContext() {
  ...
  if (!gl) {
    ...
    paragraph.textContent =
      "Fehlgeschlagen. Ihr Browser oder Gerät unterstützt möglicherweise WebGL nicht.";
    return null;
  }

...
Das `PluginArray`-Interface wird verwendet, um eine Liste von [`Plugin`](/de/docs/Web/API/Plugin)-Objekten zu speichern; es wird durch die Eigenschaft [`navigator.plugins`](/de/docs/Web/API/Navigator/plugins) zurückgegeben. Das `PluginArray` ist kein JavaScript-Array, besitzt jedoch die Eigenschaft `length` und unterstützt den Zugriff auf einzelne Elemente mittels Klammernotation (`plugins[2]`), sowie über die Methoden `item(index)` und `namedItem("name")`.
...
Zusätzlich zur Auflistung jedes Plugins als Pseudo-Array durch Null-indizierte numerische Eigenschaften stellt Firefox Eigenschaften bereit, die der Plugin-Name direkt auf dem `PluginArray`-Objekt sind.

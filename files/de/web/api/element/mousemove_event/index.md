...
- [`MouseEvent.button`](/de/docs/Web/API/MouseEvent/button) {{ReadOnlyInline}}
  - : Die Zahl der Taste, die gedrückt wurde (sofern zutreffend), als das Mausereignis ausgelöst wurde.
...
- [`MouseEvent.mozInputSource`](/de/docs/Web/API/MouseEvent/mozInputSource) {{non-standard_inline()}} {{ReadOnlyInline}}
  - : Der Typ des Geräts, das das Ereignis erzeugt hat (einer der `MOZ_SOURCE_*`-Konstanten). Dies ermöglicht es Ihnen, zum Beispiel zu bestimmen, ob ein Mausereignis von einer tatsächlichen Maus oder von einem Touch-Ereignis generiert wurde (was die Genauigkeit der Interpretierung der Koordinaten des Ereignisses beeinflussen könnte).
...
Während sich die Maus über die Seite bewegt, wird das `mousemove`-Ereignis ausgelöst. Wenn `isDrawing` true ist, ruft der Ereignishandler die Funktion `drawLine` auf, um eine Linie von den gespeicherten `x`- und `y`-Werten zur aktuellen Position zu zeichnen.

...
Jedes Rechtsklick-Ereignis, das nicht deaktiviert wurde (durch Aufrufen der `preventDefault()`-Methode des Klickevents), führt dazu, dass ein `contextmenu`-Ereignis auf das anvisierte Element ausgelöst wird.
...
- [`PointerEvent.tangentialPressure`](/de/docs/Web/API/PointerEvent/tangentialPressure) {{ReadOnlyInline}}
  - : Der normale tangentiale Druck des Zeigereingangs (auch bekannt als Barrel-Druck oder [Zylinderbelastung](https://en.wikipedia.org/wiki/Cylinder_stress)) im Bereich von `-1` bis `1`, wobei `0` die neutrale Position der Steuerung darstellt.
- [`PointerEvent.tiltX`](/de/docs/Web/API/PointerEvent/tiltX) {{ReadOnlyInline}}
  - : Der Winkel in der Ebene (in Grad, im Bereich von `-90` bis `90`) zwischen der Y-Z-Ebene und der Ebene, die sowohl die Zeigerachse (z.B. Stift) als auch die Y-Achse enthält.
- [`PointerEvent.tiltY`](/de/docs/Web/API/PointerEvent/tiltY) {{ReadOnlyInline}}
  - : Der Winkel in der Ebene (in Grad, im Bereich von `-90` bis `90`) zwischen der X-Z-Ebene und der Ebene, die sowohl die Zeigerachse (z.B. Stift) als auch die X-Achse enthält.
...
In diesem Beispiel wird die Standardaktion des `contextmenu`-Ereignisses mit `preventDefault()` abgebrochen, wenn das `contextmenu`-Ereignis beim ersten Absatz ausgelöst wird. Infolgedessen wird der erste Absatz beim Rechtsklick nichts tun, während der zweite Absatz das standardmäßige Kontextmenü Ihres Browsers anzeigen wird.

> [!NOTE]
> In Firefox wird das Kontextmenü angezeigt, wenn Sie die <kbd>Shift</kbd>-Taste gedrückt halten und mit der rechten Maustaste klicken, ohne dass das `contextmenu`-Ereignis ausgelöst wird. Daher stoppt das Abbrechen des Ereignisses die Anzeige des Kontextmenüs nicht.

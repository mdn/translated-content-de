...
- Ein `wheel`-Ereignis löst nicht unbedingt ein `scroll`-Ereignis aus. Zum Beispiel könnte das Element überhaupt nicht scrollbar sein. Auch Zoom-Aktionen, die das Rad oder das Trackpad nutzen, lösen `wheel`-Ereignisse aus (mit [`ctrlKey`](/de/docs/Web/API/MouseEvent/ctrlKey) auf true gesetzt).

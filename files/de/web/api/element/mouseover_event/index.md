...
Wenn das Zielelement Kindelemente hat, werden `mouseout`- und `mouseover`-Ereignisse ausgelöst, während die Maus über die Grenzen dieser Elemente bewegt wird, nicht nur über das Zielelement selbst. Normalerweise ist das Verhalten von `mouseenter`- und `mouseleave`-Ereignissen sinnvoller, da sie nicht vom Bewegen in Kindelemente beeinflusst werden.
...
- [`MouseEvent.mozInputSource`](/de/docs/Web/API/MouseEvent/mozInputSource) {{non-standard_inline()}} {{ReadOnlyInline}}
  - : Der Typ des Geräts, das das Ereignis erzeugt hat (eine der `MOZ_SOURCE_*` Konstanten). Dies ermöglicht es Ihnen z.B. festzustellen, ob ein Mausereignis von einer tatsächlichen Maus oder durch ein Berührungsereignis generiert wurde (was die Genauigkeit beeinflussen kann, mit der Sie die mit dem Ereignis verbundenen Koordinaten interpretieren).

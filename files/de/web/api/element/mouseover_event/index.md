...
Wenn das Zielelement Kindelemente hat, werden `mouseout`- und `mouseover`-Ereignisse ausgelöst, wenn die Maus über die Grenzen dieser Elemente hinweg bewegt wird, nicht nur über das Zielelement selbst. Normalerweise ist das Verhalten von `mouseenter`- und `mouseleave`-Ereignissen sinnvoller, da sie nicht betroffen sind, wenn in Kindelemente hineingefahren wird.
...
- [`MouseEvent.mozInputSource`](/de/docs/Web/API/MouseEvent/mozInputSource) {{non-standard_inline()}} {{ReadOnlyInline}}
  - : Der Typ des Geräts, das das Ereignis erzeugt hat (einer der `MOZ_SOURCE_*`-Konstanten).
    Damit können Sie beispielsweise feststellen, ob ein Mausereignis durch eine tatsächliche Maus oder ein Berührungsereignis erzeugt wurde (was die Genauigkeit beeinflussen könnte, mit der Sie die mit dem Ereignis verbundenen Koordinaten interpretieren).
...
Das folgende Beispiel veranschaulicht den Unterschied zwischen `mouseover`- und [`mouseenter`](/de/docs/Web/API/Element/mouseenter_event)-Ereignissen.

...
Das **`mouseup`**-Ereignis wird auf einem [`Element`](/de/docs/Web/API/Element) ausgelöst, wenn eine Taste eines Zeigegeräts (wie einer Maus oder eines Trackpads) losgelassen wird, während sich der Zeiger innerhalb des Elements befindet.

`mouseup`-Ereignisse sind das Gegenstück zu [`mousedown`](/de/docs/Web/API/Element/mousedown_event)-Ereignissen.

Dieses Verhalten unterscheidet sich von [`pointerup`](/de/docs/Web/API/Element/pointerup_event)-Ereignissen. Beim Verwenden einer physischen Maus werden `mouseup`-Ereignisse ausgelöst, wenn eine beliebige Taste einer Maus losgelassen wird. `pointerup`-Ereignisse werden nur beim Loslassen der letzten Taste ausgelöst; vorherige Tastfreigaben, während andere Tasten gedrückt gehalten werden, lösen keine `pointerup`-Ereignisse aus.
...
- [`MouseEvent.mozInputSource`](/de/docs/Web/API/MouseEvent/mozInputSource) {{non-standard_inline()}} {{ReadOnlyInline}}
  - : Der Typ des Geräts, das das Ereignis generiert hat (eines der `MOZ_SOURCE_*` Konstanten).
    Dies ermöglicht es Ihnen beispielsweise festzustellen, ob ein Mausereignis von einer tatsächlichen Maus oder durch ein Touch-Ereignis generiert wurde (was die Genauigkeit der Interpretation der mit dem Ereignis verbundenen Koordinaten beeinflussen könnte).

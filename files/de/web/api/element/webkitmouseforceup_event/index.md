...
Das nicht-standardisierte **`webkitmouseforceup`**-Ereignis wird von Safari bei einem [`Element`](/de/docs/Web/API/Element) einige Zeit nach dem [`webkitmouseforcedown`](/de/docs/Web/API/Element/webkitmouseforcedown_event)-Ereignis ausgelöst, wenn der Druck auf den Button ausreichend verringert wurde, um den "Force Click" zu beenden.

**`webkitmouseforceup`** ist ein proprietäres, WebKit-spezifisches Ereignis. Es ist Teil der [Force Touch Events](/de/docs/Web/API/Force_Touch_events)-Funktion.
...
- [`MouseEvent.buttons`](/de/docs/Web/API/MouseEvent/buttons) {{ReadOnlyInline}}
  - : Die Buttons, die gedrückt wurden (falls vorhanden), als das Mausereignis ausgelöst wurde.
...
- [`MouseEvent.mozInputSource`](/de/docs/Web/API/MouseEvent/mozInputSource) {{non-standard_inline()}} {{ReadOnlyInline}}
  - : Der Typ des Geräts, das das Ereignis erzeugt hat (eines der `MOZ_SOURCE_*`-Konstanten). Dies ermöglicht es Ihnen beispielsweise festzustellen, ob ein Mausereignis von einer tatsächlichen Maus oder von einem Touch-Ereignis generiert wurde (was die Genauigkeit beeinflussen könnte, mit der die mit dem Ereignis verbundenen Koordinaten interpretiert werden).
...
_Teilt keine Spezifikation._ Apple hat [eine Beschreibung in der Mac Developer Library](https://developer.apple.com/library/archive/documentation/AppleApplications/Conceptual/SafariJSProgTopics/RespondingtoForceTouchEventsfromJavaScript.html).

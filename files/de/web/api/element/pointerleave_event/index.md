...
Das `pointerleave`-Ereignis wird ausgelöst, wenn ein Zeigegerät aus den Treffer-Testgrenzen eines Elements herausbewegt wird. Bei Stiftgeräten wird dieses Ereignis ausgelöst, wenn der Stift den von der Digitalisier-Einheit erkennbaren Schwebebereich verlässt.
...
- [`PointerEvent.altitudeAngle`](/de/docs/Web/API/PointerEvent/altitudeAngle) {{ReadOnlyInline}} {{experimental_inline}}
  - : Stellt den Winkel zwischen der Achse eines Wandlers (eines Zeigers oder Stylus) und der X-Y-Ebene des Gerätescreens dar.
...
- [`PointerEvent.tangentialPressure`](/de/docs/Web/API/PointerEvent/tangentialPressure) {{ReadOnlyInline}}
  - : Der normalisierte Tangentialdruck der Zeigereingabe (auch als Fassdruck oder [Zylinderbelastung](https://en.wikipedia.org/wiki/Cylinder_stress) bekannt) im Bereich von `-1` bis `1`, wobei `0` die neutrale Position der Steuerung ist.

...
Das `pointerover`-Ereignis wird ausgelöst, wenn ein Zeigegerät in die Hit-Test-Grenzen eines Elements verschoben wird.

`pointerover`-Ereignisse haben die gleichen Probleme wie [`mouseover`](/de/docs/Web/API/Element/mouseover_event). Wenn das Ziel-Element untergeordnete Elemente hat, werden `pointerout`- und `pointerover`-Ereignisse ausgelöst, sobald der Zeiger die Grenzen dieser Elemente überschreitet, und nicht nur bei dem Ziel-Element selbst. Üblicherweise ist das Verhalten der [`pointerenter`](/de/docs/Web/API/Element/pointerenter_event)- und [`pointerleave`](/de/docs/Web/API/Element/pointerleave_event)-Ereignisse sinnvoller, da sie nicht durch das Bewegen auf untergeordnete Elemente beeinflusst werden.
...
- [`PointerEvent.altitudeAngle`](/de/docs/Web/API/PointerEvent/altitudeAngle) {{ReadOnlyInline}} {{experimental_inline}}
  - : Stellt den Winkel zwischen der Achse eines Transducers (einem Zeigegerät oder Stylus) und der X-Y-Ebene eines Gerätebildschirms dar.
- [`PointerEvent.azimuthAngle`](/de/docs/Web/API/PointerEvent/azimuthAngle) {{ReadOnlyInline}} {{experimental_inline}}
  - : Stellt den Winkel zwischen der Y-Z-Ebene und der Ebene dar, die sowohl die Achse des Transducers (einem Zeigegerät oder Stylus) als auch die Y-Achse enthält.
...
- [`PointerEvent.tangentialPressure`](/de/docs/Web/API/PointerEvent/tangentialPressure) {{ReadOnlyInline}}
  - : Der normalisierte tangentiale Druck der Zeigereingabe (auch bekannt als Barrel Pressure oder [Zylinderbelastung](https://en.wikipedia.org/wiki/Cylinder_stress)) im Bereich von `-1` bis `1`, wobei `0` die neutrale Position der Steuerung ist.

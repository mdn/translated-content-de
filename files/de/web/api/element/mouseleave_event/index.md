...
`mouseleave` und [`mouseout`](/de/docs/Web/API/Element/mouseout_event) sind ähnlich, unterscheiden sich jedoch darin, dass `mouseleave` nicht aufsteigt und `mouseout` dies tut. Das bedeutet, dass `mouseleave` ausgelöst wird, wenn der Zeiger das Element _und_ alle seine Nachkommen verlassen hat, während `mouseout` ausgelöst wird, wenn der Zeiger das Element _oder_ einen der Nachkommen des Elements verlässt (selbst wenn der Zeiger sich noch innerhalb des Elements befindet).

Die `mouseleave`- und `mouseout`-Ereignisse werden nicht ausgelöst, wenn das Element ersetzt oder aus dem DOM entfernt wird.

Beachten Sie, dass sich "das Verlassen eines Elements" auf die Position des Elements im DOM-Baum bezieht, nicht auf seine optische Position. Zum Beispiel, wenn zwei Geschwisterelemente so positioniert sind, dass eines innerhalb des anderen platziert ist, wird das Wechseln vom äußeren Element ins innere Element `mouseleave` am äußeren Element auslösen, obwohl der Zeiger sich noch innerhalb der Grenzen des äußeren Elements befindet.
...
- [`MouseEvent.buttons`](/de/docs/Web/API/MouseEvent/buttons) {{ReadOnlyInline}}
  - : Die Tasten, die gedrückt wurden (falls zutreffend), als das Mausereignis ausgelöst wurde.
...
- [`MouseEvent.mozInputSource`](/de/docs/Web/API/MouseEvent/mozInputSource) {{non-standard_inline()}} {{ReadOnlyInline}}
  - : Der Typ des Geräts, das das Ereignis erzeugt hat (einer der `MOZ_SOURCE_*`-Konstanten).
    Dies erlaubt Ihnen beispielsweise festzustellen, ob ein Mausereignis von einer tatsächlichen Maus oder einem Touch-Ereignis erzeugt wurde (was die Genauigkeit beeinflussen könnte, mit der Sie die mit dem Ereignis verbundenen Koordinaten interpretieren).
- [`MouseEvent.webkitForce`](/de/docs/Web/API/MouseEvent/webkitForce) {{non-standard_inline()}} {{ReadOnlyInline}}
  - : Der bei einem Klick angewendete Druck.
...
Ein `mouseleave`-Ereignis wird an jedes Element der Hierarchie gesendet, wenn es verlassen wird. Hier werden vier Ereignisse an die vier Elemente der Hierarchie gesendet, wenn der Zeiger sich vom Text zu einem Bereich außerhalb des äußersten hier dargestellten `div`-Elements bewegt.

### Verhalten von `mouseout`-Ereignissen

![Diagramm des `mouseout`-Verhaltens](mouseout.png)

Ein einziges `mouseout`-Ereignis wird an das tiefste Element des DOM-Baums gesendet, dann steigt es die Hierarchie auf, bis es von einem Handler abgebrochen wird oder die Wurzel erreicht.
...
Das folgende triviale Beispiel verwendet das `mouseenter`-Ereignis, um den Rand des `<div>` zu ändern, wenn die Maus in den ihr zugewiesenen Bereich eintritt. Es fügt dann einen Eintrag zur Liste hinzu mit der Nummer des `mouseenter`- oder `mouseleave`-Ereignisses.
...
Stilierung des `<div>`, um es sichtbarer zu machen.

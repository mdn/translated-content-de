...
Das nicht standardisierte **`webkitmouseforcechanged`**-Ereignis wird von Safari jedes Mal ausgelöst, wenn sich der Druck auf das Trackpad/Touchscreen ändert.

**`webkitmouseforcechanged`** ist ein proprietäres, WebKit-spezifisches Ereignis, das von Apple eingeführt wurde, um ihre Funktion [Force Touch Events](/de/docs/Web/API/Force_Touch_events) zu unterstützen.

Dieses Ereignis wird zuerst nach dem [`mousedown`](/de/docs/Web/API/Element/mousedown_event)-Ereignis ausgelöst und endet, bevor das [`mouseup`](/de/docs/Web/API/Element/mouseup_event)-Ereignis ausgelöst wird.
...
- [`MouseEvent.mozInputSource`](/de/docs/Web/API/MouseEvent/mozInputSource) {{non-standard_inline()}} {{ReadOnlyInline}}
  - : Der Typ des Geräts, das das Ereignis erzeugte (einer der `MOZ_SOURCE_*`-Konstanten).
...
- [`MouseEvent.webkitForce`](/de/docs/Web/API/MouseEvent/webkitForce) {{non-standard_inline()}} {{ReadOnlyInline}}
  - : Die angewandte Druckmenge beim Klicken.

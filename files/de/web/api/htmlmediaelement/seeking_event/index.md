...
Das `seeking`-Event wird ausgelöst, wenn eine Suchoperation beginnt, was bedeutet, dass das boolesche Attribut `seeking` auf `true` gewechselt ist und das Medium eine neue Position sucht.

Dieses Ereignis kann nicht abgebrochen werden und es erfolgt keine Bubbling.
...
Diese Beispiele fügen einen Ereignislistener für das `seeking`-Event des HTMLMediaElements hinzu und senden dann eine Nachricht, wenn der Event-Handler auf das Auslösen des Ereignisses reagiert hat.
...
Verwendung der `onseeking` Event-Handler-Eigenschaft:

...
Das **`DOMPointReadOnly`**-Interface legt die Koordinaten- und Perspektivenfelder fest, die von [`DOMPoint`](/de/docs/Web/API/DOMPoint) verwendet werden, um einen 2D- oder 3D-Punkt in einem Koordinatensystem zu definieren.

Es gibt zwei Möglichkeiten, eine neue `DOMPointReadOnly`-Instanz zu erstellen. Erstens, Sie können seinen Konstruktor verwenden und die Werte der Parameter für jede Dimension und optional die Perspektive übergeben:
...
- [`DOMPointReadOnly()`](/de/docs/Web/API/DOMPointReadOnly/DOMPointReadOnly)
  - : Erstellt ein neues `DOMPointReadOnly`-Objekt mit den gegebenen Werten für seine Koordinaten und Perspektive. Um einen Punkt mithilfe eines Objekts zu erstellen, können Sie stattdessen [`DOMPointReadOnly.fromPoint()`](/de/docs/Web/API/DOMPointReadOnly/fromPoint_static) verwenden.
...
- [`DOMPointReadOnly.fromPoint()`](/de/docs/Web/API/DOMPointReadOnly/fromPoint_static)
  - : Eine statische Methode, die ein neues `DOMPointReadOnly`-Objekt erstellt, gegeben die Koordinaten, die im angegebenen Objekt bereitgestellt werden.

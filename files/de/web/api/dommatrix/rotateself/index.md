...
Die `rotateSelf()` Methode der [`DOMMatrix`](/de/docs/Web/API/DOMMatrix) Schnittstelle ist eine veränderbare Transformationsmethode, die eine Matrix modifiziert. Sie rotiert die Quellmatrix um jede ihrer Achsen um die angegebene Anzahl von Grad und gibt die gedrehte Matrix zurück.
...
Wenn nur ein Parameter übergeben wird, ist `rotZ` der Wert von `rotX`, und sowohl `rotX` als auch `rotY` sind `0`, und die Rotation ist eine 2D-Rotation. Wenn `rotX` und `rotY` ungleich Null sind, ist der [`is_2d`](/de/docs/Web/API/DOMMatrix#is2d) `false`.

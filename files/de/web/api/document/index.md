...
Der DOM-Baum umfasst Elemente wie {{HTMLElement("body")}} und {{HTMLElement("table")}}, sowie [viele andere](/de/docs/Web/HTML/Element). Er bietet globale Funktionalität für das Dokument, wie zum Beispiel, wie die URL der Seite abgerufen und neue Elemente im Dokument erstellt werden.

{{InheritanceDiagram}}

Das `Document` Interface beschreibt die allgemeinen Eigenschaften und Methoden für jede Art von Dokument. Abhängig vom Dokumenttyp (z. B. [HTML](/de/docs/Web/HTML), [XML](/de/docs/Web/XML), SVG, ...), steht eine größere API zur Verfügung: HTML-Dokumente, die mit dem Inhaltstyp `"text/html"` bereitgestellt werden, implementieren ebenfalls das [`HTMLDocument`](/de/docs/Web/API/HTMLDocument) Interface, während XML- und SVG-Dokumente das [`XMLDocument`](/de/docs/Web/API/XMLDocument) Interface implementieren.
...
- [`Document.activeElement`](/de/docs/Web/API/Document/activeElement) {{ReadOnlyInline}}
  - : Gibt das [`Element`](/de/docs/Web/API/Element) zurück, welches aktuell den Fokus hat.
- [`Document.adoptedStyleSheets`](/de/docs/Web/API/Document/adoptedStyleSheets)
  - : Fügt ein Array von konstruierten Stylesheets hinzu, die vom Dokument verwendet werden sollen.
    Diese Stylesheets können auch mit Shadow-DOM-Subtrees des gleichen Dokuments geteilt werden.
...
- [`Document.prerendering`](/de/docs/Web/API/Document/prerendering) {{ReadOnlyInline}} {{experimental_inline}}
  - : Gibt einen Boolean-Wert zurück, der anzeigt, ob das Dokument derzeit im Prozess des Vorbereitens ist, wie er über die [Speculation Rules API](/de/docs/Web/API/Speculation_Rules_API) initiiert wurde.
...
- [`Document.visibilityState`](/de/docs/Web/API/Document/visibilityState) {{ReadOnlyInline}}
  - : Gibt einen `string` zurück, der den Sichtbarkeitszustand des Dokuments anzeigt. Mögliche Werte sind `visible`, `hidden`, `prerender` und `unloaded`.
...
- [`Document.dir`](/de/docs/Web/API/Document/dir)
  - : Ruft die Ausrichtung (rtl/ltr) des Dokuments ab oder setzt diese.
...
- [`Document.charset`](/de/docs/Web/API/Document/characterSet) {{Deprecated_Inline}} {{ReadOnlyInline}}
  - : Alias von [`Document.characterSet`](/de/docs/Web/API/Document/characterSet). Diese Eigenschaft sollte stattdessen verwendet werden.
...
- [`Document.rootElement`](/de/docs/Web/API/Document/rootElement) {{Deprecated_Inline}}
  - : Wie [`Document.documentElement`](/de/docs/Web/API/Document/documentElement), aber nur für {{SVGElement("svg")}}-Wurzelelemente. Diese Eigenschaft sollte stattdessen verwendet werden.
...
- [`Document.styleSheetSets`](/de/docs/Web/API/Document/styleSheetSets) {{Deprecated_Inline}} {{ReadOnlyInline}} {{Non-standard_Inline}}
  - : Gibt eine Liste der auf das Dokument verfügbar Stylesheet-Sets zurück.
...
- [`Document.xmlStandalone`](/de/docs/Web/API/Document/xmlStandalone) {{Deprecated_Inline}}
  - : Gibt `true` zurück, wenn die XML-Deklaration das Dokument als eigenständig angibt (_z.B._, ein externer Teil des DTD beeinflusst den Inhalt des Dokuments), andernfalls `false`.
...
- [`Document.adoptNode()`](/de/docs/Web/API/Document/adoptNode)
  - : Adoptiert einen Knoten von einem externen Dokument.
...
- [`Document.caretPositionFromPoint()`](/de/docs/Web/API/Document/caretPositionFromPoint)
  - : Gibt ein [`CaretPosition`](/de/docs/Web/API/CaretPosition)-Objekt zurück, das den DOM-Knoten enthält, der die Einfügemarke enthält, und den Versatz des Cursors innerhalb dieses Knotens.
...
- [`Document.enableStyleSheetsForSet()`](/de/docs/Web/API/Document/enableStyleSheetsForSet) {{Deprecated_Inline}} {{Non-standard_Inline}}
  - : Aktiviert die Stylesheets für das angegebene Styleset.
- [`Document.exitFullscreen()`](/de/docs/Web/API/Document/exitFullscreen)
  - : Beendet die Darstellung des Vollbild-Elements des Dokuments.
...
- [`Document.getAnimations()`](/de/docs/Web/API/Document/getAnimations)
  - : Gibt ein Array aller [`Animation`](/de/docs/Web/API/Animation)-Objekte zurück, die aktuell wirksam sind und deren Ziel-Elemente Nachkommen des `Dokuments` sind.
...
- [`Document.getElementById()`](/de/docs/Web/API/Document/getElementById)
  - : Gibt eine Referenz auf das identifizierte Element zurück.
...
- [`Document.hasStorageAccess()`](/de/docs/Web/API/Document/hasStorageAccess)
  - : Gibt ein {{jsxref("Promise")}} zurück, das sich mit einem Boolean-Wert erfüllt, der angibt, ob das Dokument Zugriff auf nicht-partitionierte Cookies hat.
...
- [`Document.querySelector()`](/de/docs/Web/API/Document/querySelector)
  - : Gibt den ersten Elementknoten im Dokument in Dokumentreihenfolge zurück, der mit den angegebenen Selektoren übereinstimmt.
- [`Document.querySelectorAll()`](/de/docs/Web/API/Document/querySelectorAll)
  - : Gibt eine Liste aller Elementknoten im Dokument zurück, die mit den angegebenen Selektoren übereinstimmen.
- [`Document.releaseCapture()`](/de/docs/Web/API/Document/releaseCapture) {{Non-standard_Inline}}
  - : Aufheben der aktuellen Mausaufnahme, wenn sie sich auf einem Element in diesem Dokument befindet.
...
- [`Document.requestStorageAccessFor()`](/de/docs/Web/API/Document/requestStorageAccessFor) {{experimental_inline}}
  - : Ermöglicht es Websites auf oberster Ebene, im Auftrag von eingebetteten Inhalten, die von einer anderen Website im selben [related website set](/de/docs/Web/API/Storage_Access_API/Related_website_sets) stammen, den Zugriff auf Drittanbieter-Cookies anzufordern.
- [`Document.startViewTransition()`](/de/docs/Web/API/Document/startViewTransition)
  - : Startet eine neue [Ansichtstransition](/de/docs/Web/API/View_Transitions_API) und gibt ein [`ViewTransition`](/de/docs/Web/API/ViewTransition)-Objekt zurück, um diese zu repräsentieren.
...
- [`Document.parseHTMLUnsafe()`](/de/docs/Web/API/Document/parseHTMLUnsafe_static)
  - : Erstellt ein neues `Document` Objekt aus einer HTML-Zeichenkette, ohne eine Säuberung durchzuführen.
    Die Zeichenkette kann deklarative Shadow Roots enthalten.

## Ereignisse

Diese Ereignisse können mit `addEventListener()` belauscht werden oder indem ein Ereignis-Listener der `oneventname` Eigenschaft dieses Interfaces zugewiesen wird. Neben den unten aufgeführten Ereignissen können viele Ereignisse von [Knoten](/de/docs/Web/API/Node) im Dokumentbaum hervorgehoben werden.
...
- [`prerenderingchange`](/de/docs/Web/API/Document/prerenderingchange_event) {{experimental_inline}}
  - : Wird bei einem vorbereiteten Dokument ausgelöst, wenn es aktiviert wird (d. h. der Benutzer die Seite betrachtet).
...
- [`DOMContentLoaded`](/de/docs/Web/API/Document/DOMContentLoaded_event)
  - : Wird ausgelöst, wenn das Dokument vollständig geladen und analysiert wurde, ohne darauf zu warten, dass Stylesheets, Bilder und Unterrahmen das Laden abschließen.

...
gibt eine statische (nicht dynamische) [`NodeList`](/de/docs/Web/API/NodeList) zurück, die eine Liste
...
- `selectors`
  - : Ein String, der einen oder mehrere Selektoren zum Abgleichen enthält. Dieser String
    muss ein gültiger CSS-Selektor-String sein; wenn nicht, wird eine `SyntaxError` Ausnahme
    ausgelöst.

    Beachten Sie, dass die HTML-Spezifikation nicht verlangt, dass Attributwerte gültige CSS-Bezeichner sind. Wenn ein [`class`](/de/docs/Web/HTML/Reference/Global_attributes/class)- oder [`id`](/de/docs/Web/HTML/Reference/Global_attributes/id)-Attributwert kein gültiger CSS-Bezeichner ist, müssen Sie ihn entweder durch Aufrufen von [`CSS.escape()`](/de/docs/Web/API/CSS/escape_static) auf den Wert oder durch eine der in [Escaping characters](/de/docs/Web/CSS/Reference/Values/ident#escaping_characters) beschriebenen Techniken im Selektor maskieren. Siehe [Escaping attribute values](#attributwerte_maskieren) für ein Beispiel.

### Rückgabewert

Eine nicht-dynamische [`NodeList`](/de/docs/Web/API/NodeList), die ein [`Element`](/de/docs/Web/API/Element)-Objekt für
jedes Element enthält, das mit mindestens einem der angegebenen Selektoren übereinstimmt oder eine leere
[`NodeList`](/de/docs/Web/API/NodeList) bei keinen Übereinstimmungen. Die Elemente sind in Dokumentenreihenfolge — das heißt, Eltern vor Kindern, frühere Geschwister vor späteren Geschwistern.

> [!NOTE]
> Wenn die angegebenen `selectors` ein [CSS-Pseudoelement](/de/docs/Web/CSS/Reference/Selectors/Pseudo-elements) einschließen, ist die zurückgegebene Liste
> immer leer.
...
Um eine [`NodeList`](/de/docs/Web/API/NodeList) von allen {{HTMLElement("p")}} Elementen im
...
Dieses Beispiel gibt eine Liste aller {{HTMLElement("div")}} Elemente im Dokument
...
ein {{HTMLElement("div")}} mit der Klasse `highlighted` ist und die
...
Dieses Beispiel verwendet einen [Attributselektor](/de/docs/Web/CSS/Reference/Selectors/Attribute_selectors), um eine Liste der {{HTMLElement("iframe")}} Elemente im
...
Sobald die [`NodeList`](/de/docs/Web/API/NodeList) der übereinstimmenden Elemente zurückgegeben wird, können Sie sie
...
können jede übliche Schleifenanweisung verwenden, wie zum Beispiel:
...
Dieses Beispiel zeigt, dass wir, wenn ein HTML-Dokument eine [`id`](/de/docs/Web/HTML/Reference/Global_attributes/id) enthält, die kein gültiger [CSS-Bezeichner](/de/docs/Web/CSS/Reference/Values/ident) ist, den Attributwert maskieren müssen, bevor er in `querySelectorAll()` verwendet wird.
...
- Der zweite Button maskiert den Wert mit [`CSS.escape()`](/de/docs/Web/API/CSS/escape_static).

...
Die **`ANGLE_instanced_arrays`** Erweiterung ist Teil der [WebGL API](/de/docs/Web/API/WebGL_API) und ermöglicht es, dasselbe Objekt oder Gruppen ähnlicher Objekte mehrfach zu zeichnen, sofern sie die gleichen Vertex-Daten, Primitätszähler und Typen teilen.
...
> [!NOTE]
> Diese Erweiterung ist nur für {{domxref("WebGLRenderingContext", "WebGL1", "", 1)}} Kontexte verfügbar. In {{domxref("WebGL2RenderingContext", "WebGL2", "", 1)}} ist die Funktionalität dieser Erweiterung im WebGL2-Kontext standardmäßig verfügbar und die Konstanten und Methoden sind ohne das `ANGLE_` Suffix verfügbar.
>
> Trotz des Namens "ANGLE" funktioniert diese Erweiterung auf jedem Gerät, wenn die Hardware diese unterstützt, und nicht nur unter Windows bei der Verwendung der ANGLE-Bibliothek. "ANGLE" zeigt lediglich an, dass diese Erweiterung von den Autoren der ANGLE-Bibliothek geschrieben wurde.
...
- `ext.VERTEX_ATTRIB_ARRAY_DIVISOR_ANGLE`
  - : Gibt ein [`GLint`](/de/docs/Web/API/WebGL_API/Types) zurück, das den Frequenzteiler beschreibt, der für das instanzierte Rendering verwendet wird, wenn es im [`gl.getVertexAttrib()`](/de/docs/Web/API/WebGLRenderingContext/getVertexAttrib) als `pname`-Parameter verwendet wird.
...
- [`ext.drawArraysInstancedANGLE()`](/de/docs/Web/API/ANGLE_instanced_arrays/drawArraysInstancedANGLE)
  - : Verhält sich identisch wie [`gl.drawArrays()`](/de/docs/Web/API/WebGLRenderingContext/drawArrays) außer dass mehrere Instanzen des Bereichs von Elementen ausgeführt werden und die Instanz bei jeder Iteration fortschreitet.
- [`ext.drawElementsInstancedANGLE()`](/de/docs/Web/API/ANGLE_instanced_arrays/drawElementsInstancedANGLE)
  - : Verhält sich identisch wie [`gl.drawElements()`](/de/docs/Web/API/WebGLRenderingContext/drawElements) außer dass mehrere Instanzen des Elementsatzes ausgeführt werden und die Instanz zwischen jedem Satz fortschreitet.
...
Das folgende Beispiel zeigt, wie man eine gegebene Geometrie mehrere Male mit einem einzigen Aufruf zeichnet.

> [!WARNING]
> Das folgende Beispiel ist zu Bildungszwecken gedacht und nicht für den produktiven Einsatz geeignet. Es sollte generell vermieden werden, Daten / Puffer innerhalb der Render-Schleife oder direkt vor der Nutzung zu erstellen.

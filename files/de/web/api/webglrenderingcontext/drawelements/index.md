...
- `count`
  - : Ein [`GLsizei`](/de/docs/Web/API/WebGL_API/Types), das die Anzahl der zu rendierenden Elemente des gebundenen Element-Array-Puffers angibt. Zum Beispiel, um ein Drahtgitter-Dreieck mit `gl.LINES` zu zeichnen, sollte die Anzahl 2 Endpunkte pro Linie × 3 Linien = 6 Elemente sein. Um jedoch dasselbe Drahtgitter-Dreieck mit `gl.LINE_STRIP` zu zeichnen, wiederholt der Element-Array-Puffer die Indizes für das Ende der ersten Linie/Anfang der zweiten Linie und das Ende der zweiten Linie/Anfang der dritten Linie nicht, sodass `count` vier sein wird. Um dasselbe Dreieck mit `gl.LINE_LOOP` zu zeichnen, wird auch der erste/letzte Scheitelpunkt nicht wiederholt, daher wird `count` drei sein.
- `type`
...
- `offset`
  - : Ein [`GLintptr`](/de/docs/Web/API/WebGL_API/Types), das einen Byte-Offset im Element-Array-Puffer angibt. Muss ein gültiges Vielfaches der Größe des angegebenen `type` sein.

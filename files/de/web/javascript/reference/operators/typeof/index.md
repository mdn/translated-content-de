...
Die folgende Tabelle fasst die möglichen Rückgabewerte von `typeof` zusammen. Weitere Informationen zu Typen und Primitiven finden Sie auch auf der Seite [JavaScript Datenstrukturen](/de/docs/Web/JavaScript/Guide/Data_structures).
...
Ein Fix wurde für ECMAScript vorgeschlagen (über ein Opt-In), aber [wurde abgelehnt](https://web.archive.org/web/20160331031419/http://wiki.ecmascript.org:80/doku.php?id=harmony:typeof_null). Dies hätte zu `typeof null === "null"` geführt.

### Verwendung des new Operators

Alle mit [`new`](/de/docs/Web/JavaScript/Reference/Operators/new) aufgerufenen Konstruktorfunktionen geben Nicht-Primitiven (`"object"` oder `"function"`) zurück. Die meisten geben Objekte zurück, mit der bemerkenswerten Ausnahme von [`Function`](/de/docs/Web/JavaScript/Reference/Global_Objects/Function), die eine Funktion zurückgibt.
...
Allerdings führt die Verwendung von `typeof` bei lexikalischen Deklarationen ({{jsxref("Statements/let", "let")}} {{jsxref("Statements/const", "const")}}, und [`class`](/de/docs/Web/JavaScript/Reference/Statements/class)) im gleichen Block vor der Deklarationsstelle zu einem Auftreten eines {{jsxref("ReferenceError")}}. Block-skopierte Variablen befinden sich in einer _[temporalen Toten Zone](/de/docs/Web/JavaScript/Reference/Statements/let#temporal_dead_zone_tdz)_ vom Beginn des Blocks bis zur Verarbeitung der Initialisierung, währenddessen ein Zugriff einen Fehler auslöst.
...
Obwohl `document.all` ebenfalls {{Glossary("Falsy", "falsey")}} und [lose gleich](/de/docs/Web/JavaScript/Reference/Operators/Equality) `undefined` ist, ist es nicht [`undefined`](/de/docs/Web/JavaScript/Reference/Global_Objects/undefined). Der Fall, dass `document.all` den Typ `"undefined"` hat, wird in den Web-Standards als eine "bewusste Verletzung" des ursprünglichen ECMAScript-Standards für die Web-Kompatibilität klassifiziert.
...
Um potenziell nicht existierende Variablen zu überprüfen, die sonst einen {{jsxref("ReferenceError")}} auslösen würden, verwenden Sie `typeof nonExistentVar === "undefined"`, da dieses Verhalten nicht mit benutzerdefiniertem Code nachgeahmt werden kann.

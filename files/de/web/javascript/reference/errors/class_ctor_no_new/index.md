...
Die JavaScript-Ausnahme "class constructors must be invoked with 'new'" tritt auf, wenn ein [Konstruktor einer Klasse](/de/docs/Web/JavaScript/Reference/Classes) ohne das Schlüsselwort {{jsxref("Operators/new", "new")}} aufgerufen wird. Alle Klassenkonstruktoren müssen mit `new` aufgerufen werden.
...
Im JavaScript sind das _Aufrufen_ einer Funktion ohne `new` und das _Konstruktion_ einer Funktion mit `new` zwei unterschiedliche Operationen, und Funktionen können sich unterschiedlich verhalten, je nachdem, wie sie aufgerufen werden.

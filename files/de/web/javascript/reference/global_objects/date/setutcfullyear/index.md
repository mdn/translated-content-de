...
Die **`setUTCFullYear()`**-Methode von {{jsxref("Date")}}-Instanzen ändert das Jahr für dieses Datum entsprechend der Weltzeit.
...
- `yearValue`
  - : Eine ganze Zahl, die das Jahr darstellt. Zum Beispiel 1995.
...
Wenn ein von Ihnen angegebener Parameter außerhalb des erwarteten Bereichs liegt, versucht `setUTCFullYear()`, die anderen Parameter und die Datumsinformationen im {{jsxref("Date")}}-Objekt entsprechend zu aktualisieren. Wenn Sie beispielsweise 15 für `monthValue` angeben, wird das Jahr um 1 erhöht

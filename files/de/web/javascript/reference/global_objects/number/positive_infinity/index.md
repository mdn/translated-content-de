...
Gleich dem Wert der globalen {{jsxref("Infinity")}}-Eigenschaft.
...
Sie könnten die Eigenschaft `Number.POSITIVE_INFINITY` verwenden, um einen Fehlerzustand zu kennzeichnen, der im Erfolgsfall eine endliche Zahl zurückgibt. Beachten Sie jedoch, dass {{jsxref("NaN")}} in einem solchen Fall angemessener wäre.

Da `POSITIVE_INFINITY` eine statische Eigenschaft von {{jsxref("Number")}} ist, verwenden Sie immer `Number.POSITIVE_INFINITY`, anstatt es als Eigenschaft eines Zahlenwerts zu verwenden.

## Beispiele

### Verwendung von POSITIVE_INFINITY

Im folgenden Beispiel wird der Variable `bigNumber` ein Wert zugewiesen, der größer als der Maximalwert ist. Wenn die {{jsxref("Statements/if...else", "if")}}-Anweisung ausgeführt wird, hat `bigNumber` den Wert `Infinity`, so dass `bigNumber` vor der Fortsetzung auf einen handlicheren Wert gesetzt wird.

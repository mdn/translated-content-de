...
Die **`toString()`** Methode von {{jsxref("Symbol")}}-Werten gibt einen String zurück, der diesen Symbolwert darstellt.
...
Die `toString()`-Methode erfordert, dass ihr `this`-Wert ein `Symbol`-Primitiv oder Wrapper-Objekt ist. Sie wirft einen {{jsxref("TypeError")}} für andere `this`-Werte, ohne zu versuchen, sie in Symbolwerte zu konvertieren.

Da `Symbol` eine [`[Symbol.toPrimitive]()`](/de/docs/Web/JavaScript/Reference/Global_Objects/Symbol/Symbol.toPrimitive) Methode hat, hat diese Methode immer Vorrang vor `toString()`, wenn ein `Symbol`-Objekt [in einen String umgewandelt](/de/docs/Web/JavaScript/Reference/Global_Objects/String#string_coercion) wird. Allerdings, weil `Symbol.prototype[Symbol.toPrimitive]()` ein Symbol-Primitive zurückgibt und Symbol-Primitives einen {{jsxref("TypeError")}} werfen, wenn sie implizit in einen String konvertiert werden, wird die `toString()`-Methode nie implizit von der Sprache aufgerufen. Um ein Symbol zu einer Zeichenkette zu machen, müssen Sie explizit seine `toString()`-Methode aufrufen oder die [`String()`](/de/docs/Web/JavaScript/Reference/Global_Objects/String/String#using_string_to_stringify_a_symbol) Funktion verwenden.
...
Die einzige Möglichkeit, JavaScript dazu zu bringen, `toString()` anstelle von [`[Symbol.toPrimitive]()`](/de/docs/Web/JavaScript/Reference/Global_Objects/Symbol/Symbol.toPrimitive) auf einem Symbol-Wrapper-Objekt implizit aufzurufen, besteht darin, die Methode `[Symbol.toPrimitive]()` zuerst zu [löschen](/de/docs/Web/JavaScript/Reference/Operators/delete).

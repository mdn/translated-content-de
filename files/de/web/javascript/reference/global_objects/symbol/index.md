...
**`Symbol`** ist ein eingebautes Objekt, dessen Konstruktor ein `symbol` {{Glossary("Primitive", "Primitiv")}} zurückgibt — auch als **Symbolwert** oder einfach **Symbol** bezeichnet — das garantiert eindeutig ist. Symbole werden oft verwendet, um Objekten eindeutige Eigenschaftsschlüssel hinzuzufügen, die nicht mit Schlüsseln kollidieren, die anderer Code möglicherweise zu dem Objekt hinzufügt, und die von Mechanismen verborgen sind, die anderer Code typischerweise verwendet, um auf das Objekt zuzugreifen. Dies ermöglicht eine Form der schwachen {{Glossary("encapsulation", "Kapselung")}} oder eine schwache Form der [Informationsverbergung](https://en.wikipedia.org/wiki/Information_hiding).

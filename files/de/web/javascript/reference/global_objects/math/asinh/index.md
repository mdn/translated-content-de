...
Da `asinh()` eine statische Methode von `Math` ist, verwendet man sie immer als `Math.asinh()`, anstatt als Methode eines selbst erstellten `Math`-Objekts (`Math` ist kein Konstruktor).

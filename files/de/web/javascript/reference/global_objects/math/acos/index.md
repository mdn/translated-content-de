...
Da `acos()` eine statische Methode von `Math` ist, wird sie immer als `Math.acos()` verwendet, und nicht als Methode eines erstellten `Math`-Objekts (`Math` ist kein Konstruktor).

...
Da `cbrt()` eine statische Methode von `Math` ist, verwenden Sie sie immer als `Math.cbrt()`, nicht als Methode eines von Ihnen erstellten `Math`-Objekts (`Math` ist kein Konstruktor).

...
Die statische Methode **`Math.acosh()`** liefert den inversen hyperbolischen Kosinus einer Zahl. Das heißt,
...
Da `acosh()` eine statische Methode von `Math` ist, verwenden Sie sie immer als `Math.acosh()`, anstatt als Methode eines erstellten `Math`-Objekts (`Math` ist kein Konstruktor).

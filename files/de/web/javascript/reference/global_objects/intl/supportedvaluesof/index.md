...
Unten sind alle Werte aufgeführt, die gängigerweise von Browsern für den `collation` Schlüssel unterstützt werden. Diese Werte können für die `collation` Option oder den `co` [Unicode Erweiterungsschlüssel](/de-DE/docs/Web/JavaScript/Reference/Global_Objects/Intl#locales_argument) beim Erstellen von Objekten wie {{jsxref("Intl.Collator")}} verwendet werden.

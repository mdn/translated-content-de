...
Die **`findLast()`**-Methode von {{jsxref("Array")}}-Instanzen iteriert über das Array in umgekehrter Reihenfolge und gibt den Wert des ersten Elements zurück, das die bereitgestellte Testfunktion erfüllt. Wenn kein Element die Testfunktion erfüllt, wird {{jsxref("undefined")}} zurückgegeben.

Falls Sie Folgendes benötigen:

- das _erste_ passende Element zu finden, verwenden Sie {{jsxref("Array/find", "find()")}}.
- den _Index_ des letzten passenden Elements im Array zu finden, verwenden Sie {{jsxref("Array/findLastIndex", "findLastIndex()")}}.
- den _Index eines Wertes_ zu finden, verwenden Sie {{jsxref("Array/indexOf", "indexOf()")}}.
  (Es ist ähnlich wie {{jsxref("Array/findIndex", "findIndex()")}}, prüft aber jedes Element auf Gleichheit mit dem Wert statt mit einer Testfunktion.)
- um zu prüfen, ob ein Wert im Array _existiert_, verwenden Sie {{jsxref("Array/includes", "includes()")}}.
  Auch hier wird jedes Element auf Gleichheit mit dem Wert geprüft statt mit einer Testfunktion.
- falls ein beliebiges Element die bereitgestellte Testfunktion erfüllt, verwenden Sie {{jsxref("Array/some", "some()")}}.
...
Die `findLast()`-Methode ist [generisch](/de/docs/Web/JavaScript/Reference/Global_Objects/Array#generic_array_methods). Sie erwartet lediglich, dass der `this`-Wert eine `length`-Eigenschaft und integer-gekoppelte Eigenschaften hat.

## Beispiele

### Letztes Objekt in einem Array finden, das auf Element-Eigenschaften basiert
...
Das vorherige Beispiel könnte unter Verwendung einer Pfeilfunktion und [Objekt-Dekonstruktion](/de/docs/Web/JavaScript/Reference/Operators/Destructuring#object_destructuring) geschrieben werden:
...
### Verwenden von findLast() auf dünn besetzten Arrays

Leere Slots in dünn besetzten Arrays werden _besucht_ und genauso behandelt wie `undefined`.
...
Die `findLast()`-Methode liest die `length`-Eigenschaft von `this` und greift dann auf jede Eigenschaft zu, deren Schlüssel ein nicht-negativer Integer kleiner als `length` ist.

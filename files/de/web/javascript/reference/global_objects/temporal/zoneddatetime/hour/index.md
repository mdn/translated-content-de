...
Die **`hour`**-Accessor-Eigenschaft von {{jsxref("Temporal.ZonedDateTime")}}-Instanzen gibt eine ganze Zahl von 0 bis 23 zurück, die die Stundenkomponente dieser Zeit darstellt.
...
Für `ZonedDateTime` kann `hour` nicht kontinuierlich sein aufgrund von Offsets-Änderungen wie Zeitumstellungen zur Sommerzeit. In diesem Fall kann die Stunde wiederholt auftreten oder übersprungen werden.
...
Nicht-kontinuierliche Stunden sind sehr häufig aufgrund von Zeitumstellungen zur Sommerzeit, was näher erklärt wird in [Unsicherheiten und Lücken von lokaler Zeit zu UTC-Zeit](/de/docs/Web/JavaScript/Reference/Global_Objects/Temporal/ZonedDateTime#ambiguity_and_gaps_from_local_time_to_utc_time).
...
Aus diesem Grund sollten Sie immer {{jsxref("Temporal/ZonedDateTime/add", "add()")}} und {{jsxref("Temporal/ZonedDateTime/subtract", "subtract()")}} bevorzugen, um Daten und Zeiten zu manipulieren, anstatt direkt die `hour`-Eigenschaft zu ändern.

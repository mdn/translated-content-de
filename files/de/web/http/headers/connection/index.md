...
Der HTTP-Header **`Connection`** steuert, ob die Netzwerkverbindung nach Abschluss der aktuellen Transaktion offen bleibt. Wenn der gesendete Wert `keep-alive` ist, bleibt die Verbindung bestehen und wird nicht geschlossen, sodass nachfolgende Anfragen an denselben Server über dieselbe Verbindung möglich sind.

> [!WARNING]
> Verbindungsbezogene Headerfelder wie `Connection` und {{HTTPHeader("Keep-Alive")}} sind in [HTTP/2](https://httpwg.org/specs/rfc9113.html#ConnectionSpecific) und [HTTP/3](https://httpwg.org/specs/rfc9114.html#header-formatting) verboten. Chrome und Firefox ignorieren sie in HTTP/2-Antworten, aber Safari entspricht den Anforderungen der HTTP/2-Spezifikation und lädt keine Antwort, die sie enthält.

Alle [Hop-by-Hop-Header](/de/docs/Web/HTTP/Compression#hop-by-hop_compression), einschließlich der standardmäßigen Hop-by-Hop-Header ({{HTTPHeader("Keep-Alive")}}, {{HTTPHeader("Transfer-Encoding")}}, {{HTTPHeader("TE")}}, `Connection`, {{HTTPHeader("Trailer")}}, {{HTTPHeader("Upgrade")}}, {{HTTPHeader("Proxy-Authorization")}}, und {{HTTPHeader("Proxy-Authenticate")}}) müssen im `Connection`-Header aufgelistet werden, damit der erste Proxy weiß, dass er sie konsumieren muss und nicht weiterleitet.

Der Standardwert von `Connection` hat sich zwischen HTTP/1.0 und HTTP/1.1 geändert. Daher senden Browser zur Sicherstellung der Rückwärtskompatibilität oft explizit `Connection: keep-alive`, auch wenn es in HTTP/1.1 der Standard ist.

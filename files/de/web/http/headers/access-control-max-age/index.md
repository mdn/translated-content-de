...
Der **`Access-Control-Max-Age`** Antwortheader gibt an, wie lange die Ergebnisse einer {{Glossary("preflight_request", "Preflight-Anfrage")}} (also die in den Headern {{HTTPHeader("Access-Control-Allow-Methods")}} und {{HTTPHeader("Access-Control-Allow-Headers")}} enthaltenen Informationen) zwischengespeichert werden können.

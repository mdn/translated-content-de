...
Der **`Access-Control-Max-Age`** Antwort-Header gibt an, wie lange die Ergebnisse einer [Preflight-Anfrage](/de/docs/Glossary/preflight_request) (also die Informationen, die in den {{HTTPHeader("Access-Control-Allow-Methods")}} und {{HTTPHeader("Access-Control-Allow-Headers")}} Headern enthalten sind) zwischengespeichert werden können.
...
- \<delta-seconds>
  - : Maximale Anzahl von Sekunden, die die Ergebnisse zwischengespeichert werden können, als nicht-negativer Integer ohne Vorzeichen.

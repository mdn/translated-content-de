...
Der HTTP **`Sec-CH-UA-Full-Version`** {{Glossary("request_header", "Request-Header")}} ist ein [User-Agent-Client-Hinweis](/de/docs/Web/HTTP/Client_hints#user-agent_client_hints), der die vollständige Versionsnummer des User-Agents bereitstellt.
...
Ein Server fordert den `Sec-CH-UA-Full-Version`-Header an, indem er den {{HTTPHeader("Accept-CH")}} in eine _Antwort_ auf jede Anfrage vom Client einschließt und den Namen des gewünschten Headers als Token benutzt:
...
Beispielsweise könnte der Client den Header wie folgt hinzufügen:

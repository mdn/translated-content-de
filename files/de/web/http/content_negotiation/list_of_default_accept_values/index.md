...
Dieser Artikel dokumentiert die Standardwerte für den HTTP-Header [`Accept`](/de/docs/Web/HTTP/Headers/Accept) für spezifische Eingaben und Browser-Versionen.
...
\[1] Der Wert kann auf eine beliebige Zeichenkette gesetzt werden, indem die `network.http.accept`-Einstellung (`about:config`) verwendet wird.

\[2] Der Wert kann auf eine beliebige Zeichenkette gesetzt werden, indem die [`network.http.accept.default`](https://kb.mozillazine.org/Network.http.accept.default)-Einstellung (`about:config`) verwendet wird.

\[3] Dies ist eine Verbesserung gegenüber früheren `Accept`-Headern, da `image/png` nicht mehr über `text/html` eingeordnet wird.

## Werte für ein Bild

Wenn ein Bild angefordert wird, z. B. durch ein HTML-{{HTMLElement("img")}}-Element, setzt der Nutzer-Agent häufig eine spezifische Liste von akzeptierten Medientypen.
...
\[1] Der Wert kann auf eine beliebige Zeichenkette gesetzt werden, indem der `image.http.accept`-Parameter verwendet wird (_[source](https://searchfox.org/mozilla-central/search?q=image.http.accept)_).
...
Wenn eine Audiodatei angefordert wird, etwa über das {{HTMLElement("audio")}}-HTML-Element, verwenden die meisten Browser spezifische Werte.
...
Wenn ein Skript angefordert wird, etwa über das {{HTMLElement("script")}}-HTML-Element, verwenden einige Browser spezifische Werte.

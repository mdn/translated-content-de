---
title: Liste der Standard-Akzeptanzwerte
...
| User-Agent                | Wert                                                                                                                                |
| ------------------------- | ----------------------------------------------------------------------------------------------------------------------------------- |
| Firefox 128 und höher [1] | `text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/png,image/svg+xml,*/*;q=0.8`                     |
...
\[1] Der Wert kann auf eine beliebige Zeichenkette gesetzt werden, indem die Präferenz `network.http.accept` (`about:config`) verwendet wird.

\[2] Der Wert kann auf eine beliebige Zeichenkette gesetzt werden, indem die Präferenz [`network.http.accept.default`](https://kb.mozillazine.org/Network.http.accept.default) (`about:config`) verwendet wird.

\[3] Dies ist eine Verbesserung gegenüber früheren `Accept`-Headers, da `image/png` nicht mehr über `text/html` rangiert.

## Werte für ein Bild

Beim Anfordern eines Bildes, z. B. durch ein HTML-{{HTMLElement("img")}}-Element, setzt der User-Agent oft eine spezifische Liste von Medientypen, die akzeptiert werden sollen.

| User-Agent                    | Wert                                                                       |
| ----------------------------- | -------------------------------------------------------------------------- |
| Firefox 128 und höher [1]     | `image/avif,image/webp,image/png,image/svg+xml,image/*;q=0.8,*/*;q=0.5`    |
...
| Chrome und Edge 121 und höher | `image/avif,image/webp,image/apng,image/*,*/*;q=0.8`                       |

\[1] Der Wert kann auf eine beliebige Zeichenkette gesetzt werden, indem der Parameter `image.http.accept` verwendet wird (_[Quelle](https://searchfox.org/mozilla-central/search?q=image.http.accept)_).
...
| User-Agent             | Wert                                                                               |
| ---------------------- | ---------------------------------------------------------------------------------- |
| Firefox 3.6 und höher  | `video/webm,video/ogg,video/*;q=0.9,application/ogg;q=0.7,audio/*;q=0.6,*/*;q=0.5` |
| Firefox früher als 3.6 | _keine Unterstützung für {{HTMLElement("video")}}_                                 |
| Chrome                 | `*/*`                                                                              |

## Werte für Audio-Ressourcen

Wenn eine Audiodatei angefordert wird, z. B. über das {{HTMLElement("audio")}}-HTML-Element, verwenden die meisten Browser spezifische Werte.

| User-Agent                | Wert                                                                                         |
| ------------------------- | -------------------------------------------------------------------------------------------- |
| Firefox 3.6 und höher [1] | `audio/webm,audio/ogg,audio/wav,audio/*;q=0.9,application/ogg;q=0.7,video/*;q=0.6,*/*;q=0.5` |
| Safari, Chrome            | `*/*`                                                                                        |

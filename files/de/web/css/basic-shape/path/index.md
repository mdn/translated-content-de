...
- [`<fill-rule>`](/de/docs/Web/SVG/Reference/Attribute/fill-rule) {{optional_inline}}
  - : Definiert, welche Teile des Pfads innerhalb der Form liegen. Die möglichen Werte sind:
    - `nonzero`: Ein Punkt gilt als innerhalb der Form liegend, wenn ein Strahl, der von diesem Punkt ausgeht, mehr von links nach rechts als von rechts nach links durch Pfadsegmente verläuft, was zu einem nicht-null Ergebnis führt. Dies ist der Standardwert, wenn `<fill-rule>` weggelassen wird.

    - `evenodd`: Ein Punkt gilt als innerhalb der Form liegend, wenn ein Strahl, der von diesem Punkt ausgeht, eine ungerade Anzahl von Pfadsegmenten kreuzt. Das bedeutet, dass der Strahl für jedes Mal, wenn er in die Form eintritt, nicht ebenso oft austritt, womit eine ungerade Anzahl von Eintritten ohne entsprechende Austritte angezeigt wird.

    > [!WARNING]
    > `<fill-rule>` wird nicht in {{cssxref("offset-path")}} unterstützt und seine Verwendung macht die Eigenschaft ungültig.

- {{cssxref("string")}}
  - : Ein [Datenstring](/de/docs/Web/SVG/Reference/Attribute/d), der in Anführungszeichen eingeschlossen ist und einen [SVG path](/de/docs/Web/SVG/Reference/Element/path) definiert. Der SVG-Pfad-Datenstring enthält [Pfadbefehle](/de/docs/Web/SVG/Reference/Attribute/d#path_commands), die implizit Pixel-Einheiten verwenden. Ein leerer Pfad wird als ungültig angesehen.
...
Eine `path()` Funktion wurde im folgenden Beispiel als {{cssxref("offset-path")}} Wert bereitgestellt, um einen elliptischen Pfad zu erstellen, auf dem sich ein Ball bewegt.
...
Das `path()` kann verwendet werden, um den Wert des SVG [`d` Attributs](/de/docs/Web/SVG/Reference/Attribute/d) zu ändern, das auch in Ihrem CSS auf `none` gesetzt werden kann.

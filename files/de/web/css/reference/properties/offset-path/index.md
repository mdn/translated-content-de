...
- `none`
  - : Gibt an, dass das Element keinem Offset-Pfad folgt. Der Wert `none` ist gleichbedeutend damit, dass das Element keine [Offset-Transformation](/de/docs/Web/CSS/Reference/Properties/offset) hat. Die Bewegung des Elements wird in diesem Fall durch seine Standardpositions-Eigenschaften bestimmt, wie {{cssxref("top")}} und {{cssxref("left")}}, anstatt durch einen Offset-Pfad. Dies ist der Standardwert.
...
- `<offset-path>`
  - : Eine `ray()` Funktion, ein `<url>` Wert oder ein `<basic-shape>` Wert, der den geometrischen Offset-Pfad angibt. Wenn weggelassen, ist die Pfadform für den `<coord-box>` Wert `inset(0 round X)`, wobei `X` der Wert des {{cssxref("border-radius")}} des Elements ist, das den [Containing Block](/de/docs/Web/CSS/CSS_display/Containing_block) festlegt.
    - {{cssxref("ray", "ray()")}}
      - : Definiert eine Linie, die an einem festgelegten Punkt beginnt, eine festgelegte Länge hat und im angegebenen Winkel verläuft. Die `ray()` Funktion akzeptiert bis zu vier Parameter – einen {{CSSxRef("angle")}}, einen optionalen Größenwert, das optionale Schlüsselwort `contain` und ein optionales `at <position>`.
    ...
    - {{cssxref("basic-shape")}}
      - : Gibt den Offset-Pfad als den äquivalenten Pfad einer [CSS-Basisformenfunktion](/de/docs/Web/CSS/basic-shape) an, wie zum Beispiel {{cssxref("basic-shape/circle", "circle()")}}, {{cssxref("basic-shape/ellipse", "ellipse()")}}, {{cssxref("basic-shape/inset", "inset()")}}, {{cssxref("basic-shape/path", "path()")}}, {{cssxref("basic-shape/polygon", "polygon()")}}, {{cssxref("basic-shape/rect", "rect()")}}, oder {{cssxref("basic-shape/xywh", "xywh()")}}. Zum Beispiel, wenn das `<basic_shape>` eine `ellipse()` Funktion ist, dann ist der Pfad der Umriss der Ellipse, beginnend am rechten Punkt der Ellipse, im Uhrzeigersinn durch eine vollständige Drehung verlaufend. Für `ellipse()` und `circle()`, die den Parameter `at <position>` akzeptieren, falls das `<position>` weggelassen wird, wird die Position standardmäßig zu `center`, außer das Element hat eine {{cssxref("offset-position")}} angegeben. In diesem Fall wird der `offset-position` Wert für den Parameter `at <position>` verwendet. Komplexere Formen können mit der {{cssxref("basic-shape/shape", "shape()")}} Funktion definiert werden.

- [`<coord-box>`](/de/docs/Web/CSS/box-edge#values)
  - : Gibt die Größeninformation der [Referenzbox](/de/docs/Web/CSS/CSS_shapes/Basic_shapes#the_reference_box) an, die den Pfad enthält. Die Referenzbox wird aus dem Element abgeleitet, das den Containing Block für dieses Element festlegt. Dieser Parameter ist optional. Wenn nicht angegeben, ist der Standardwert `border-box` in CSS-Kontexten. In SVG-Kontexten wird der Wert als `view-box` behandelt. Wenn `ray()` oder `<basic-shape>` verwendet werden, um den Offset-Pfad zu definieren, liefert der `<coord-box>` Wert die Referenzbox für den Strahl oder die `<basic-shape>`, jeweils. Wenn `<url>` verwendet wird, um den Offset-Pfad zu definieren, liefert der `<coord-box>` Wert das Ansichtsfenster und das Benutzerskoordinatensystem für das Formelement, mit dem Ursprung (`0 0`) in der oberen linken Ecke und der Größe `1px`.

## Beschreibung

Die `offset-path` Eigenschaft definiert einen Pfad, dem ein animiertes Element folgen kann. Ein Offset-Pfad ist entweder ein spezifizierter Pfad mit einem oder mehreren Unterpfaden oder die Geometrie einer nicht-gestylten Basisform. Die genaue Position des Elements auf dem Offset-Pfad wird durch die Eigenschaft {{cssxref("offset-distance")}} bestimmt. Jede Form oder Pfad muss eine Anfangsposition für den berechneten Wert von `0` für {{cssxref("offset-distance")}} definieren und eine Anfangsrichtung, die die Drehung des Objekts zur Anfangsposition angibt.
...
In diesem Beispiel wurden Rand, Rahmen und Polsterung absichtlich groß gewählt, um die Platzierung der blauen, grünen und roten Rechtecke auf ihren jeweiligen `<coord-box>` Kanten zu zeigen: border-box, padding-box und content-box.

![Das blaue Rechteck sitzt am äußeren Rand der Border-Box, das grüne Rechteck an der inneren Border-Kante, die der äußeren Rand der Padding-Box ist, und das rote Rechteck am äußeren Rand der Content-Box.](offset-path-coord-box.png)
...
Ohne die `offset-path` Eigenschaft würden die beiden Hälften der Schere standardmäßig in der oberen linken Ecke der Zeichenfläche sein. Durch die Verwendung von `offset-path` sind die beiden Scherenhälften jedoch mit dem Startpunkt des SVG-Pfads ausgerichtet, sodass sie sich entlang des Pfads bewegen können.

{{EmbedLiveSample('offset_path_path', '100%', '450')}}

### Erstellen eines Offset-Pfads mit url()

Dieses Beispiel zeigt, wie auf eine SVG-Form verwiesen wird, um die Form des Pfads zu definieren, dem ein Element folgen kann. Der grüne Kreis (definiert durch `.target`) folgt dem Pfad eines Rechtecks, das definiert wird, indem die ID der SVG-Form (`svgRect`) an die `offset-path` Eigenschaft übergeben wird, indem `url()` verwendet wird.

...
Die `offset-path` Eigenschaft nimmt als Wert einen `<offset-path>` Wert, einen [`<coord-box>`](/de/docs/Web/CSS/Reference/Values/box-edge#values) Wert oder beides, sowie das Schlüsselwort `none` an. Der `<offset-path>` Wert ist eine {{cssxref("ray","ray()")}} Funktion, ein {{cssxref("url_value", "&lt;url&gt;")}} Wert oder ein [`<basic-shape>`](/de/docs/Web/CSS/Reference/Values/basic-shape) Wert.

- `none`
  - : Gibt an, dass das Element keinem Offset-Pfad folgt. Der `none` Wert entspricht dem Zustand, dass das Element keine [Offset-Transformation](/de/docs/Web/CSS/Reference/Properties/offset) hat. Die Bewegung des Elements wird in diesem Fall durch die Standardpositionseigenschaften wie {{cssxref("top")}} und {{cssxref("left")}} statt durch einen Offset-Pfad bestimmt. Dies ist der Standardwert.

- `<offset-path>`
  - : Eine `ray()` Funktion, ein `<url>` Wert oder ein `<basic-shape>` Wert, das den geometrischen Offset-Pfad angibt. Wird dies weggelassen, ist die Pfadform für den `<coord-box>` Wert `inset(0 round X)`, wobei `X` den Wert von {{cssxref("border-radius")}} des Elements darstellt, das den [Containing Block](/de/docs/Web/CSS/CSS_display/Containing_block) bildet.
    - {{cssxref("ray","ray()")}}
      - : Definiert eine Linie, die an einer festgelegten Position beginnt, eine bestimmte Länge hat und im angegebenen Winkel verläuft. Die `ray()` Funktion akzeptiert bis zu vier Parameter – einen {{CSSxRef("angle")}}, einen optionalen Größenwert, das optionale Schlüsselwort `contain` und ein optionales `at <position>`.
    ...
    - {{cssxref("basic-shape")}}
      - : Gibt den Offset-Pfad als den äquivalenten Pfad einer [CSS-Standardformfunktion](/de/docs/Web/CSS/Reference/Values/basic-shape) an, wie z.B. {{cssxref("basic-shape/circle","circle()")}}, {{cssxref("basic-shape/ellipse","ellipse()")}}, {{cssxref("basic-shape/inset","inset()")}}, {{cssxref("basic-shape/path","path()")}}, {{cssxref("basic-shape/polygon","polygon()")}}, {{cssxref("basic-shape/rect","rect()")}}, oder {{cssxref("basic-shape/xywh","xywh()")}}. Wenn zum Beispiel das `<basic_shape>` eine `ellipse()` Funktion ist, dann ist der Pfad der Umriss der Ellipse, beginnend am rechten Punkt der Ellipse, im Uhrzeigersinn durch eine volle Rotation. Für `ellipse()` und `circle()`, die den `at <position>` Parameter akzeptieren, wird, falls `<position>` weggelassen wird, die Position standardmäßig auf `center` gesetzt, es sei denn, das Element hat eine {{cssxref("offset-position")}} spezifiziert. In diesem Fall wird der `offset-position` Wert für den `at <position>` Parameter verwendet. Komplexere Formen können mit der {{cssxref("basic-shape/shape","shape()")}} Funktion definiert werden.

- [`<coord-box>`](/de/docs/Web/CSS/Reference/Values/box-edge#values)
  - : Gibt die Größeninformationen der [Referenzbox](/de/docs/Web/CSS/CSS_shapes/Basic_shapes#the_reference_box) an, die den Pfad enthält. Die Referenzbox wird von dem Element abgeleitet, das den Containing Block für dieses Element bildet. Dieser Parameter ist optional. Falls nicht angegeben, beträgt der Standardwert `border-box` in CSS-Kontexten. In SVG-Kontexten wird der Wert als `view-box` behandelt. Wird `ray()` oder `<basic-shape>` zur Definition des Offset-Pfads verwendet, liefert der `<coord-box>` Wert die Referenzbox für den Strahl oder die `<basic-shape>`. Wenn `<url>` zur Definition des Offset-Pfads verwendet wird, liefert der `<coord-box>` Wert das Ansichtsfenster und das Benutzerkoordinatensystem für das Formelement, wobei der Ursprung (`0 0`) in der oberen linken Ecke liegt und die Größe `1px` beträgt.

## Beschreibung

Die `offset-path` Eigenschaft definiert einen Pfad, dem ein animiertes Element folgen kann. Ein Offset-Pfad ist entweder ein festgelegter Pfad mit einem oder mehreren Unterpfaden oder die Geometrie einer nicht gestylten Standardform. Die genaue Position des Elements auf dem Offset-Pfad wird durch die {{cssxref("offset-distance")}} Eigenschaft bestimmt. Jede Form oder jeder Pfad muss eine Anfangsposition für den berechneten Wert von `0` für {{cssxref("offset-distance")}} und eine Anfangsrichtung definieren, die die Drehung des Objekts auf die Anfangsposition festlegt.

Frühe Versionen der Spezifikation nannten diese Eigenschaft `motion-path`. Sie wurde zu `offset-path` geändert, da die Eigenschaft statische Positionen beschreibt, nicht die Bewegung.
...
### Erstellung eines Offset-Pfads mit Box-Kanten-Positionierung
...
In diesem Beispiel wurden Rand, Rahmen und Auffüllung absichtlich mit großen Werten versehen, um die Platzierung der blauen, grünen und roten Rechtecke auf ihren jeweiligen `<coord-box>` Kanten zu demonstrieren: border-box, padding-box und content-box.

![Das blaue Rechteck befindet sich am äußeren Rand der Border-Box, das grüne Rechteck befindet sich am inneren Rand der Border-Box, der gleichzeitig der äußere Rand der Padding-Box ist, und das rote Rechteck liegt am äußeren Rand der Content-Box.](offset-path-coord-box.png)

#### Ergebnis

{{EmbedLiveSample('Create an offset-path using box-edge positioning', '100%', 400)}}

### Erstellung eines Offset-Pfads mit path()

In diesem Beispiel erstellt das {{svgelement("svg")}} Element ein Haus mit Schornstein und definiert auch zwei Hälften einer Schere. Das Haus und der Schornstein bestehen aus Rechtecken und Polygonen, und die Scherenhälften werden durch zwei verschiedene Pfadelemente dargestellt. Im CSS-Code wird die `offset-path` Eigenschaft verwendet, um einen Pfad für die beiden Scherenhälften festzulegen. Dieser im CSS definierte Pfad ist identisch mit dem von dem `<path>` Element im SVG dargestellten Pfad, der den Umriss des Hauses einschließlich des Schornsteins beschreibt.
...
Ohne die `offset-path` Eigenschaft würden die beiden Hälften der Schere standardmäßig in der oberen linken Ecke der Zeichenfläche sein. Durch die Verwendung von `offset-path` sind die beiden Scherenhälften jedoch mit dem Ausgangspunkt des SVG-Pfads ausgerichtet und können sich entlang dieses Pfads bewegen.

{{EmbedLiveSample('offset_path_path', '100%', '450')}}

### Erstellung eines Offset-Pfads mit url()

Dieses Beispiel zeigt, wie man auf eine SVG-Form verweisen kann, um die Form des Pfads zu definieren, dem ein Element folgen kann. Der grüne Kreis (definiert durch `.target`) folgt dem Pfad eines Rechtecks, das durch das Übergeben der ID des SVG-Formelements (`svgRect`) an die `offset-path` Eigenschaft durch `url()` definiert wird.

...
Die [CSS](/de/docs/Web/CSS)-Eigenschaft **`column-wrap`** legt das Umbruchverhalten von Überlaufspalten in einem [CSS-Mehrspaltenlayout](/de/docs/Web/CSS/Guides/Multicol_layout) fest.
...
- `auto`
  - : Der Initialwert. Wenn {{cssxref("column-height")}} des Inhaltscontainers auf einen {{cssxref("&lt;length>")}} gesetzt ist, wird `auto` zu `wrap` aufgelöst, andernfalls zu `nowrap`.
- `nowrap`
  - : Spalten laufen in Inline-Richtung über.
- `wrap`
  - : Überlaufspalten werden in Blockrichtung in einer neuen Zeile platziert.

## Beschreibung

Die Eigenschaft `column-wrap` kann verwendet werden, um die Spalten eines [CSS-Mehrspaltenlayouts](/de/docs/Web/CSS/Guides/Multicol_layout) in eine neue Zeile umbrechen zu lassen, wenn sie beginnen, die Spaltenbreite zu überschreiten. Dies ist nützlich, um besser lesbare Layouts zu erstellen, wenn die Eigenschaft {{cssxref("column-count")}} oder {{cssxref("column-width")}} verwendet wird, um mehrere Spalten festzulegen.

Ohne `column-wrap` laufen überschüssige Spalten seitlich über, und Lesende müssen in Inline-Richtung scrollen, um den gesamten Inhalt zu lesen. Die Eigenschaft {{cssxref("column-height")}} ermöglicht es Ihnen zusammen mit `column-wrap`, eine bestimmte Höhe für die Spalten festzulegen und sie in eine neue Spaltenzeile umbrechen zu lassen, wenn der Containerrand erreicht wird.

Der Standardwert von `column-wrap` ist `auto`, was zu `wrap` aufgelöst wird, wenn `column-height` auf einen `<length>`-Wert gesetzt ist; `wrap` ermöglicht es Spalten mit fester Höhe, in mehrere Zeilen umzubrechen. Wenn `column-height` gleich `auto` ist, wird `column-wrap: auto` zu `nowrap` aufgelöst, wodurch die Spalten horizontal überlaufen können, wenn eine feste Containerhöhe festgelegt ist.
...
Dieses Beispiel zeigt die grundlegende Verwendung der Eigenschaft `column-wrap`, um durch Festlegen einer Eigenschaft `column-height` ein umgebrochenes Mehrspaltenlayout zu erstellen.

#### HTML

Wir fügen ein Gedicht von Dr. Seuss mithilfe eines {{htmlelement("ol")}} mit 28 {{htmlelement("li")}}s ein, gefolgt vom Namen des Autors in einem {{htmlelement("p")}}.
...
Dieses Beispiel enthält ein Mehrspaltenlayout, das den Unterschied zwischen den Werten `wrap` und `nowrap` zeigt, indem Sie den Wert `column-wrap` des Spaltencontainers zwischen beiden umschalten können. Das Ergebnis ist ein Layout, das dynamisch zwischen horizontalem und vertikalem Scrollen wechselt.

#### HTML und JavaScript

Das Markup für dieses Beispiel enthält mehrere Inhaltsabsätze, die den Startseiten von MDN für HTML, CSS und JavaScript entnommen wurden, sowie ein durch JavaScript gesteuertes Element [`<input type="checkbox">`](/de/docs/Web/HTML/Reference/Elements/input/checkbox), um den Eigenschaftswert `column-wrap` des Containers zwischen `nowrap` und `wrap` umzuschalten. Das HTML und JavaScript wurden der Kürze halber ausgeblendet.
...
Wir machen das Element {{htmlelement("body")}} zu einem Mehrspaltencontainer, indem wir {{cssxref("column-count")}} auf `3` setzen. Anschließend setzen wir ein {{cssxref("gap")}} von `3em 2em`, was zu einem Abstand von `3em` zwischen Zeilen und von `2em` zwischen Spalten führt.

Anschließend setzen wir `column-height` auf `90vh`, wodurch die Spalten fast so hoch wie der Viewport werden. Außerdem setzen wir `column-wrap` auf `nowrap`, wodurch überschüssige Inhaltsspalten horizontal überlaufen. Dies ist erforderlich, weil der Initialwert von `column-wrap` `auto` ist, der zu `wrap` aufgelöst wird, wenn `column-height` auf einen `<length>`-Wert gesetzt ist.

Das Kontrollkästchen schaltet die Eigenschaft `column-wrap` zwischen `nowrap` und `wrap` um. Wenn sie auf `wrap` gesetzt ist, laufen die überschüssigen Inhaltsspalten vertikal in neue Spaltenzeilen über und erzeugen so das vertikale Layout. Der Wert `column-height` bewirkt, dass jede Spaltenzeile den Viewport ausfüllt.
...
Als Nächstes setzen wir die Eigenschaft {{cssxref("column-span")}} des Elements [`<h1>`](/de/docs/Web/HTML/Reference/Elements/Heading_Elements) auf `all`, damit die Überschrift alle Spalten überspannt. Außerdem setzen wir die Eigenschaft {{cssxref("margin-top")}} des ersten {{htmlelement("p")}} auf `0`, damit sie mit dem oberen Rand der Spalten ausgerichtet ist.
...
Schalten Sie das Kontrollkästchen um, um den Wert der Eigenschaft `column-wrap` zu ändern und das Layout zwischen horizontalem und vertikalem Scrollen umzuschalten. Wenn `column-wrap` auf `nowrap` gesetzt ist, laufen die Spalten horizontal über; wenn `column-wrap` auf `wrap` gesetzt ist, werden vertikal neue Spaltenzeilen hinzugefügt.

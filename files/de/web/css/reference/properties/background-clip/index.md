...
Die **`background-clip`**-Eigenschaft von [CSS](/de/docs/Web/CSS) legt fest, ob der Hintergrund eines Elements sich unterhalb seiner Rahmen-Box, Padding-Box oder Inhalts-Box erstreckt.
...
- `border-area`
  - : Der Hintergrund wird innerhalb des vom Rahmen bemalten Bereichs gemalt (darauf beschränkt), unter Berücksichtigung von {{Cssxref("border-width")}} und {{Cssxref("border-style")}}, jedoch ohne Berücksichtigung von Transparenz, die durch {{Cssxref("border-color")}} eingeführt wird.
...
Erwägen Sie die Verwendung von Feature-Queries mit {{cssxref("@supports")}} um zu testen, ob `background-clip: text` unterstützt wird, und bieten Sie eine barrierefreie Alternative an, falls dies nicht der Fall ist.
...
- Die {{cssxref("clip-path")}}-Eigenschaft erstellt eine Ausschnittsregion, die definiert, welcher Teil eines _gesamten Elements_ angezeigt werden soll.

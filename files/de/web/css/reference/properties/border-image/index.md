...
Die **`border-image`** [CSS](/de/docs/Web/CSS) Eigenschaft zeichnet ein Bild um ein gegebenes Element. Sie ersetzt den regulären [Border](/de/docs/Web/CSS/Reference/Properties/border) des Elements.

> [!NOTE]
> Sie sollten einen separaten {{cssxref("border-style")}} angeben, falls das Border-Bild nicht geladen werden kann. Obwohl die Spezifikation es nicht strikt erfordert, rendern einige Browser das Border-Bild nicht, wenn {{cssxref("border-style")}} `none` oder {{cssxref("border-width")}} `0` ist.
...
- `<'border-image-source'>`
  - : Das Ausgangsbild. Siehe {{cssxref("border-image-source")}}.
- `<'border-image-slice'>`
  - : Die Maße zum Zerteilen des Ausgangsbildes in Regionen. Es können bis zu vier Werte angegeben werden. Siehe {{cssxref("border-image-slice")}}.
- `<'border-image-width'>`
...
- `<'border-image-repeat'>`
  - : Definiert, wie die Kantenregionen des Ausgangsbildes angepasst werden, um die Dimensionen des Border-Bildes zu entsprechen. Es können bis zu zwei Werte angegeben werden. Siehe {{cssxref("border-image-repeat")}}.
...
![Acht Rauten: vier rote Rauten, eine in jedem Eck, und vier orange Rauten, eine auf jeder Seite. Die Mitte ist leer.](border.png)
...
{{cssxref("border-radius")}} hat keinen Einfluss auf das Border-Bild. Dies liegt daran, dass {{cssxref("border-image-outset")}} das Bild außerhalb der Borderbox platzieren kann, sodass es keinen Sinn macht, dass das Border-Bild durch den Randbereich geklippt wird. Um runde Ränder zu erstellen, wenn ein Border-Bild verwendet wird, sollten Sie das Bild selbst mit abgerundeten Ecken erstellen oder, im Falle eines Verlaufs, es stattdessen als Hintergrund zeichnen. Unten zeigen wir einen Ansatz hierfür, indem zwei {{cssxref("background-image")}}s verwendet werden: eines, das sich auf die Borderbox erstreckt, und eines für die Paddingbox.
...
- [Border-Bilder in CSS: Ein zentrales Fokusgebiet für Interop 2023](/en-US/blog/border-images-interop-2023/) auf dem MDN-Blog (2023)

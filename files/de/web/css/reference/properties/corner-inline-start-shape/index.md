...
Die **`corner-inline-start-shape`** [CSS](/de/docs/Web/CSS) Eigenschaft spezifiziert die Form beider Ecken an der inline-start Kante eines Kastens, innerhalb ihres {{cssxref("border-radius")}} Bereiches.
...
Wir geben dem Kasten einige grundlegende Stile, die wir der Kürze halber verborgen haben. Außerdem wenden wir einen {{cssxref("box-shadow")}}, einen `border-radius` von 60 Pixeln und ein `corner-inline-start-shape` von `square bevel` an.

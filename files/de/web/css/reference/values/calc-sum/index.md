...
Der **`<calc-sum>`** [CSS](/de/docs/Web/CSS) [Datentyp](/de/docs/Web/CSS/Reference/Values/Data_types) repräsentiert einen Ausdruck, der eine Berechnung in einer [CSS-Mathematischen Funktion](/de/docs/Web/CSS/Reference/Values/Functions#math_functions) durchführt. Der Ausdruck führt eine grundlegende arithmetische Operation der Addition und Subtraktion zwischen zwei Werten aus.
...
Die Typen der beiden Operanden müssen konsistent sein. Bei Längen kann `0` nicht als `0px` (oder eine andere Längeneinheit) verwendet werden. Stattdessen müssen Sie eine explizite Einheit hinzufügen: `margin-top: calc(0px + 20px);` ist gültig, während `margin-top: calc(0 + 20px);` ungültig ist. Prozentwerttypen werden basierend auf dem Kontext aufgelöst. Zum Beispiel ist `margin-top: calc(50% + 20px);` gültig, weil `margin-top` Prozentsätze zu Längen auflöst.

Das Einbeziehen von [CSS-Variablen](/de/docs/Web/CSS/Guides/Cascading_variables) in `calc-sum` Ausdrücken ist ebenfalls erlaubt. Der folgende Code `calc(10px + var(--variable))`, ist ein gültiger Ausdruck.

...
Der **`<color>`** [CSS](/de/docs/Web/CSS) [Datentyp](/de/docs/Web/CSS/Reference/Values/Data_types) repräsentiert eine Farbe. Ein `<color>` kann auch einen [Alpha-Kanal](https://en.wikipedia.org/wiki/Alpha_compositing) _Transparenzwert_ enthalten, der angibt, wie die Farbe mit ihrem Hintergrund [komponiert](https://drafts.csswg.org/compositing-1/#simplealphacompositing) werden soll.

> [!NOTE]
> Obwohl `<color>`-Werte genau definiert sind, kann ihr tatsächliches Erscheinungsbild von Gerät zu Gerät variieren (manchmal erheblich). Dies liegt daran, dass die meisten Geräte nicht kalibriert sind und einige Browser die [Farbprofile](https://en.wikipedia.org/wiki/ICC_profile) der Ausgabegeräte nicht unterstützen.
...
- Durch Verwendung der Syntax für [relative Farben](/de/docs/Web/CSS/Guides/Colors/Using_relative_colors), um eine neue Farbe basierend auf einer bestehenden Farbe auszugeben. Jede der oben genannten Farbmethoden kann eine **Ursprungsfarbe** enthalten, die durch das Schlüsselwort `from` eingeleitet und von Definitionen der Kanalwerte für die neue **Ausgabefarbe** gefolgt wird.
...
Jede Komponente einer CSS-Farbmethode - mit Ausnahme derjenigen, die die veraltete kommaseparierte Syntax verwenden - kann als das Schlüsselwort `none` angegeben werden, um eine fehlende Komponente darzustellen.

Das explizite Angeben [fehlender Komponenten in der Farbsinterpolation](#interpolation_mit_fehlenden_komponenten) ist nützlich, wenn Sie einige Farbkomponenten interpolieren, andere jedoch nicht interpolieren möchten. Für alle anderen Zwecke hat eine fehlende Komponente effektiv einen Nullwert in einer geeigneten Einheit: `0`, `0%` oder `0deg`. Zum Beispiel sind die folgenden Farben äquivalent, wenn sie außerhalb der Interpolation verwendet werden:
...
Bei der Interpolation von `<color>`-Werten werden diese zuerst in einen gegebenen Farbraum konvertiert und dann werden die einzelnen Komponenten der [berechneten Werte](/de/docs/Web/CSS/Guides/Cascade/Property_value_processing#computed_value) linear interpoliert. Die Geschwindigkeit der Interpolation wird von der [Beschleunigungsfunktion](/de/docs/Web/CSS/Reference/Values/easing-function) in Übergängen und Animationen bestimmt. Der Standardfarbraum für die Interpolation ist Oklab, kann jedoch durch {{CSSXref("&lt;color-interpolation-method&gt;")}} in einigen farbbezogenen funktionalen Notationen überschrieben werden.

### Interpolation mit fehlenden Komponenten

#### Interpolation von Farben im selben Raum

Wenn Farben interpoliert werden, die sich genau im Interpolationsfarbraum befinden, werden fehlende Komponenten von einer Farbe durch bestehende Werte derselben Komponenten der anderen Farbe ersetzt. Zum Beispiel sind die folgenden zwei Ausdrücke äquivalent:
...
Wenn eine Farbe, die interpoliert werden soll, nicht im Interpolationsfarbraum liegt, werden ihre fehlenden Komponenten in die konvertierte Farbe auf Basis der **analogen Komponenten** derselben Kategorie übertragen, wie in der folgenden Tabelle beschrieben:
...
### Erzeugung von vollständig gesättigten sRGB-Farben

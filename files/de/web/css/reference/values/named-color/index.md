...
Der **`<named-color>`** [CSS](/de/docs/Web/CSS) [Datentyp](/de/docs/Web/CSS/Reference/Values/Data_types) ist der Name einer Farbe, wie zum Beispiel `red`, `blue`, `black`, oder `lightseagreen`. Syntaktisch gesehen ist ein `<named-color>` ein [`<ident>`](/de/docs/Web/CSS/Reference/Values/ident).
...
Neben diesen 16 Farben gibt es noch etwa 150 weitere Farben, die mit einem Schlüsselwort verbunden sind:
...
Anfänglich wurden in [CSS Level 1](https://www.w3.org/TR/CSS1/#color-units) nur 16 Grundfarben definiert, wobei `orange` in [CSS Level 2](https://www.w3.org/TR/CSS2/syndata.html#value-def-color) hinzukam. Webdesigner fanden diese Liste zu kurz, und Browserhersteller fügten zahlreiche Namen für Farben basierend auf den X11-Farbnamen hinzu. In [SVG 1](https://www.w3.org/TR/SVG11/types.html#DataTypeColor) und dann in [CSS Colors Level 3](https://drafts.csswg.org/css-color-3/#svg-color) wurden diese Namen standardisiert, formal definiert und vereinheitlicht (einige hatten unterschiedliche Schreibweisen, die nun Aliase sind). Sie werden _erweiterte Farb-Schlüsselwörter_, _X11-Farben_ oder _SVG-Farben_ genannt.

In [CSS Colors Level 4](https://drafts.csswg.org/css-color-4/#named-colors) wurde eine weitere Farbe, `rebeccapurple`, hinzugefügt, um [Web-Pionier Eric Meyer](https://meyerweb.com/eric/thoughts/2014/06/19/rebeccapurple/) zu ehren.
...
Um unerwartetes Verhalten, wie z.B. in einem {{cssxref("gradient")}}, zu verhindern, gibt die aktuelle CSS-Spezifikation an, dass `transparent` im [alpha-vorberechneten Farbraum](https://drafts.csswg.org/css-color/#interpolation-alpha) berechnet werden sollte. Beachten Sie jedoch, dass ältere Browser es möglicherweise als Schwarz mit einem Alphawert von `0` behandeln.
...
Alle Namen spezifizieren eine Farbe im [sRGB-Farbraum](https://de.wikipedia.org/wiki/SRGB). Obwohl die Namen mehr oder weniger ihre jeweiligen Farben beschreiben, sind sie im Wesentlichen künstlich, ohne eine strikte Begründung hinter den verwendeten Begriffen.
...
Obwohl viele Schlüsselwörter aus [X11](https://de.wikipedia.org/wiki/X_Window_System) angepasst wurden, können sich ihre RGB-Werte von der entsprechenden Farbe auf X11-Systemen unterscheiden, da Hersteller manchmal X11-Farben an ihre spezifische Hardware anpassen.
...
- [`<color>`](/de/docs/Web/CSS/Reference/Values/color_value): der Datentyp, dessen Definition Teil von `<named-color>` ist.

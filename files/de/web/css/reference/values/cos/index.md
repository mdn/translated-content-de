...
Die **`cos()`**-Funktion in [CSS](/de/docs/Web/CSS) ist eine trigonometrische Funktion, die den Kosinus einer Zahl zurückgibt, welcher ein Wert zwischen `-1` und `1` ist. Die Funktion enthält eine einzelne Berechnung, die entweder auf eine {{cssxref("&lt;number&gt;")}} oder einen {{cssxref("&lt;angle&gt;")}} aufgelöst werden muss, indem das Ergebnis des Arguments als Radiant interpretiert wird. Das heißt, `cos(45deg)`, `cos(0.125turn)` und `cos(3.14159 / 4)` repräsentieren alle den gleichen Wert, ungefähr `0.707`.
...
- `angle`
  - : Eine Berechnung, die sich auf eine {{cssxref("&lt;number&gt;")}} oder einen {{cssxref("&lt;angle&gt;")}} auflöst. Bei der Angabe von einheitenlosen Zahlen werden diese als Radianten interpretiert, die einen {{cssxref("&lt;angle&gt;")}} darstellen.
...
Wenn das Element mit {{cssxref("transform-function/rotate", "rotate()")}} gedreht wird, geht es über seine ursprüngliche Größe hinaus. Um dies zu korrigieren, verwenden wir `cos()`, um die Elementgröße anzupassen.

...
Der CSS **Attributselektor** wählt Elemente basierend darauf aus, dass das Element ein spezifisch gesetztes Attribut hat, mit Optionen zum Definieren eines Attributwertes oder Teilstringwertes.
...
- `[attr|=value]`
  - : Repräsentiert Elemente mit einem Attributnamen _attr_, dessen Wert genau _value_ sein kann oder mit _value_ beginnt, gefolgt von einem Bindestrich `-` (U+002D). Es wird häufig für Sprachsubcodes verwendet.
- `[attr^=value]`
  - : Repräsentiert Elemente mit einem Attributnamen _attr_, dessen Wert vorangestellt (präfixiert) mit _value_ ist.
- `[attr$=value]`
  - : Repräsentiert Elemente mit einem Attributnamen _attr_, dessen Wert nachgestellt (suffigiert) mit _value_ ist.
- `[attr*=value]`
...
- `<attr>`
  - : Ein {{cssxref("ident")}}, das heißt der unquoted Name des Attributs. Dies kann jedes gültige sprachspezifische Attribut (SVG, HTML, XML, etc.), ein [`data-*` Attribut](/de/docs/Web/HTML/Global_attributes/data-*) oder ein vom Autor erstelltes Attribut sein.
- `<value>`
...
- `s` oder `i`
  - : Groß-/Kleinschrift-Sensitivitäts- oder -Unsensitivitäts-Flag. Wenn vor der schließenden Klammer (`]`) enthalten, wird der Wert unabhängig von Groß-/Kleinschreibung sensibel oder unsensibel gemacht, unabhängig von der Markup-Sprache.
...
Die HTML-Spezifikation erfordert, dass das [`type`](/de/docs/Web/HTML/Element/input#type) Attribut unabhängig von Groß-/Kleinschreibung abgeglichen wird, da es hauptsächlich im {{HTMLElement("input")}} Element verwendet wird.
Beachten Sie, dass, wenn ein Modifikator von der Benutzerumgebung nicht unterstützt wird, der Selektor nicht übereinstimmt.
...
- [Case-insensitive attribute selector values](https://html.spec.whatwg.org/multipage/semantics-other.html#case-sensitivity-of-selectors) bei WHATWG

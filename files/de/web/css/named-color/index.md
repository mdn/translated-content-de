...
Benannte Farben bestehen aus Standardfarben, den Stichwörtern [`transparent`](#transparent) und [`currentColor`](/de/docs/Web/CSS/color_value#currentcolor_keyword).
...
Zusätzlich zu diesen 16 Farben gibt es etwa 150 weitere Farben, die ein zugeordnetes Stichwort haben:
...
Anfangs wurden in [CSS Level 1](https://www.w3.org/TR/CSS1/#color-units) nur 16 Grundfarben definiert, mit `orange`, das in [CSS Level 2](https://www.w3.org/TR/CSS2/syndata.html#value-def-color) hinzugefügt wurde. Webdesigner fanden diese Liste zu kurz, und Browseranbieter fügten zahlreiche Namen für Farben basierend auf den X11-Farbnamen hinzu. In [SVG 1](https://www.w3.org/TR/SVG11/types.html#DataTypeColor) und dann in [CSS Colors Level 3](https://drafts.csswg.org/css-color-3/#svg-color) wurden diese Namen standardisiert, formal definiert und vereinheitlicht (einige hatten unterschiedliche Schreibweisen, die jetzt Aliase sind). Sie werden _erweiterte Farbstichwörter_, _X11-Farben_ oder _SVG-Farben_ genannt.

In [CSS Colors Level 4](https://drafts.csswg.org/css-color-4/#named-colors) wurde zur Ehre von [Webpionier Eric Meyer](https://meyerweb.com/eric/thoughts/2014/06/19/rebeccapurple/) eine zusätzliche Farbe, `rebeccapurple`, hinzugefügt.

### transparent

Das Stichwort `transparent` steht für eine komplett transparente Farbe. Dadurch wird der Hintergrund hinter dem farbigen Element vollständig sichtbar. Technisch ist `transparent` eine Abkürzung für `rgb(0 0 0 / 0%)`.

Um unerwartetes Verhalten zu vermeiden, wie beispielsweise in einem {{cssxref("gradient")}}, gibt die aktuelle CSS-Spezifikation an, dass `transparent` im [alpha-prämultiplierten Farbraum](https://drafts.csswg.org/css-color/#interpolation-alpha) berechnet werden sollte. Allerdings sollten Sie sich bewusst sein, dass ältere Browser es möglicherweise als Schwarz mit einem Alphawert von `0` behandeln.

Das Stichwort `transparent` war in CSS Level 2 (Revision 1) keine echte Farbe. Es war ein spezielles Stichwort, das anstelle eines regulären `<color>`-Wertes in zwei CSS-Eigenschaften verwendet werden konnte: {{Cssxref("background")}} und {{Cssxref("border")}}. Es wurde im Wesentlichen hinzugefügt, um Entwicklern die Möglichkeit zu geben, eine geerbte Volltonfarbe zu überschreiben. Mit dem Aufkommen von Alphakanälen in CSS Colors Level 3 wurde `transparent` als echte Farbe neu definiert. Es kann jetzt überall verwendet werden, wo ein `<color>`-Wert verwendet werden kann.

## Beschreibung

Alle Namen geben eine Farbe im [sRGB-Farbraum](https://en.wikipedia.org/wiki/SRGB) an. Obwohl die Namen ihre jeweiligen Farben mehr oder weniger beschreiben, sind sie im Wesentlichen künstlich und ohne eine strenge Begründung für die verwendeten Begriffe.

Die Farb-Stichwörter repräsentieren alle einfache, solide Farben ohne Transparenz.

Mehrere Stichwörter sind einander Synonyme:
...
Obwohl viele Stichwörter aus [X11](https://en.wikipedia.org/wiki/X_Window_System) angepasst wurden, können sich ihre RGB-Werte von der entsprechenden Farbe in X11-Systemen unterscheiden, da Hersteller die X11-Farben manchmal an ihre spezifische Hardware anpassen.
...
- [`<color>`](/de/docs/Web/CSS/color_value): der Datentyp, dessen Definition `<named-color>` ein konstituierender Teil ist.

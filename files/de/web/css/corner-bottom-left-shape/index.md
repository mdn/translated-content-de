...
Für eine vollständige Beschreibung des Eckenformverhaltens und mehrere Beispiele siehe die Seite der Kurzschreibweiseigenschaft {{cssxref("corner-shape")}}.
...
Die Eigenschaft `corner-bottom-left-shape` wird mit einem {{cssxref("&lt;corner-shape-value>")}} Wert spezifiziert.
...
Wir geben dem Kasten einige grundlegende Stile, die wir aus Gründen der Kürze versteckt haben. Wir wenden auch einen {{cssxref("box-shadow")}}, einen `border-radius` von `80px 40% 60px 20%` und eine `corner-bottom-left-shape` von `squircle` an.

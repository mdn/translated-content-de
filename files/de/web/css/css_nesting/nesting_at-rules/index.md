...
Jede [At-Regel](/de/docs/Web/CSS/At-rule), deren Körper Stilregeln enthält, kann mit CSS-Verschachtelung innerhalb einer anderen Stilregel verschachtelt werden. Verschachtelte Stilregeln innerhalb von At-Regeln übernehmen ihre Verschachtelungsselektor-Definition von der nächstgelegenen übergeordneten Stilregel. Eigenschaften können direkt innerhalb einer verschachtelten At-Regel enthalten sein, wobei sie so wirken, als wären sie in einem `& {...}` Block verschachtelt.
...
### Verschachtelung der `@media` At-Regel
...
[Kaskadenschichten](/de/docs/Web/CSS/@layer) können verschachtelt werden, um [Kinderschichten zu erstellen](/de/docs/Web/CSS/@layer#nesting_layers). Diese werden mit einem `.` (Punkt) verbunden.

#### Definition der Eltern- und Kinderschichten
...
Hier weist der `.foo` Selektor seine Regeln der **Basis-** `@layer` zu. Die verschachtelte **support** `@layer` erstellt die `base.support` Unterebene, und der `&` Verschachtelungsselektor wird verwendet, um die Regeln für den `.foo .bar` Selektor zu erstellen.
...
#### Gleichwertiges ohne Verschachtelung
...
- [Verschachtelung von Container-Anfragen](/de/docs/Web/CSS/CSS_containment/Container_size_and_style_queries#nested_queries)

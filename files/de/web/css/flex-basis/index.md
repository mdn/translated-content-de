...
Die **`flex-basis`**-Eigenschaft von [CSS](/de/docs/Web/CSS) legt die anfängliche Hauptgröße eines {{Glossary("flex_item", "Flex-Elements")}} fest. Sie bestimmt die Größe des Inhaltsbereichs, es sei denn, sie wird mit {{Cssxref("box-sizing")}} anders festgelegt.

> [!NOTE]
> Es wird empfohlen, die {{cssxref("flex")}}-Kurzform mit einem Schlüsselwortwert wie `auto` oder `initial` zu verwenden, anstatt `flex-basis` alleine zu setzen. Die [Schlüsselwortwerte](/de/docs/Web/CSS/flex#values) expandieren zu zuverlässigen Kombinationen von {{cssxref("flex-grow")}}, {{cssxref("flex-shrink")}} und `flex-basis`, die helfen, die häufig gewünschten Flex-Verhaltensweisen zu erreichen.
...
Die Demo ändert den `flex-basis`-Wert, der auf das erste Flex-Element gesetzt ist, was dazu führt, dass es wächst oder schrumpft, um den verfügbaren Platz auszufüllen. Die anderen Flex-Elemente werden auch ihre Größe ändern; sie werden mindestens `min-content`-groß sein. Zum Beispiel, wenn die `flex-basis` des ersten Elements auf `200px` gesetzt ist, wird es bei `200px` beginnen, aber dann schrumpfen, um in den verfügbaren Raum zu passen.

Wenn `flex-basis` auf einen Wert außer `auto` gesetzt ist und eine `width` (oder `height` im Fall von `flex-direction: column`) für dasselbe Flex-Element gesetzt ist, hat der `flex-basis`-Wert Vorrang.
...
- `<'width'>`
  - : Jede der folgenden Einheiten:
    ...
    - {{cssxref("&lt;percentage&gt;")}} setzt einen Prozentsatz der Breite oder Höhe des Inhaltsbereichs des umgebenden Blocks. Prozentwerte von `flex-basis` werden relativ zum Flex-Container aufgelöst. Wenn die Größe des Flex-Containers unbestimmt ist, wird der verwendete Wert für `flex-basis` als `content` genommen.
...
- `content`
  - : Gibt eine automatische Größenanpassung auf Basis des Inhalts des Flex-Elements an.
...
Wir fügen zwei Flex-Container mit der gleichen Struktur hinzu, die ähnlich gestylt werden, außer hinsichtlich ihrer `flex-basis`-Werte. Die Container haben jeweils zwei Kinder: ein Kopfzeilen-`<div>` und ein `<section>`. Das `<section>`-Element hat ein Inhalts-`<div>`-Kind, das nicht als Flex-Element festgelegt wird, aber eine Höhe erhalten wird.
...
Wir gestalten die Container als Inline-Flex-Container, die nebeneinander angezeigt werden sollen, um den Vergleich zu erleichtern. Wir setzen die `flex-direction` auf `column`. Die Flex-Elemente des ersten Containers haben einen `flex-basis`-Wert von `0`, während die des zweiten Containers einen `flex-basis`-Wert von `0%` haben. Weder die Flex-Container noch ihre Flex-Elemente haben eine explizit festgelegte Höhe, aber die Höhen der `section`-Elemente können `200px` nicht überschreiten und ihre Kinder haben eine Höhe von `300px`.

...
Er wird in den Farb-Funktionen verwendet, die einen Farbton als Einzelwert akzeptieren, insbesondere in den funktionalen Notationen [`hsl()`](/de/docs/Web/CSS/color_value/hsl), [`hwb()`](/de/docs/Web/CSS/color_value/hwb), [`lch()`](/de/docs/Web/CSS/color_value/lch) und [`oklch()`](/de/docs/Web/CSS/color_value/oklch).
...
- {{CSSXref("&lt;angle&gt;")}}
  - : Ein Winkel, ausgedrückt in Grad, Graden, Radianten oder Umdrehungen, mit `deg`, `grad`, `rad` oder `turn` entsprechend.
- `<number>`
...
Das obige Farbrad zeigt Farbtöne bei allen Winkeln im [sRGB](https://en.wikipedia.org/wiki/SRGB) {{Glossary("color_space", "Farbraum")}}. Insbesondere ist _rot_ bei `0deg`, _gelb_ bei `60deg`, _lime_ bei `120deg`, _cyan_ bei `180deg`, _blau_ bei `240deg` und _magenta_ bei `300deg`.
...
Das folgende Beispiel zeigt die Wirkung des Änderns des `hue`-Wertes der [`hsl()`](/de/docs/Web/CSS/color_value/hsl)-Funktionsnotation auf eine Farbe.

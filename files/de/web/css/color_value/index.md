...
Ein `<color>` kann auch einen [Alpha-Kanal](https://en.wikipedia.org/wiki/Alpha_compositing) _Transparenzwert_ enthalten, der angibt, wie die Farbe mit ihrem Hintergrund [kompositieren](https://drafts.fxtf.org/compositing-1/#simplealphacompositing) soll.
...
- Durch `<color-function>` mit Parametern in einem {{Glossary("color_space", "Farbraum")}} unter Verwendung funktioneller Notationen:
...
- Durch [relative Farbsyntax](/de/docs/Web/CSS/CSS_colors/Relative_colors) zur Ausgabe einer neuen Farbe basierend auf einer vorhandenen Farbe. Jede der obigen Farbfunktionen kann eine **Ursprungsfarbe** mit dem Schlüsselwort `from` und darauf folgende Definitionen der Kanalwerte für die neue **Ausgabefarbe** verwenden.
- Durch Mischen zweier Farben: {{CSSXref("color_value/color-mix", "color-mix()")}}.
- Durch Angabe einer Farbe, für die eine kontrastreiche Farbe zurückgegeben werden soll: {{CSSXref("color_value/contrast-color", "contrast-color()")}}.
...
Das `currentColor`-Schlüsselwort repräsentiert den Wert der {{Cssxref("color")}}-Eigenschaft eines Elements. Dadurch können Sie den `color`-Wert auf Eigenschaften verwenden, die diesen standardmäßig nicht erhalten.
...
Jede Komponente einer beliebigen CSS-Farbfunktion - außer bei der Verwendung der veralteten komma-separierten Syntax - kann als Schlüsselwort `none` angegeben werden, um eine fehlende Komponente darzustellen.

Das explizite Festlegen von [fehlenden Komponenten in der Farbabstufung](#interpolation_mit_fehlenden_komponenten) ist in Fällen nützlich, in denen Sie einige Farbkomponenten interpolieren, andere jedoch nicht möchten. Für alle anderen Zwecke hat eine fehlende Komponente effektiv einen Nullwert in einer geeigneten Einheit: `0`, `0%` oder `0deg`. Zum Beispiel sind die folgenden Farben gleichbedeutend, wenn sie außerhalb der Interpolation verwendet werden:
...
Beim Interpolieren von `<color>`-Werten werden diese zunächst in einen bestimmten Farbraum konvertiert, und dann wird jede Komponente der [berechneten Werte](/de/docs/Web/CSS/CSS_cascade/Value_processing#computed_value) linear interpoliert, wobei die Geschwindigkeit der Interpolation von der [Easing-Funktion](/de/docs/Web/CSS/easing-function) in Übergängen und Animationen bestimmt wird. Der Standardfarbraum für die Interpolation ist Oklab, kann aber bei einigen farbbezogenen funktionellen Notationen durch {{CSSXref("&lt;color-interpolation-method&gt;")}} überschrieben werden.

### Interpolation mit fehlenden Komponenten

#### Interpolation von Farben im gleichen Raum

Beim Interpolieren von Farben, die sich genau im Interpolationsfarbraum befinden, werden fehlende Komponenten einer Farbe durch vorhandene Werte derselben Komponenten aus der anderen Farbe ersetzt. Zum Beispiel sind die folgenden beiden Ausdrücke gleichbedeutend:
...
Wenn eine zu interpolierende Farbe nicht im Interpolationsfarbraum ist, werden ihre fehlenden Komponenten in die konvertierte Farbe basierend auf **analogen Komponenten** derselben Kategorie übertragen, wie in der folgenden Tabelle beschrieben:
...
Das Vorbereitungsverfahren ist:
...
Einige Menschen haben Schwierigkeiten, Farben zu unterscheiden. Die [WCAG 2.2](/de/docs/Web/Accessibility/Guides/Understanding_WCAG/Perceivable/Use_of_color) Empfehlung rät dringend davon ab, Farbe als einziges Mittel zur Übermittlung einer spezifischen Nachricht, Aktion oder eines Ergebnisses zu verwenden. Weitere Informationen finden Sie unter [Farbe und Farbkontrast](/de/docs/Web/Accessibility/Guides/Understanding_WCAG/Perceivable/Color_contrast).
...
In diesem Beispiel bieten wir ein `<div>` und eine Texteingabe an. Die Eingabe einer gültigen Farbe in das Feld führt dazu, dass das `<div>` diese Farbe annimmt, sodass Sie unsere Farbwerte testen können.
...
### Erstellen unterschiedlicher Rotschattierungen

Dieses Beispiel zeigt verschiedene Rot-Schattierungen im sRGB-Farbraum.
...
### Erstellung von Rottönen mit unterschiedlicher Sättigung
...
- {{CSSXref("opacity")}}: die Eigenschaft zur Definition von Transparenz auf Elementebene

...
- `angle`
  - : Eine Berechnung, die zu einer {{cssxref("&lt;number&gt;")}} oder einem {{cssxref("&lt;angle&gt;")}} aufgelöst wird. Wenn zahlenlose Zahlen angegeben werden, werden diese als Anzahl von Radiant interpretiert, welche einen {{cssxref("&lt;angle&gt;")}} darstellen.
...
### Ändern der Kasten-Größe

In diesem Beispiel wird `sin(30deg)` `0.5` zurückgeben, wodurch der Kasten eine Breite von `50px` und eine Höhe von `50px` hat.
...
Ein weiterer Anwendungsfall ist die Steuerung der {{cssxref("animation-duration")}}, wobei die Dauer basierend auf dem Sinuswert reduziert wird. In diesem Fall beträgt die Animationsdauer `1s`.

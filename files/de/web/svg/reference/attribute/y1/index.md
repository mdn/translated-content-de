...
Das **`y1`**-Attribut wird verwendet, um die erste y-Koordinate zum Zeichnen eines SVG-Elements anzugeben, das mehr als eine Koordinate benötigt. Elemente, die nur eine Koordinate benötigen, verwenden stattdessen das {{SVGAttr("y")}}-Attribut.
...
Für {{SVGElement('linearGradient')}} definiert `y1` die y-Koordinate des Startpunkts des _Gradientenvektors_, der zur Abbildung der Gradientenstoppwerte verwendet wird. Das genaue Verhalten dieses Attributs wird von den {{SVGAttr('gradientUnits')}}-Attributen beeinflusst.

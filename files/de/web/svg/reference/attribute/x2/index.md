...
Das **`x2`**-Attribut wird verwendet, um die zweite x-Koordinate für das Zeichnen eines SVG-Elements anzugeben, das mehr als eine Koordinate erfordert. Elemente, die nur eine Koordinate benötigen, verwenden stattdessen das {{SVGAttr("x")}}-Attribut.
...
Für {{SVGElement('linearGradient')}} definiert `x2` die x-Koordinate des Endpunkts des _Gradientenvektors_, der verwendet wird, um die Werte der Farbverläufe zuzuordnen. Das genaue Verhalten dieses Attributs wird durch die {{SVGAttr('gradientUnits')}}-Attribute beeinflusst.

...
Die einzigen Bildformate, die von SVG-Software unterstützt werden müssen, sind {{Glossary("JPEG", "JPEG")}}, {{Glossary("PNG", "PNG")}} und andere SVG-Dateien. Das Verhalten von animierten {{Glossary("GIF", "GIF")}}s ist nicht definiert.
...
- {{SVGAttr("crossorigin")}}
  - : Bestimmt den Wert der Berechtigungsflagge für CORS-Anfragen.
    _Wertetyp_: [ `anonymous` | `use-credentials` ]?; _Standardwert_: Keine; _Animierbar_: **ja**
- {{SVGAttr("decoding")}}
  - : Gibt dem Browser einen Hinweis, ob die Bilddecodierung synchron oder asynchron durchgeführt werden soll.
...
- {{SVGAttr("fetchpriority")}} {{experimental_inline}} {{non-standard_inline}}
  - : Gibt einen Hinweis auf die relative Priorität bei der Abrufung eines externen Bildes.
    Erlaubte Werte:
    - `high`

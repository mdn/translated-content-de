...
> [!NOTE]
> Die von {{SVGElement("use")}} Elementen referenzierten Elemente erben die Stile dieses Elements. Um unterschiedliche Stile auf sie anzuwenden, sollten Sie [CSS-Custom-Properties](/de/docs/Web/CSS/CSS_cascading_variables) verwenden.

## Beispiel

Erstellen Sie ein neues SVG-Dokument als einfache Textdatei, `doc8.svg`. Kopieren Sie den Inhalt von hier und fügen Sie ihn dort ein, und stellen Sie sicher, dass Sie nach unten scrollen, um alles zu erfassen:
...
Erstellen Sie eine neue CSS-Datei, `style8.css`, im gleichen Verzeichnis wie `doc8.svg`. Kopieren Sie den Inhalt von hier und fügen Sie ihn dort ein, und stellen Sie sicher, dass Sie nach unten scrollen, um alles zu erfassen:
...
Ändern Sie das Stylesheet so, dass die inneren Blütenblätter alle rosa werden, wenn der Mauszeiger über eines von ihnen bewegt wird, ohne die Funktionsweise der äußeren Blütenblätter zu ändern.

[Sehen Sie eine Lösung zu dieser Herausforderung.](/de/docs/Learn_web_development/Core/Challenges#svg_and_css)

## Vereinfachte Struktur

Die oben gezeigte SVG-Struktur könnte viel kürzer geschrieben werden, indem die einzelnen Teile der Blume über {{SVGElement("use")}} Elemente referenziert werden. Dieses [Verhalten ist standardisiert](https://svgwg.org/svg2-draft/struct.html#UseStyleInheritance), auch wenn derzeit nur wenige Browser die {{cssxref(":hover")}} Pseudoklasse und andere komplexere CSS-Selektoren auf Elemente unterstützen, die über `<use>` Elemente referenziert werden. Es gibt [einige Diskussionen darüber, welche Regeln auf solche referenzierten Elemente angewendet werden](https://github.com/w3c/svgwg/issues/504).
...
In dieser Demonstration weiß Ihr SVG-fähiger Browser bereits, wie SVG-Elemente angezeigt werden. Das Stylesheet modifiziert die Anzeige nur auf bestimmte Weise. Dies gilt auch für HTML-Dokumente. Aber Sie können CSS auch für allgemeine XML-Dokumente verwenden, bei denen es keine vordefinierte Art gibt, die Elemente anzuzeigen. Die nächste Seite zeigt dies: [XML-Einführung](/de/docs/Web/XML/Guides/XML_introduction)

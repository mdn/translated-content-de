...
Das **`<desc>`**-Element liefert eine zugängliche, langtextliche Beschreibung eines beliebigen SVG-[Container-Elements](/de/docs/Web/SVG/Element#container_elements) oder [Grafik-Elements](/de/docs/Web/SVG/Element#graphics_elements).
...
Der versteckte Text eines `<desc>`-Elements kann auch mit dem sichtbaren Text anderer Elemente durch die Verwendung mehrerer IDs in einem `aria-describedby`-Wert verkettet werden. In diesem Fall muss das `<desc>`-Element eine ID zur Referenzierung bereitstellen.

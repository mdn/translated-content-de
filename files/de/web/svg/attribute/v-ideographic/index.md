---
title: v-ideographic
slug: Web/SVG/Attribute/v-ideographic
l10n:
  sourceCommit: 8799df4d0dbc74bdcf0de1e7a24563a46dcb2478
---

{{SVGRef}}{{Deprecated_Header}}

Das **`v-ideographic`**-Attribut gibt die Ausrichtungskoordinate für [Glyphen](/de/docs/Glossary/glyph) an, um eine ideographische [Basislinie](/de/docs/Glossary/baseline/typography) für vertikal ausgerichtete Glyphenlayouts zu erreichen. Der Wert ist ein Versatz im Schriftkoordinatensystem relativ zum spezifischen {{SVGAttr("vert-origin-x")}}-Attribut der Glyphe.

Dieses Attribut kann mit den folgenden SVG-Elementen verwendet werden:

- {{SVGElement("font-face")}}

## Verwendungshinweise

<table class="properties">
  <tbody>
    <tr>
      <th scope="row">Wert</th>
      <td>
        <code
          ><a href="/de/docs/Web/SVG/Content_type#number"
            >&#x3C;number></a
          ></code
        >
      </td>
    </tr>
    <tr>
      <th scope="row">Standardwert</th>
      <td><em>Keiner</em></td>
    </tr>
    <tr>
      <th scope="row">Animierbar</th>
      <td>Nein</td>
    </tr>
  </tbody>
</table>

- `<number>`
  - : Dieser Wert gibt die Ausrichtungskoordinate für die Glyphen an.

## Spezifikationen

{{Specifications}}

## Browser-Kompatibilität

{{Compat}}

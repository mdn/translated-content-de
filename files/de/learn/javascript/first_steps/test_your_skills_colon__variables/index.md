...
Ziel dieses Fähigkeitstests ist es zu beurteilen, ob Sie unseren Artikel [Die benötigten Informationen speichern — Variablen](/de/docs/Learn/JavaScript/First_steps/Variables) verstanden haben.
...
- `myName` mit einem geeigneten Wert initialisieren, in einer separaten Zeile (Sie können Ihren tatsächlichen Namen verwenden, oder etwas anderes).
- Eine Variable namens `myAge` deklarieren und sie in derselben Zeile initialisieren.
...
In dieser Aufgabe müssen Sie eine neue Zeile hinzufügen, um den in der vorhandenen Variable `myName` gespeicherten Wert auf Ihren eigenen Namen zu korrigieren.
...
Die letzte Aufgabe für jetzt — in diesem Fall wird Ihnen ein vorhandener Code bereitgestellt, der zwei Fehler enthält. Das Resultatsfenster sollte den Namen `Chris` ausgeben und eine Aussage darüber, wie alt Chris in 20 Jahren sein wird. Wie können Sie das Problem beheben und die Ausgabe korrigieren?

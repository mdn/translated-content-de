...
Zu guter Letzt beachten Sie die Syntax von `<input>` vs. `<textarea></textarea>`. Dies ist eine Besonderheit von HTML. Das `<input>`-Tag ist ein {{Glossary("void_element", "leeres Element")}}, was bedeutet, dass es kein schließendes Tag benötigt. {{HTMLElement("textarea")}} ist kein leeres Element, das bedeutet, es sollte mit dem korrekten End-Tag geschlossen werden. Dies hat einen Einfluss auf ein spezielles Feature von Formularen: die Art und Weise, wie Sie den Standardwert festlegen. Um den Standardwert eines {{HTMLElement("input")}}-Elements festzulegen, müssen Sie das [`value`](/de/docs/Web/HTML/Element/input#value)-Attribut verwenden, so:
